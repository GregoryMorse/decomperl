...
%cd("d:/source/repos/efop/EFOP_362/work/wp-a/a11/morse/decomperl").
%filelib:ensure_dir("ebin/").
%filelib:ensure_dir("temp/").
%c("src/semequiv.erl", [debug_info,{outdir, "ebin"}]).
%code:add_path("ebin").
%c("src/decomp.erl", [{outdir, "ebin"}]). code:add_path("ebin").
%decomp:disassemble("temp/calcpi.beamasm", "calcpi").
%decomp:disassemble("temp/emulator.beamasm", "emulator").
%decomp:getabstractsyntax("ebin/decomptest.beam", "temp/decomptest.ast").
%decomp:transform("ebin/emulator.beam", "temp/em.erl").

%decomp:decompile("ebin/decomptest", "temp/out.erl", [writeast,bfsscan,compile,stubfuncs]).
%decomp:decompile("ebin/decomptest", matchhelper, 0, "temp/out.erl", [writeast,stubfuncs,optimize]).
%decomp:decompile("ebin/decomptest", testtrycatchrecrs, 0, "temp/out.erl", []).
%decomp:decompile(""c:/Program Files/erl-23.0/lib/asn1-5.0.13/ebin/asn1ct.beam", set_passes, 0, "temp/out.erl", []).
...
%"%ProgramFiles%\erl-23.0\bin\escript.exe" erlgrind fprof.trace
...
%decomp:decomp_otp("beam_block_SUITE", get_map_elements, 3).
%decomp:decomp_lib("mnesia").
%decomp:decomp_lib("erts", "erlang").
%decomp:decomp_lib("megaco", "megaco_ber_media_gateway_control_prev3a", unused_bitlist, 3).

%decomp:decomp_lib("kernel", "disk_log", log_loop, 7).
%decomp:decomp_lib("kernel", "file_io_server", get_chars_apply, 7).
%decomp:decomp_lib("ssl", "ssl", ssl_accept,3).
%decomp:decomp_lib("stdlib", "ets", file2tab,2).
%decomp:decomp_lib("stdlib", "unicode", characters_to_binary,3).
%decomp:decomp_lib("orber", "orber", dbg, 3).

%decomp:decomp_lib("common_test", "test_server").
%decomp:decomp_lib("stdlib", "ets", tab2file,3).
%decomp:decomp_lib("runtime_tools", "observer_backend", get_table_list,2).
%decomp:decomp_lib("erts", "erlang", convert_time_unit, 3).
%decomp:decomp_lib("stdlib", "binary").
%decomp:decomp_lib("stdlib", "ms_transform").


%decomp:decomp_lib("observer", "observer_trace_wx", handle_event,2).
%decomp:decomp_lib("stdlib", "unicode_util", unicode_table,1).
...
-define(LibPath, code:root_dir() ++ "/lib/"). %os:getenv("ProgramFiles") ++ "/erl-23.0/lib/"). %/usr/lib/erlang/lib
-define(OTPPath, os:getenv("OTPSOURCEPATH")). %os:getenv("USERPROFILE") ++ "/Desktop/Apps/otp-OTP-23.0").
...
-export([gen_paper_dot/0, check_lib_stats/0, decomp_all_lib/0, decomp_lib/1,
	...
	do_get_dfs_parent/3, do_dfs_parent/4, do_tarjan_immdom/3, do_dfs/2,
	...
	random_graph/1, test_rev_graph/1, test_inc_algo/1, test_inc_nca/1, check_tarjan_scc/0, check_newalgo/0,
	check_sgl_loops/0, make_dot_graph/0,
	succ_to_pred/1, get_j_succ/2, check_pred_succ/1, traverse_ast/3,
	ast_otp/0, ast_all_lib/0, check_ast_lib/1,
	disassemble/2, transform/2, disassembleToDis/2, getabstractsyntax/2, get_clean_ast/4,
	decomp_dir/4, decompileast/4, decompile/3, decompile/5]).
...
get_lib_ver_str(Lib) ->
  L = filelib:wildcard(?LibPath ++ Lib ++ "-*"),
  case L of [F] -> lists:nthtail(length(Lib) + 1, filename:basename(F));
  _ -> "" end.

ast_otp() -> filelib:ensure_dir("temp/compiler_test/"),
	filelib:fold_files(?OTPPath ++ "/lib/compiler/test",
		".*\.erl$", false, fun (Filename, _Acc) ->
			{ok, AST} = epp:parse_file(Filename, []),
			lists:all(fun (El) -> check_traverse_ast(fun (E, A) -> {E, A} end, [], El), true end, AST), erlang:display(Filename) end, []).
check_ast_lib(Filename) ->
{ok, AST} = epp:parse_file(Filename, [{includes, ["temp", "temp/misc", "temp/kernel/src",
			filename:dirname(Filename), filename:dirname(Filename) ++ "/../include",
			?LibPath ++ "snmp-" ++ get_lib_ver_str("snmp") ++ "/src/compiler", ?LibPath ++ "wx-" ++ get_lib_ver_str("wx") ++ "/src",
			?LibPath ++ "et-" ++ get_lib_ver_str("et") ++ "/include", ?LibPath ++ "snmp-" ++ get_lib_ver_str("snmp") ++ "/src/misc",
			?LibPath ++ "stdlib-" ++ get_lib_ver_str("stdlib") ++ "/include", ?LibPath ++ "eldap-" ++ get_lib_ver_str("eldap") ++ "/ebin",
			?LibPath ++ "xmerl-" ++ get_lib_ver_str("xmerl") ++ "/include", ?LibPath ++ "kernel-" ++ get_lib_ver_str("kernel") ++ "/src",
		  ?LibPath ++ "kernel-" ++ get_lib_ver_str("kernel") ++ "/include", ?LibPath ++ "snmp-" ++ get_lib_ver_str("snmp") ++ "/include"]},
		  {macros, [{'COMPILER_VSN',"7.2.7"},{'ENABLE_MEGACO_FLEX_SCANNER',true},
		  {'MEGACO_REENTRANT_FLEX_SCANNER',true}|case Filename =:= ?LibPath ++ "kernel-" ++ get_lib_ver_str("kernel") ++ "/src/group_history.erl" of true -> []; _ -> [{'VSN',"3.3.1"}] end ++
		  case Filename =:= ?LibPath ++ "common_test-" ++ get_lib_ver_str("common_test") ++ "/src/ct_run.erl" orelse Filename =:= ?LibPath ++ "common_test-" ++ get_lib_ver_str("common_test") ++ "/src/ct_util.erl" of true -> []; _ -> [{'default_verbosity',silence}] end ++
		  case Filename =:= ?LibPath ++ "tools-" ++ get_lib_ver_str("tools") ++ "/src/lcnt.erl" of true -> []; _ -> [{'version',"5.3"}] end]}]), %{'VSN',"6.1"}]}]),
			lists:all(fun (El) -> check_traverse_ast(fun (E, A) -> {E, A} end, [], El), true end, AST).
ast_all_lib() ->
	filelib:ensure_dir("temp/kernel/src/"),
	file:copy(?LibPath ++ "kernel-" ++ get_lib_ver_str("kernel") ++ "/src/logger_internal.hrl", "temp/kernel/src/logger_internal.hrl"),
	filelib:ensure_dir("temp/compiler/src/"),
	file:copy(?LibPath ++ "compiler-" ++ get_lib_ver_str("compiler") ++ "/src/beam_disasm.hrl", "temp/compiler/src/beam_disasm.hrl"),
	file:copy(?LibPath ++ "compiler-" ++ get_lib_ver_str("compiler") ++ "/src/core_parse.hrl", "temp/compiler/src/core_parse.hrl"),
	filelib:ensure_dir("temp/hipe/main/"),
	file:copy(?LibPath ++ "hipe-" ++ get_lib_ver_str("hipe") ++ "/main/hipe.hrl", "temp/hipe/main/hipe.hrl"),
	filelib:ensure_dir("temp/otp_mibs/include/"),
	file:copy(?LibPath ++ "otp_mibs-" ++ get_lib_ver_str("otp_mibs") ++ "/include/OTP-MIB.hrl", "temp/otp_mibs/include/OTP-MIB.hrl"),
  Files = ["https://raw.githubusercontent.com/erlang/otp/master/lib/ftp/src/ftp_internal.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/flow/hipe_bb.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/flow/cfg.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/misc/hipe_consttab.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/ssa/hipe_ssa.inc",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/ssa/hipe_ssa_liveness.inc",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/ssa/hipe_ssa_const_prop.inc",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/hipe/ssa/hipe_ssa_copy_prop.inc",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/inets/examples/httpd_load_test/hdlt_logger.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/inets/src/http_lib/http_internal.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/inets/src/inets_app/inets_internal.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/lib/common_test/src/test_server_internal.hrl",
  	"https://raw.githubusercontent.com/erlang/otp/master/erts/preloaded/src/zip_internal.hrl"],
  Dload = lists:map(fun (El) -> filelib:is_file("temp/" ++ filename:basename(El)) end, Files),
  case not lists:all(fun (El) -> El end, Dload) of true ->
	  inets:start(),
	  ssl:start(), %otherwise use ibrowse?
	  lists:map(fun ({E, Dl}) ->
	  if Dl -> true; true ->
	  case httpc:request(get, {E, []}, [], []) of
	    {ok, {{_, 200, "OK"}, _Headers, Body}} ->
	      file:write_file("temp/" ++ filename:basename(E), Body); _ -> true end end end, lists:zip(Files, Dload)),
  	ssl:stop(),
  	inets:stop(); _ -> true end,
	filelib:ensure_dir("temp/misc/"),
	file:copy("temp/hipe_consttab.hrl", "temp/misc/hipe_consttab.hrl"),
	filelib:ensure_dir("temp/flow/"),
	file:copy("temp/cfg.hrl", "temp/flow/cfg.hrl"),
	file:copy("temp/hipe_bb.hrl", "temp/flow/hipe_bb.hrl"),
	filelib:ensure_dir("temp/ssa/"),
	file:copy("temp/hipe_ssa.inc", "temp/ssa/hipe_ssa.inc"),
	file:copy("temp/hipe_ssa_liveness.inc", "temp/ssa/hipe_ssa_liveness.inc"),
	file:copy("temp/hipe_ssa_const_prop.inc", "temp/ssa/hipe_ssa_const_prop.inc"),
	file:copy("temp/hipe_ssa_copy_prop.inc", "temp/ssa/hipe_ssa_copy_prop.inc"),
	%megaco requires its ENABLE_MEGACO_FLEX_SCANNER and MEGACO_REENTRANT_FLEX_SCANNER set to true or false: https://github.com/erlang/otp/blob/master/lib/megaco/configure.in
  %compiler requires its COMPILER_VSN set: https://github.com/erlang/otp/blob/master/lib/compiler/vsn.mk
  %dialyzer requires its DIALYZER_VSN set to a VSN macro: https://github.com/erlang/otp/blob/master/lib/dialyzer/vsn.mk
  %kernel requires its KERNEL_VSN set to a VSN macro: https://githubcom/erlang/otp/master/lib/kernel/vsn.mk
  %snmp agent requies version set to VSN and PRE_VSN: https://github.com/erlang/otp/blob/master/lib/snmp/src/agent/Makefile https://github.com/erlang/otp/blob/master/lib/snmp/vsn.mk
	%https://github.com/erlang/otp/blob/master/lib/common_test/src/test_server_internal.hrl is not present, must be downloaded and included
	%https://github.com/erlang/otp/blob/master/erts/preloaded/src/zip_internal.hrl is not present, must be downloaded and included
	%kernel\include and kernel\src are both referenced not with kernel-6.1 versioned directory
	filelib:fold_files(?LibPath, ".*\.erl$",
		true, fun (Filename, _Acc) ->
		check_ast_lib(Filename), erlang:display(Filename) end, []).
...
decomp_dir(Dir, Recurse, OutDir, Options) ->
	filelib:fold_files(Dir, ".*\.beam$", Recurse, fun (Filename, _Acc) ->
			filelib:ensure_dir(OutDir ++ "/" ++ lists:nthtail(length(Dir), filename:dirname(Filename)) ++ "/"),
				decomp:decompile(Filename, OutDir ++ "/" ++ lists:nthtail(length(Dir), filename:dirname(Filename)) ++ "/" ++
					filename:basename(Filename, ".beam") ++ ".erl", Options) end, []).
...
decomp_lib(LibName) ->
	decomp_dir(?LibPath ++ LibName ++ "-" ++ get_lib_ver_str(LibName) ++
		"/ebin", false, "temp/" ++ LibName, [writeast,compile,stubfuncs]).
	%filelib:fold_files(?LibPath ++ LibName ++ "-" ++ get_lib_ver_str(LibName) ++
...
decomp_lib(LibName, ModName) -> decomp:decompile(
	?LibPath ++ LibName ++ "-" ++ get_lib_ver_str(LibName) ++ "/ebin/" ++ ModName, "temp/" ++ LibName ++ "/" ++
	ModName ++ ".erl", [writeast,compile,stubfuncs]).
decomp_lib(LibName, ModName, FuncName, Arity) ->
	decomp:decompile(
	?LibPath ++ LibName ++ "-" ++ get_lib_ver_str(LibName) ++ "/ebin/" ++ ModName, FuncName, Arity,
	"temp/" ++ LibName ++ "/" ++ ModName ++ ".erl", [progress,dotfile,writeast,stubfuncs]).
decomp_otp() -> filelib:ensure_dir("temp/compiler_test/"),
	filelib:fold_files(?OTPPath ++ "/lib/compiler/test",
		".*\.erl$", false, fun (Filename, _Acc) ->
			filelib:ensure_dir("temp/compiler_test/"),
			case filelib:is_file("temp/compiler_test/" ++ filename:basename(Filename, ".erl") ++ ".beam") of
			true -> true; _ -> compile:file(Filename, [{outdir, "temp/compiler_test"}]) end end, []),
	decomp_dir("temp/compiler_test", false, "temp/compiler_test", [writeast,compile,stubfuncs]).
	%filelib:fold_files(?OTPPath ++ "/lib/compiler/test",
	%	".*\.erl$", false, fun (Filename, _Acc) ->
	%		filelib:ensure_dir("temp/compiler_test/"),
...
decomp_otp(ModName) -> filelib:ensure_dir("temp/compiler_test/"),
	Filename =
		?OTPPath ++ "/lib/compiler/test" ++ "/" ++ ModName,
	case filelib:is_file("temp/compiler_test/" ++ filename:basename(Filename, ".erl") ++ ".beam") of
		true -> true; _ -> compile:file(Filename, [{outdir, "temp/compiler_test"}]) end,
	decomp:decompile("temp/compiler_test/" ++ filename:basename(Filename, ".erl"),
		"temp/compiler_test/" ++ filename:basename(Filename, ".erl") ++ ".erl", [writeast,compile,stubfuncs]).
decomp_otp(ModName, FuncName, Arity) -> filelib:ensure_dir("temp/compiler_test/"),
	Filename =
		?OTPPath ++ "/lib/compiler/test" ++ "/" ++ ModName,
	case filelib:is_file("temp/compiler_test/" ++ filename:basename(Filename, ".erl") ++ ".beam") of
		true -> true; _ -> compile:file(Filename, [{outdir, "temp/compiler_test"}]) end,
	decomp:decompile("temp/compiler_test/" ++ filename:basename(Filename, ".erl"), FuncName, Arity, 
...
disassembleToDis(Outfile, Fname) ->
	Filename = case is_atom(Fname) of true -> code:which(Fname); _ -> Fname end,
	%a bit dangerous to do for long term use without restoring...
	%scripts/diffable also has script to go from tree hierarchical .S format to flat format
	%where labels are not symbolic but resolved, and no nesting but merely dot terminated
	%code:unstick_dir(?LibPath ++ "compiler-" ++ get_lib_ver_str("compiler") ++ "/ebin"),
	code:unstick_mod(beam_disasm),
	c:c(?LibPath ++ "compiler-" ++ get_lib_ver_str("compiler") ++ "/src/beam_disasm.erl", [debug_info,{d,'DEBUG_DISASM'},{outdir, "ebin"}]),
	DisasmCode = element(6, beam_disasm:file(Filename)), beam_disasm:pp(Outfile, [{file, Filename}, {code, DisasmCode}]),
	%code:stick_dir(?LibPath ++ "compiler-" ++ get_lib_ver_str("compiler") ++ "/ebin"),
	ok
.

transform(Fname, ErlFName) ->
	Filename = case is_atom(Fname) of true -> code:which(Fname); _ -> Fname end,
  case beam_lib:chunks(Filename, [abstract_code]) of
	{ok, {_, [{abstract_code, {raw_abstract_v1,Forms}}]}} ->
	  Src = 
		erl_prettypr:format(erl_syntax:form_list(tl(Forms))),
		{ok, Fd} = file:open(ErlFName, [write,{encoding,utf8}]),
		io:fwrite(Fd, "~s~n", [Src]),
		file:close(Fd); 
	Error -> Error
  end
.
getabstractsyntax(Fname, ErlFName) ->
	Filename = case is_atom(Fname) of true -> code:which(Fname); _ -> Fname end,
  case beam_lib:chunks(Filename, [abstract_code]) of
	{ok, {_, [{abstract_code, {raw_abstract_v1,Forms}}]}} ->
	  %Src = tl(Forms),
	  Src = lists:map(fun clean_ast_linenums/1, tl(Forms)),
	  %{ok,_,S} = compile:forms(Src, [to_pp, 'S']), %'S'=to_asm, to_core, to_core0. to_kernel, to_exp, to_pp
		{ok, Fd} = file:open(ErlFName, [write,{encoding,utf8}]),
...
%0 type verifiers: is_integer, is_float, is_atom, is_list(of is_integer)
%other type verifiers: literal subsetof safe subset of (patternmatch and guard) subsetofs expression
%comma delimiter in function calls, map and binary enumerating elements, makes it so some sort of limited expressions are allowed only
%apparently a syntactic issue with the function call ternary colon-parenthesis operator : () exists so parenthesis cannot be used without begin...end blocks
%limiting of syntax apparently also occurs with fun : / quarternary operator
%negation is represented as op and can appear in patternmatch subset
check_lookup_ast_args(AST) -> %erlang:display(AST),
	Key = element(1, AST),
	Safe = [var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record,op],
	PatternMatch = [match|Safe],
	Guard = [call,record_field|Safe],
	Expression = Safe ++ [match,call,record_field,record_index,block,'case','if','catch','fun',named_fun,'try','receive',lc,bc],
	Map = #{
		%0 - literal, 1 - traversal, 2 - list traversal
		var => [{is_atom}], char => [{is_integer}], integer => [{is_integer}], float => [{is_float}],
		atom => [{is_atom}], string => [{is_list}], nil => [{}],
		%cons => [{{1,[char,integer,float,tuple,cons,nil,string,atom,var,map,bin,record,record_field,record_index,match,call,op,block,'case','if','catch','fun','try',lc]},{1,[cons,nil,string,var,integer,atom,tuple,map,bin,record,record_field,match,call,op,block,'case','if','fun','try',lc]}}], tuple => [{{2,[cons,nil,string,char,integer,float,atom,tuple,var,map,bin,record,record_field,record_index,match,call,op,block,'case','if','fun','catch','try',lc]}}],
		cons => [{{1,Expression},{1,Expression}}], tuple => [{{2,Expression}}],
		%record_field => [{{1,[var,record,record_field,call]},is_atom,{1,[atom]}},{{1,[var,atom]},{1,[var,atom,char,integer,float,cons,nil,string,tuple,bin,map,record,record_field,match,call,op,'case','if','fun',lc]}},{{1,[atom]}}], %record_index => [{0}],
		record_field => [{{1,[var,record,record_field,call,string,integer]},is_atom,{1,[atom]}},{{1,[var,atom]},{1,Expression}},{{1,[atom]}}], %record_index => [{0}],
		record_index => [{is_atom, {1,[atom]}}], record => [{is_atom,{2,[record_field]}},{{1,[var,record_field,call]},is_atom,{2,[record_field]}}],
		map => [{{2,[map_field_assoc,map_field_exact]}},{{1,[map,var,atom,nil,bin,call,'fun',integer,'catch','receive',record_field]},{2,[map_field_assoc,map_field_exact]}}],
		%map_field_assoc => [{{1,[var,atom,integer,float,cons,nil,string,tuple,bin,map,call,op]},{1,[var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record,op,call,block,'if','fun']}}], map_field_exact => [{{1,[var,atom,integer,float,nil,string,tuple,bin,map,op,call]},{1,[var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record,match,op,call]}}],
		map_field_assoc => [{{1,Expression},{1,Expression}}], map_field_exact => [{{1,Expression},{1,Expression}}],
		bin => [{{2,[bin_element]}}], %bin_element => [{{1,[var,atom,char,integer,float,string,bin,record_field,op,call,block,'case','if',bc]},default,bin_element},{{1,[var,char,integer,float,bin,record_field,op,call,block,'case','if']},{1,[var,atom,integer,float,op,call]},bin_element}],
		bin_element => [{{1,Expression},default,bin_element},{{1,Expression},{1,[var,atom,integer,float,op,call,bin,match,char,'case',string,nil,record_field,map,tuple]},bin_element}],
		
		%call => [{{1,[remote,var,atom,integer,float,record_field,call,block,'fun',named_fun]},{2,[var,cons,nil,string,char,integer,float,atom,tuple,map,bin,record,record_field,record_index,match,call,op,block,'case','if','catch','fun',named_fun,'try','receive',lc,bc]}}],
		call => [{{1,[remote,var,atom,integer,float,record_field,call,block,'fun',named_fun,'catch',cons,'try']},{2,Expression}}],
		%match => [{{1,[var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record,match,op]},{1,[var,atom,char,integer,float,cons,nil,string,tuple,bin,map,record,record_field,record_index,match,call,op,block,'case','if','catch','fun',named_fun,'try','receive',lc,bc]}}],
		match => [{{1,PatternMatch},{1,Expression}}],
		%op => [{is_atom,{1,[var,atom,integer,float,map,record_field,match,op,call,block,'case']}},{is_atom,{1,[var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record_field,match,op,call,'case','if','catch','try',lc]},{1,[var,atom,char,integer,float,cons,nil,string,tuple,map,bin,record,record_field,record_index,match,op,call,block,'case','if','catch','fun','try',lc]}}],
		op => [{is_atom,{1,Expression}},{is_atom,{1,Expression},{1,Expression}}],

		%lc => [{{1,[var,atom,char,integer,cons,nil,string,tuple,map,bin,record,record_field,block,match,op,call,'case','if','catch','try','receive',lc]},{2,[generate,b_generate,var,atom,record_field,op,call,block,'case','if']}}], generate => [{{1,[var,atom,char,integer,cons,tuple,bin,map,record,match]},{1,[var,atom,integer,cons,nil,string,record_field,op,call,'case','try',lc]}}],
		lc => [{{1,Expression},{2,[generate,b_generate] ++ Expression}}], generate => [{{1,PatternMatch},{1,Expression}}],
		%bc => [{{1,[var,atom,integer,float,cons,tuple,bin,block,call,'case','if','fun','try','receive',bc]},{2,[generate,b_generate,op,call]}}], b_generate => [{{1,[bin]},{1,[var,tuple,bin,call]}}],
		bc => [{{1,Expression},{2,[generate,b_generate] ++ Expression}}], b_generate => [{{1,[bin]},{1,Expression}}],
		%block => [{{2,[var,cons,nil,string,atom,tuple,bin,record,record_field,match,op,call,block,'if','case','catch','fun','try']}}],
		block => [{{2,Expression}}],
		'if' => [{{2,[clause]}}], %'case' => [{{1,[var,atom,integer,cons,nil,string,tuple,bin,map,record_field,match,op,call,block,'case','if','catch','fun','try',lc]},{2, [clause]}}],
		'case' => [{{1,Expression},{2, [clause]}}],
		%'catch' => [{{1,[atom,integer,cons,tuple,bin,map,record_field,match,call,op,block,'case','if','receive',lc,bc]}}],
		'catch' => [{{1,Expression}}],
		%'try' => [{{2,[var,cons,integer,atom,string,tuple,bin,match,call,op,block,record,'case','if','catch','fun','try','receive',lc]},{2,[clause]},{2,[clause]},{2,[var,atom,nil,match,call,op,'case','if','catch','fun','try','receive',lc]}}],
		'try' => [{{2,Expression},{2,[clause]},{2,[clause]},{2,Expression}}],
		%'receive' => [{{2,[clause]}},{{2,[clause]},{1,[var,atom,integer,nil,record_field,op,call]},{2,[var,atom,string,tuple,nil,match,call,op,record,'case','if','catch','try','receive']}}],
		'receive' => [{{2,[clause]}},{{2,[clause]},{1,[var,atom,integer,nil,record_field,op,call]},{2,Expression}}],
		'fun' => [{{clauses,{2,[clause]}}},{{function,{1,[var,atom]},{1,[var,atom]},{1,[var,integer]}}},{{function,is_atom,is_integer}}],
		named_fun => [{is_atom,{2,[clause]}}],
		remote => [{{1,[var,atom,tuple,record_field,call,op,'case',integer,'catch']},{1,[var,integer,nil,atom,call]}}], %clause => [{{2,[var,cons,nil,string,atom,char,integer,float,tuple,bin,map,record,record_index,match,op]},{3,[var,atom,call,op,nil,cons,tuple,bin,integer,float,string,map,record_field]},{2,[var,cons,nil,string,atom,char,integer,float,tuple,map,bin,match,call,op,block,record,record_field,record_index,'case','if','catch','fun',named_fun,'try','receive',lc,bc]}}],
		clause => [{{2,[var,cons,nil,string,atom,char,integer,float,tuple,bin,map,record,record_index,match,op]},{3,Guard},{2,Expression}}],
		function => [{is_atom,is_integer,{2,[clause]}}],
		
		%http://erlang.org/doc/reference_manual/typespec.html
		typed_record_field => [{{1,[record_field]},{1,[atom,type,user_type,remote_type]}}], type => [{any},{is_atom,any},{is_atom,{2,[type,user_type,remote_type,ann_type,var,atom,integer,op,{2, [type,user_type,remote_type,var,atom]}]}}], %can have a list of types in the type list...
		remote_type => [{[{1,[atom]},{1,[atom]},{2,[type,user_type,remote_type,ann_type,var]}]}], ann_type => [{{2,[type,user_type,remote_type,var,atom]}}], user_type => [{is_atom,{2,[type,user_type,ann_type,var,atom]}}],
		attribute => [{record,{is_atom,{2,[record_field,typed_record_field]}}},
		  {opaque,{is_atom,{1,[type,user_type]},{2, [var]}}}, %set, misc_SUITE_test_cases
			{type,{is_atom,{1,[atom,type,user_type,ann_type,remote_type]},{2, [var]}}}, %all_return_type, type_name
			{spec,{spec, {2, [type]}}}, %2nd spec is actually a {function_name,arity} pair per manual not having an fa atom only mfa
			{callback,{spec, {2, [type]}}},
			{is_atom,attribute}], eof => [{}]},
	case Map of #{Key := StrucPosbl} ->
	StrucL = lists:filter(fun (El) ->
		tuple_size(El) + if Key =:= typed_record_field -> 1; true -> 2 end =:= case AST of [] -> 2; _ -> tuple_size(AST) end end, StrucPosbl),
	case StrucL of [H] -> H; [_|_] -> hd(lists:dropwhile(fun (Hd) ->
		if is_atom(element(1, Hd)) andalso element(1, Hd) =/= is_atom ->
			if element(1, Hd) =:= element(3, AST) -> false;
			true -> true end;
		is_tuple(element(1, Hd)) andalso element(1, element(1, Hd)) =/= 1 ->
			if tuple_size(element(1, Hd)) =:= tuple_size(element(3, AST)) andalso element(1, element(1, Hd)) =:= element(1, element(3, AST)) ->
				false; true -> true end;
		is_atom(element(2, Hd)) andalso element(2, Hd) =/= attribute ->
			if element(2, Hd) =:= element(4, AST) -> false;
			true -> true end;
		true -> false
		end end, StrucL)) end; _ -> error(AST) end.

check_traverse_ast(Fun, Acc, AST) -> 
	Struc = check_lookup_ast_args(AST),
	{NewAST, NewAcc} =
	lists:foldl(fun (El, {CurAST, CurAcc}) ->
		case element(El, Struc) of
		{1, L} -> case lists:member(element(1, element(El + if element(1, CurAST) =:= typed_record_field -> 1; true -> 2 end, CurAST)), L) of false -> error({El, L, AST});
		  _ -> {New, NewAcc} = check_traverse_ast(Fun,CurAcc,element(El + if element(1, CurAST) =:= typed_record_field -> 1; true -> 2 end, CurAST)),
			  {setelement(El + if element(1, CurAST) =:= typed_record_field -> 1; true -> 2 end, CurAST, New), NewAcc} end;
		{2, L} -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
		  case is_list(Elem) andalso lists:any(fun (E) -> case E of {2, _} -> true; _ -> false end end, L) of true ->
		  	LL = element(2, hd(lists:dropwhile(fun (E) -> case E of {2, _} -> false; _ -> true end end, L))),
		    {NewElem, TAc} = lists:foldl(fun (ElemL, {ListL,AcL}) ->
					case lists:member(element(1, ElemL), LL) of false -> error({ElemL,AST}); _ ->
					{NewElemL, TAcL} = check_traverse_ast(Fun,AcL,ElemL), {ListL ++ [NewElemL], TAcL} end end,
						{[], Ac}, Elem),
					{List ++ [NewElem], TAc};
			_ -> case lists:member(element(1, Elem), L) of false -> error({Elem,AST}); _ ->
			{NewElem, TAc} = check_traverse_ast(Fun,Ac,Elem), {List ++ [NewElem], TAc} end end end,
				{[], CurAcc}, element(El + 2, CurAST)),
			{setelement(El + 2, CurAST, New), NewAcc};
		{3, L} -> {New, NewAcc} = lists:foldl(fun (Elem, {Lt, Accu}) ->
			{NewEl, NewAccu} = lists:foldl(fun (E, {List,Ac}) ->
			  case lists:member(element(1, E), L) of false -> error(E); _ ->
				{NewElem, TAc} = check_traverse_ast(Fun,Ac,E), {List ++ [NewElem], TAc} end end,
					{[], Accu}, Elem),
				{Lt ++ [NewEl], NewAccu} end, {[], CurAcc}, element(El + 2, CurAST)),
			{setelement(El + 2, CurAST, New), NewAcc};
		X when is_atom(X), X =:= element(El + 2, CurAST) -> {CurAST, CurAcc};
		X when is_list(X), length(X) =:= length(element(El + 2, CurAST)) ->
			{New, NewAcc} = lists:foldl(fun (E, {NCurAST, NCurAcc}) ->
				case lists:nth(E, X) of 0 -> {NCurAST, NCurAcc};
				{1, L} -> case lists:member(element(1, lists:nth(E, element(El + 2, CurAST))), L) of false -> error(AST);
				  _ -> {New, NewAcc} = check_traverse_ast(Fun,NCurAcc,lists:nth(E, element(El + 2, CurAST))),
					  {NCurAST ++ [New], NewAcc} end;
				{2, L} -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
					case lists:member(element(1, Elem), L) of false -> error({Elem,AST}); _ ->
					{NewElem, TAc} = check_traverse_ast(Fun,Ac,Elem), {List ++ [NewElem], TAc} end end,
						{[], NCurAcc}, lists:nth(E, element(El + 2, CurAST))),
					{NCurAST ++ [New], NewAcc};
				X when is_atom(X) -> case X =:= lists:nth(E, element(El + 2, CurAST)) of true -> {NCurAST, NCurAcc}; _ -> error(AST) end end
				end, {[], CurAcc}, lists:seq(1, length(X))),
			{setelement(El + 2, CurAST, New), NewAcc};		  
		Y when is_tuple(Y), tuple_size(Y) =:= tuple_size(element(El + 2, CurAST)) ->
			{New, NewAcc} = lists:foldl(fun (E, {NCurAST, NCurAcc}) ->
				case element(E, Y) of 0 -> {NCurAST, NCurAcc};
				{1, L} -> case lists:member(element(1, element(E, NCurAST)), L) of false -> error(AST);
				  _ -> {New, NewAcc} = check_traverse_ast(Fun,NCurAcc,element(E, NCurAST)),
					  {setelement(E, NCurAST, New), NewAcc} end;
				{2, L} -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
					case lists:member(element(1, Elem), L) of false -> error({Elem,AST}); _ ->
					{NewElem, TAc} = check_traverse_ast(Fun,Ac,Elem), {List ++ [NewElem], TAc} end end,
						{[], NCurAcc}, element(E, NCurAST)),
					{setelement(E, NCurAST, New), NewAcc};
				X when is_atom(X), X =:= element(E, NCurAST) -> {NCurAST, NCurAcc};
				is_atom when is_atom(element(E, NCurAST)) -> {NCurAST, NCurAcc};
				is_integer when is_integer(element(E, NCurAST)) -> {NCurAST, NCurAcc};
				is_list when is_list(element(E, NCurAST)) -> {NCurAST, NCurAcc};
				spec when is_tuple(element(E, NCurAST)) -> case element(E, NCurAST) of {E1,E2} when is_atom(E1),is_integer(E2) -> {NCurAST, NCurAcc};
					{E1,E2,E3} when is_atom(E1),is_atom(E2),is_integer(E3) -> {NCurAST, NCurAcc}; _ -> error(AST) end;
				_ -> error(AST) end
				end, {element(El + 2, CurAST), CurAcc}, lists:seq(1, tuple_size(Y))),
			{setelement(El + 2, CurAST, New), NewAcc};
		is_atom when is_atom(element(El + 2, CurAST)) -> {CurAST, CurAcc};
		is_integer when is_integer(element(El + 2, CurAST)) -> {CurAST, CurAcc};
		is_float when is_float(element(El + 2, CurAST)) -> {CurAST, CurAcc};
		is_list when is_list(element(El + 2, CurAST)) -> case not lists:all(fun (E) -> is_integer(E) end, element(El + 2, CurAST)) of true -> error(AST); _ -> {CurAST, CurAcc} end;
		attribute when is_list(element(El + 2, CurAST)) -> 
		  case element(El + 1, CurAST) of X when X =:= export; X =:= export_type; X =:= inline; X =:= optional_callbacks; X =:= deprecated; X =:= deprecated_type ->
		    case lists:all(fun (E) -> case E of {E1, E2} when is_atom(E1),is_integer(E2) -> true;
		      {E1, E2} when is_atom(E1),is_atom(E2),X=:=deprecated -> true;
		      {E1, E2, E3} when is_atom(E1),is_integer(E2),is_atom(E3),X=:=deprecated -> true;
			  {E1, E2, E3} when is_atom(E1),is_integer(E2),is_list(E3),X=:=deprecated orelse X =:= deprecated_type -> true;
			  {E1, E2, E3} when is_atom(E1),is_atom(E2),is_list(E3),X=:=deprecated -> true;
		      E1 when is_atom(E1),X=:=optional_callbacks -> true; _ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end; %[{function_name,arity}]
		  export_records -> case lists:all(fun (E) -> is_atom(E) end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end; %[record_name]
		  author -> case lists:all(fun (E) -> is_integer(E) end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end; %[string]
		  compile -> case lists:all(fun (E) -> is_atom(E) orelse case E of {E1, E2} when is_atom(E1),is_atom(E2) -> true;
		  	{E1, {E2,E3,E4}} when is_atom(E1),is_atom(E2),is_atom(E3),is_integer(E4) -> true;
		    {E1, E2} when is_atom(E1),is_list(E2) -> lists:all(fun (E3) -> case E3 of {E4, E5} when is_atom(E4),is_integer(E5);is_atom(E4),is_atom(E5) -> true; {E4, E5, E6} when is_atom(E4),is_atom(E5),is_integer(E6) -> true; _ -> false end end, E2);
				_ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end; %[{key,value}]
		  removed -> case lists:all(fun (E) -> case E of {E1, E2, E3} when is_atom(E1),is_integer(E2) orelse is_atom(E2),is_list(E3) -> true; _ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
		  removed_type -> case lists:all(fun (E) -> case E of {E1, E2, E3} when is_atom(E1),is_integer(E2) orelse is_atom(E2),is_list(E3) -> true; _ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
		  dialyzer -> case lists:all(fun (E) -> case E of E1 when is_atom(E1) -> true; {E1, {E2, E3}} when is_atom(E1),is_atom(E2),is_integer(E3) -> true; _ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
		  asn1_info -> case lists:all(fun (E) -> case E of {E1, E2} when is_atom(E1),is_atom(E2) -> true;
		    {E1, E2} when is_atom(E1),is_list(E2) -> lists:all(fun (E3) -> is_atom(E3) orelse case E3 of {E4, E5} when is_atom(E4),is_list(E5) -> lists:all(fun (E6) -> is_integer(E6) end, E5); _ -> false end end, E2);
		    _ -> false end end, element(El + 2, CurAST)) of true -> {CurAST, CurAcc}; _ -> error(AST) end; %[{key,value}]
		  _ -> error(AST) end;
		attribute when is_tuple(element(El + 2, CurAST)) ->
		  case element(El + 1, CurAST) of file -> case element(El + 2, CurAST) of {E1, E2} when is_list(E1),is_integer(E2) -> case lists:all(fun (E3) -> is_integer(E3) end, E1) of true -> {CurAST, CurAcc}; _ -> error(AST) end; _ -> error(AST) end; %{filename,label_number}
		  on_load -> case element(El + 2, CurAST) of {E1, E2} when is_atom(E1),is_integer(E2) -> {CurAST, CurAcc}; _ -> error(AST) end; %{module_name,label_number}
		  import -> case element(El + 2, CurAST) of {E1, E2} when is_atom(E1),is_list(E2) ->
		    case lists:all(fun (E) -> case E of {E3, E4} when is_atom(E3),is_integer(E4) -> true; _ -> false end end, E2) of true -> {CurAST, CurAcc}; _ -> error(AST) end; _ -> error(AST) end; %{module_name,[{function_name,arity}]}
		  removed -> case element(El + 2, CurAST) of {E1, E2, E3} when E1=:=encode orelse E1=:=decode,is_atom(E2),is_list(E3) -> {CurAST, CurAcc}; _ -> error(AST) end;
		  compile -> case element(El + 2, CurAST) of {E1, E2} when is_atom(E1),is_atom(E2);is_atom(E1),is_integer(E2) -> {CurAST, CurAcc};
		    {E1, E2} when is_atom(E1),is_list(E2) -> case lists:all(fun (E3) -> case E3 of {E4, E5} when is_atom(E4),is_integer(E5) -> true; {E4, E5, E6} when is_atom(E4),is_atom(E5),is_integer(E6) -> true; _ -> false end end, E2) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
		  {E1, {E2,E3}} when is_atom(E1),is_atom(E2),is_integer(E3) -> {CurAST, CurAcc}; _ -> error(AST) end; %{key,value}
		  dialyzer -> case element(El + 2, CurAST) of {E1, {E2, E3}} when is_atom(E1),is_atom(E2),is_integer(E3) -> {CurAST, CurAcc};
		    {E1, E2} when is_atom(E1),is_list(E2) -> case lists:all(fun (E3) -> case E3 of {E4, E5} when is_atom(E4),is_integer(E5) -> true; _ -> false end end, E2) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
			  {E1, {E2, E3}} when is_list(E1),is_atom(E2),is_integer(E3) -> case lists:all(fun (E4) -> is_atom(E4) end, E1) of true -> {CurAST, CurAcc}; _ -> error(AST) end; _ -> error(AST) end; %{option atom|[option atom],{function_name,arity}|[{function_name,arity}]}
		  deprecated -> case element(El + 2, CurAST) of {E1, E2, E3} when is_atom(E1),is_integer(E2) orelse is_atom(E2),is_atom(E3) orelse is_list(E3) -> {CurAST, CurAcc}; _ -> error(AST) end; %{function_name,arity,is_atom}
		  _ -> error(AST) end;
		attribute when is_atom(element(El + 2, CurAST)) ->
		  case lists:member(element(El + 1, CurAST), [author,date,deprecated,dialyzer,behavoiur,behaviour,behavior,vsn,vc,hrl_name,compile,module]) of true -> {CurAST, CurAcc}; _ -> error(AST) end;
		bin_element when element(El + 2, CurAST) =:= default; is_list(element(El + 2, CurAST)) ->
		  case element(El + 2, CurAST) =/= default andalso not lists:all(fun (E) ->
		    case E of {unit,U} when is_integer(U) -> true; _ ->
		      lists:member(E, [binary, bitstring, bits, bytes, signed, unsigned, integer, float, utf8, utf16, utf32, big, little, native]) end end, element(El + 2, CurAST)) of true -> error(AST); _ -> {CurAST, CurAcc} end;
		_ -> error({El, Struc, AST})
		end end, {AST, Acc}, lists:seq(1, tuple_size(Struc))),
	Fun(NewAST, NewAcc)
.


lookup_ast_args(AST) -> %erlang:display(AST),
	Key = element(1, AST),
	Map = #{
		%0 - literal, 1 - traversal, 2 - list traversal
		...
		atom => [{0}], string => [{0}], nil => [{}],
		cons => [{1,1}], tuple => [{2}],
		record_field => [{1,0,1},{1,1},{1}], %record_index => [{0}],
		record_index => [{0, 1}], record => [{0,2},{1,0,2}],
		map => [{2},{1,2}], map_field_assoc => [{1,1}], map_field_exact => [{1,1}],
		bin => [{2}], bin_element => [{1,default,0},{1,1,0}],
		
		call => [{1,2}], match => [{1,1}], op => [{0,1},{0,1,1}],

		lc => [{1,2}], generate => [{1,1}],
		bc => [{1,2}], b_generate => [{1,1}],
		block => [{2}], 'if' => [{2}], 'case' => [{1,2}], 'catch' => [{1}], 'try' => [{2,2,2,2}],
		'receive' => [{2},{2,1,2}],
		'fun' => [{{clauses,2}},{{function,1,1,1}},{{function,0,0}}],
		named_fun => [{0,2}],
		remote => [{1,1}], clause => [{2,3,2}],
		function => [{0,0,2}],

		typed_record_field => [{1,1}], type => [{any},{0,any},{0,2}],
		remote_type => [{[1,1,2]}], ann_type => [{2}], user_type => [{0,2}],
		attribute => [{record,{0,2}},{opaque,{0,1,2}},{type,{0,1,2}},{spec,{0,2}},{callback,{0,2}},{0,attribute}],
		eof => [{}]},
	case Map of #{Key := StrucPosbl} ->
	StrucL = lists:filter(fun (El) ->
		tuple_size(El) + if Key =:= typed_record_field -> 1; true -> 2 end =:= case AST of [] -> 2; _ -> tuple_size(AST) end end, StrucPosbl),
...
traverse_ast(Fun, Acc, AST) -> 
	...
	lists:foldl(fun (El, {CurAST, CurAcc}) ->
		case element(El, Struc) of
		0 -> {CurAST, CurAcc};
		1 -> {New, NewAcc} = traverse_ast(Fun,CurAcc,element(El + if element(1, CurAST) =:= typed_record_field -> 1; true -> 2 end, CurAST)),
			{setelement(El + if element(1, CurAST) =:= typed_record_field -> 1; true -> 2 end, CurAST, New), NewAcc};
		2 -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
		  case is_list(Elem) of true -> %only type has a list in a list
		  	{NewElem, TAc} = lists:foldl(fun (ElemL, {ListL,AcL}) ->
					{NewElemL, TAcL} = traverse_ast(Fun,AcL,ElemL),
					{ListL ++ [NewElemL], TAcL} end, {[], Ac}, Elem),
					{List ++ [NewElem], TAc};
			_ -> {NewElem, TAc} = traverse_ast(Fun,Ac,Elem), {List ++ [NewElem], TAc} end end,
				{[], CurAcc}, element(El + 2, CurAST)),
			{setelement(El + 2, CurAST, New), NewAcc};
		...
		X when is_list(X) ->
			{New, NewAcc} = lists:foldl(fun (E, {NCurAST, NCurAcc}) ->
				case lists:nth(E, X) of 0 -> {NCurAST, NCurAcc};
				1 -> {New, NewAcc} = traverse_ast(Fun,NCurAcc,lists:nth(E, element(El + 2, CurAST))),
					{NCurAST ++ [New], NewAcc};
				2 -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
					{NewElem, TAc} = traverse_ast(Fun,Ac,Elem),
					{List ++ [NewElem], TAc} end, {[], NCurAcc}, lists:nth(E, element(El + 2, CurAST))),
					{NCurAST ++ [New], NewAcc};
		...
		Y when is_tuple(Y) ->
			{New, NewAcc} = lists:foldl(fun (E, {NCurAST, NCurAcc}) ->
				case element(E, Y) of 0 -> {NCurAST, NCurAcc};
				1 -> {New, NewAcc} = traverse_ast(Fun,NCurAcc,element(E, NCurAST)),
					{setelement(E, NCurAST, New), NewAcc};
				2 -> {New, NewAcc} = lists:foldl(fun (Elem, {List,Ac}) ->
					{NewElem, TAc} = traverse_ast(Fun,Ac,Elem),
...
%	{'fun',L,{function,A,B,C}} -> {NewA, Acc1} = traverse_ast(Fun,Acc,A),
%		{NewB, Acc2} = traverse_ast(Fun,Acc1,B), 
%   {NewC, NewAcc} = traverse_ast(Fun,Acc2,C),
%   {'fun',L,{function,NewA,NewB,NewC}};
...
%https://github.com/erlang/otp/blob/master/erts/emulator/beam/erl_printf_term.c
	...
	is_function(Lit) -> %if have current module name, could compare?
	  %if element(2, erlang:fun_info(Lit, module)) =:= CurrentDecompileModule ->
		%  {'fun', 0, {function, element(2, erlang:fun_info(Lit, name)),
		%    element(2, erlang:fun_info(Lit, arity))}}; true ->
	  {'fun', 0, {function, {atom,0,element(2, erlang:fun_info(Lit, module))},
	    {atom,0,element(2, erlang:fun_info(Lit, name))},
	    {integer,0,element(2, erlang:fun_info(Lit, arity))}}}
	end
.
...
isblockstructresolvable({_, NodesToAST, _, _}, P, Node, PIdom, PSucc) ->
	...
	%{Suffix, SuffixAfter} = if Kind =:= 'try' -> {[4, 1, 3, 1], [4, 1, 5, 1, 5, if IsOTP20 -> 2; true -> 1 end]};
...
issinglepath({_, NodesToAST, _, _}, Kind, HeadNode, HeadNodeIdom, IsOTP20) ->
...
getblockstructchild({_, NodesToAST, _, _}, Succs, Kind, IsFirst, P, IsOTP20) ->
	Path = tuple_to_list(getnextsibling(lists:last(array:get(P-1, NodesToAST)))),
	Suffix = if Kind =:= 'try' -> if IsFirst -> [4, 1, 3, 1]; true -> [4, 1, 5, 1, 5, if IsOTP20 -> 2; true -> 1 end] end;
...
getgraphpathchild({AST, NodesToAST, _, _}, Kind, IsFirst, PathNode, IsOTP20) ->
	Path = list_to_tuple(tuple_to_list(getnextsibling(lists:last(array:get(PathNode-1, NodesToAST)))) ++
		...
		timeout -> [4, 1, 4, 1]; 'try' -> if IsFirst -> [4, 1, 4, 1, 5, 1]; true -> [4, 1, 5, 1, 5, if IsOTP20 -> 2; true -> 1 end] end;
		{'try', true} -> [4, 1] end),
...
setgraphpathchild({AST, NodesToAST, ASTDFS, IdxDFS}, Kind, IsFirst, PathNode, Val, IsOTP20) ->
	Path = list_to_tuple(tuple_to_list(getnextsibling(lists:last(array:get(PathNode-1, NodesToAST)))) ++ case Kind of timeout -> [4, 1];
		'try' -> if IsFirst -> [4, 1, 4, 1, 5, 1]; true -> [4, 1, 5, 1, 5, if IsOTP20 -> 2; true -> 1 end] end;
	{'try', true} -> [4, 1] end),
...
insertgraphnodechild({AST, NodesToAST, ASTDFS, IdxDFS}, Node, Kind, IsFirst, NewValue, NewNode, IsOTP20) ->
	Path = tuple_to_list(getnextsibling(lists:last(array:get(Node-1, NodesToAST)))) ++
		case Kind of 'receive' -> if IsFirst -> [4, 1, 4, 1, 3, 1, 2, 1, 5, 1]; true -> [4, 1, 4, 3, 3, 1, 2, 1, 5, 1] end;
			...
			wait_timeout -> [5, 1];
			'try' -> if IsFirst -> [4, 1, 3, 1]; true -> [4, 1, 5, 1, 5, if IsOTP20 -> 2; true -> 1 end] end;
...
simple_link(V, W, {_Semi, Ancestor, Best, Link}) ->
	{array:set(W-1, V, Ancestor), Best, Link}.
...
%	end
%.
%simple_eval(V, {Semi, Ancestor, Best}) -> {simple_evalhf(Semi, Ancestor, V, array:get(V-1, Ancestor)), Ancestor, Best}.
ds_compress(V, {Semi, Ancestor, Best}) ->
	A = array:get(V-1, Ancestor),
	case array:get(A-1, Ancestor) =:= 0 of
		true -> {Ancestor, Best};
		false ->
			{NewAncestor, NewBest} = ds_compress(A, {Semi, Ancestor, Best}),
			{array:set(V-1, array:get(A-1, NewAncestor), NewAncestor),
					case array:get(array:get(V-1, NewBest)-1, Semi) > array:get(array:get(A-1, NewBest)-1, Semi) of
						true -> array:set(V-1, array:get(A-1, NewBest), NewBest);
						false -> NewBest end}
	end
.
make_ds_link(Child, Size) ->
	fun (V, W, {Semi, Ancestor, Best, _Link}) -> ds_link(V, W, {Semi, Ancestor, Best, Child, Size}) end.
ds_link(V, W, {Semi, Ancestor, Best, Child, Size}) ->
	{CurAncestor, CurChild, CurSize, S} = fun SetSizes(CurA, CurC, CurSz, S) ->
		case array:get(array:get(W-1, Best)-1, Semi) < array:get(array:get(S-1, Best)-1, Semi) of true ->
			case array:get(S-1, CurSz) + array:get(array:get(array:get(S-1, CurC)-1, CurC)-1, CurSz) >= array:get(array:get(S-1, CurC)-1, CurSz) bsl 1 of true ->
				SetSizes(array:set(array:get(S-1, CurC), S, CurA), array:set(S-1, array:get(array:get(S-1, CurC)-1, CurC), CurC), CurSz, S);
			_ -> SetSizes(array:set(S-1, array:get(S-1, CurC), CurA), CurC, array:set(array:get(S-1, CurC), array:get(S-1, CurSz), CurSz), array:get(S-1, CurC)) end;
		_ -> {CurA, CurC, CurSz, S} end end(Ancestor, Child, Size, W),
	%erlang:display({V, W, array:to_list(CurChild), array:to_list(CurSize)}),
	NewBest = array:set(S-1, array:get(W-1, Best), Best),
	NewSize = array:set(V-1, array:get(V-1, CurSize) + array:get(W-1, CurSize), CurSize),
	{NewS, NewChild} = case array:get(V-1, NewSize) < array:get(W-1, NewSize) bsr 1 of true ->
		{array:get(V-1, CurChild), array:set(V-1, S, CurChild)}; _ -> {S, CurChild} end,
	NewAncestor = fun SetAncestors(CurA, CurS) -> if CurS =/= 0 -> SetAncestors(array:set(S-1, V, CurA), array:get(S-1, NewChild)); true -> CurA end end(CurAncestor, NewS),
	{NewAncestor, NewBest, make_ds_link(NewChild, NewSize)}.
ds_eval(V, {Semi, Ancestor, Best}) ->
	case array:get(V-1, Ancestor) =:= 0 of
		true -> {array:get(V-1, Best), Ancestor, Best};
		false -> {NewAncestor, NewBest} = ds_compress(V, {Semi, Ancestor, Best}),
			{array:get(case array:get(array:get(V-1, NewAncestor)-1, Semi) < array:get(array:get(V-1, NewBest)-1, Semi) of true -> array:get(V-1, NewAncestor); _ -> V end-1, NewBest), NewAncestor, NewBest}
	end
.
simple_eval(V, {Semi, Ancestor, Best}) ->
	case array:get(V-1, Ancestor) =:= 0 of
		true -> {V, Ancestor, Best};
		false -> {NewAncestor, NewBest} = ds_compress(V, {Semi, Ancestor, Best}),
			{array:get(V-1, NewBest), NewAncestor, NewBest}
	end
.

do_tarjan_immdom({Pred, Succ, _, _, _, _, _, _}, RootIdx, UseDS) -> %Succ, Pred
	...
	{Semi, Idom, _, _, _} = tarjan_immdom1({DFSTup, IdxDFS}, Pred, {DFS, ZeroTup, ZeroTup, array:from_list(lists:seq(1, DFSSize)), array:new(DFSSize, {default,[]}),
		if UseDS -> fun ds_eval/2; true -> fun simple_eval/2 end, if UseDS -> make_ds_link(ZeroTup, array:from_list([0|lists:duplicate(DFSSize, 1)])); true -> fun simple_link/3 end},
			DFSSize),
	element(2, tarjan_immdom4(IdxDFS, Pred, {Semi, Idom}, RootIdx, 2))
.
...
check_tarjan() ->
	...
	[array:to_list(do_tarjan_immdom({succ_to_pred(?LTPaperSucc), ?LTPaperSucc, {}, {}, {}, {}, {}, {}}, 1, true)) =:= Idom,
	 array:to_list(do_tarjan_immdom(rev_interval({succ_to_pred(?LTPaperSucc), ?LTPaperSucc, {}, {}, {}, {}, {}, {}}), 1, true))
...
tarjan_immdom4(IdxDFS, Pred, {Semi, Idom}, RootIdx, W) ->
	%io:format("~p~n", [{DFS, Pred, {Semi, Idom, Ancestor, Best, Bucket}, W}]),
	case
		tuple_size(IdxDFS) + 1 of W -> {Semi, array:set(RootIdx-1, 0, Idom)};
...
tarjan_immdom3({Semi, Idom, Ancestor, Best, Bucket, Eval, Link}, P, []) ->
	{Semi, Idom, Ancestor, Best, array:set(P-1, [], Bucket), Eval, Link};
tarjan_immdom3({Semi, Idom, Ancestor, Best, Bucket, Eval, Link}, P, [V|Vs]) ->
	{U, NewAncestor, NewBest} = Eval(V, {Semi, Ancestor, Best}),
	tarjan_immdom3({Semi, array:set(V-1,
		case array:get(U-1, Semi) < array:get(V-1, Semi) of
			true -> U;
			false -> P
		end, Idom), NewAncestor, NewBest, Bucket, Eval, Link}, P, Vs)
.

tarjan_immdom2(IdxDFS, {Semi, Ancestor, Best, Bucket, _Eval, Link}, W, P, {0, nil}) -> %gb_sets:new() =:= {0, nil}
	Idx = element(array:get(W-1, Semi), IdxDFS),
	NewBucket = array:set(Idx-1, [W|array:get(Idx-1, Bucket)], Bucket),
	{NewA, NewB, NewLink} = Link(P, W, {Semi, Ancestor, Best, Link}), {Semi, NewA, NewB, NewLink, NewBucket};
tarjan_immdom2(IdxDFS, {Semi, Ancestor, Best, Bucket, Eval, Link}, W, P, VVs) ->
	...
	NewSemi = case array:get(W-1, Semi) > USemi of
		...
		false -> Semi end,
	tarjan_immdom2(IdxDFS, {NewSemi, NewAncestor, NewBest, Bucket, Eval, Link}, W, P, gb_sets:del_element(V, VVs))
.

tarjan_immdom1({DFS, IdxDFS}, Pred, {Semi, Idom, Ancestor, Best, Bucket, Eval, Link}, W) ->
	%io:format("~p~n", [{DFS, Pred, {Semi, Idom, Ancestor, Best, Bucket}, W}]),
	if
		W =:= 1 -> {Semi, Idom, Ancestor, Best, Bucket};
		true -> DepthW = element(W, IdxDFS), P = do_get_dfs_parent({DFS, IdxDFS}, Pred, DepthW),
			{NewSemi, NewAncestor, NewBest, NewLink, NewBucket} =
				tarjan_immdom2(IdxDFS, {Semi, Ancestor, Best, Bucket, Eval, Link}, DepthW, P, array:get(DepthW-1, Pred)),
			tarjan_immdom1({DFS, IdxDFS}, Pred,
				tarjan_immdom3({NewSemi, Idom, NewAncestor, NewBest, NewBucket, Eval, NewLink}, P, array:get(P-1, NewBucket)), W - 1)
	end
.
...
check_idf() ->
	...
	[array:to_list(do_tarjan_immdom({succ_to_pred(PaperSucc), PaperSucc, {}, {}, {}, {}, {}, {}}, 17, true)) =:= array:to_list(PaperDom),
	...
	array:to_list(do_tarjan_immdom({succ_to_pred(LadderSucc), LadderSucc, {}, {}, {}, {}, {}, {}}, 8, true)) =:= array:to_list(LaddDom),
...
paper_rev_graph() -> Succ = to_gb_sets({[4], [3], [], [5, 6], [7], [9, 11], [8, 9], [3], [10], [3], [2]}),
	...
	io:format("~p~n", [{Idom, get_post_dom_divide({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}), check_post_doms({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}})}]).
...
%brute_force_graph(Size) -> true.

%by construction, return and exit are never reachable from a multi-out edge/branching node, only as a single path way
%by construction, no self-loops are allowed
random_acyclic_graph(Size) -> GI = lists:foldl(fun (El, {AccReach, AccGraph}) -> if El =:= 2 -> {AccReach#{3 => true}, [[3]|AccGraph]}; El =:= 3 -> {AccReach, [[]|AccGraph]};
	true -> NextEl = if El =:= 1 -> 4; El =:= Size -> 2; true -> El + 1 end, case AccReach of #{NextEl := true} -> Num = rand:uniform(3), Acc = {AccReach, [[]|AccGraph]}; _ -> Num = rand:uniform(3-1), Acc = {AccReach#{NextEl => true}, [[NextEl]|AccGraph]} end,
	lists:foldl(fun (_E, {AR, AG}) -> NewEl = if El =:= 1 -> El + rand:uniform(Size - 1); true -> case El + rand:uniform(Size - El + if El =:= 1 -> 0; true -> 2 end) of X when X > Size -> X - Size + 1; Y -> Y end end, NotCanAdd = (NewEl =:= 2 orelse NewEl =:= 3) andalso hd(AG) =/= [] orelse hd(AG) =:= [2] orelse hd(AG) =:= [3], {if NotCanAdd -> AR; true -> AR#{NewEl => true} end, if NotCanAdd -> AG; true -> [lists:usort([NewEl|hd(AG)])|tl(AG)] end} end, Acc, lists:seq(1, Num)) end end, {#{}, []}, lists:seq(1, Size)),
	Succs = lists:reverse(element(2, GI)),
	to_gb_sets(list_to_tuple(case element(1, GI) of #{2 := true} -> Succs; _ -> [hd(Succs),[],[2],tl(tl(tl(Succs)))] end)).
...
%number of distinct labeled simple directed graphs is (2^(n choose 2))^2=4^(n choose 2) graphs but this does not account for isomorphism nor connectedness
%or e edges of the n^2 possible edges so (n^2 choose e)=(n^2)!/((E!)*(N^2-E)!)
%4^(n choose 2)=2^(n(n-1)) digraphs without loops, 2^(n^2) with loops, asymetric where edges (a, b) can exist only if (b, a) does not exist: 3^(n choose 2)=3^(n(n-1)/2)
random_graph(Size) -> GI = lists:foldl(fun (El, {AccReach, AccGraph}) -> if El =:= 2 -> {AccReach#{3 => true}, [[3]|AccGraph]}; El =:= 3 -> {AccReach, [[]|AccGraph]};
	true -> NextEl = if El =:= 1 -> 4; El =:= Size -> 2; true -> El + 1 end, case AccReach of #{NextEl := true} -> Num = rand:uniform(3), Acc = {AccReach, [[]|AccGraph]}; _ -> Num = rand:uniform(3-1), Acc = {AccReach#{NextEl => true}, [[NextEl]|AccGraph]} end,
	lists:foldl(fun (_E, {AR, AG}) -> NewEl = if El =:= 1 -> El + rand:uniform(Size - 1); true -> case El + rand:uniform(Size - 1) of X when X > Size -> X - Size; Y -> Y end end, NotCanAdd = (NewEl =:= 2 orelse NewEl =:= 3) andalso hd(AG) =/= [] orelse hd(AG) =:= [2] orelse hd(AG) =:= [3], {if NotCanAdd -> AR; true -> AR#{NewEl => true} end, if NotCanAdd -> AG; true -> [lists:usort([NewEl|hd(AG)])|tl(AG)] end} end, Acc, lists:seq(1, Num)) end end, {#{}, []}, lists:seq(1, Size)),
	Succs = lists:reverse(element(2, GI)),
	to_gb_sets(list_to_tuple(case element(1, GI) of #{2 := true} -> Succs; _ -> [hd(Succs),[],[2],tl(tl(tl(Succs)))] end)).

-define(BaseNoRetCase, to_gb_sets({[3], [], [2]})).
-define(MultiFencePossible, to_gb_sets({[4, 5], [3], [], [2, 6], [2, 6], [3]})).
-define(MultiFencePossible2, to_gb_sets({[4, 5], [3], [], [2, 6], [2, 7], [3], [3]})).
-define(MultiFenceException, to_gb_sets({[4,5,8],[3],[],[3],[4,6],[7,9],[11],[5,6,9],[4,10],[3],[5,9,12],[2]})).
-define(MultiExitNodeCase, to_gb_sets({[4,12], [3], [], [5,12], [6,12], [7,8], [8, 11, 12], [9, 11], [10], [2], [2], [3]})).
-define(SingleRetExitCase, to_gb_sets({[4,6,9], [3], [], [5,9], [2], [7,12], [8], [10, 12], [3], [11], [3], [2]})).
-define(NewCase, to_gb_sets({[4,5,11], [3], [], [9, 12], [6,11], [7,12], [8, 10], [2], [11, 12], [12], [3], [2]})).
test_rev_graph(Loops) ->
  %case check_post_doms({succ_to_pred(?MultiFenceException), ?MultiFenceException, {}, {}, {}, {}, {}, {}}) of true -> true; _ -> error(get_edgeset(?MultiFenceException)) end,
  case check_post_doms({succ_to_pred(?MultiFencePossible), ?MultiFencePossible, {}, {}, {}, {}, {}, {}}) of true -> true; _ -> error(get_edgeset(?MultiFencePossible)) end,
	[case check_post_doms({succ_to_pred(?BaseNoRetCase), ?BaseNoRetCase, {}, {}, {}, {}, {}, {}}) of true -> true; _ -> get_edgeset(?BaseNoRetCase) end|
	lists:map(fun (_El) -> Succ = if Loops -> random_graph(12); true -> random_acyclic_graph(12) end, erlang:display(get_edgeset(Succ)),
		%Succ = ?MultiExitNodeCase,
		%Succ = ?SingleRetExitCase,
		%Succ = ?NewCase,
		case check_post_doms({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}) of true -> true; _ -> error({get_edgeset(Succ), element(1, get_post_dom({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}})), get_post_dom_divide({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}), io:format("~s ~s", [edgeset_to_dot(Succ, "v"), edgeset_to_dot(element(2, element(1, get_revgraph({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}))), "w")])}) end
		end, lists:seq(1, 20000))].
		
make_dot_graph() ->
	Succ = ?NewCase,
	Idom = get_post_dom_divide({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}),
	io:format("~s", [edgeset_to_dot(Succ, "v") ++ edgeset_to_dot(make_dom_tree(Idom), "i") ++ edgeset_to_dot(make_dom_tree(do_tarjan_immdom(rev_graph({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}), 3, true)), "j")]),
	%[4,5,11], [3], [], [9, 12], [6,11], [7,12], [8, 10], [2], [11, 12], [12], [3], [2]
	Check = to_gb_sets({[4,5,11], [], [5,9,11], [9, 12], [6,11], [7,12], [8, 10], [2], [11, 12], [12], [3], [2]}),
	erlang:display(get_edgeset(element(1, rev_graph({succ_to_pred(Check), Check, {}, {}, {}, {}, {}, {}})))),
	do_tarjan_immdom(rev_graph({succ_to_pred(Check), Check, {}, {}, {}, {}, {}, {}}), 2, true).


test_inc_algo(Loops) ->
	lists:map(fun (_El) -> Succ = if Loops -> random_graph(12); true -> random_acyclic_graph(20) end,
		Edges = array:foldl(fun (Idx, Val, Acc) ->
			gb_sets:fold(fun (S, A) -> Vs = gb_sets:size(Val),
				if Idx + 1 =:= 2 andalso S =:= 3 orelse S =:= 2 andalso Vs =:= 1 -> A; true ->
				%dependency scheme - left number must come only after it has been a right number
				%then its right number must come before any other right number which is greater
				%back edges must come last to avoid situation where infinite loops disconnect the return node, while edges still remain to process
				Chk = lists:takewhile(fun ({P, E}) -> Idx+1 =/= P andalso Idx+1 =/= E end, A),
				St = lists:takewhile(fun ({P, E}) -> not (E =/= 2 andalso E =/= 3 andalso E < P) andalso (S =/= 2 andalso S =/= 3 andalso S < Idx+1 andalso (E =:= 2 orelse E =:= 3 orelse P < E) orelse Idx+1 =:= E andalso E =/= 2 andalso E =/= 3 orelse S =:= 2 orelse S =:= 3 orelse (E =/= 2 andalso E =/= 3) andalso S >= E) end, lists:nthtail(length(Chk), A)), %returns come last followed by exits, if neither are there then it would be an infinite loop which causes the one time unreturnable transformation
				Chk ++ St ++ [{Idx+1, S}|lists:nthtail(length(Chk) + length(St), A)] end end, Acc, Val) end, [], Succ),
		erlang:display({get_edgeset(Succ), Edges}),
		{Graph, _AST} = lists:foldl(fun ({P, S}, {AccGraph, AccAST}) -> case S =:= 2 andalso gb_sets:is_element(P, array:get(2-1, element(1, AccGraph))) orelse
			S =:= 3 andalso gb_sets:is_element(P, array:get(3-1, element(1, AccGraph))) of true -> {AccGraph, AccAST}; %infinite loop previously maintained a return connection
			_ -> {add_edge(P, S, AccGraph, AccAST, false), AccAST} end end,
			{{array:from_list([gb_sets:new(), gb_sets:from_list([1]), gb_sets:from_list([2])], gb_sets:new()),
			 ...
			 #{1 => {2, gb_sets:new(), gb_sets:new(), 3},
			 	...
			 	array:from_list([gb_sets:new(), gb_sets:new(), gb_sets:new()], gb_sets:new())},
			 	{[], array:from_list(lists:duplicate(array:size(Succ), [{}]), []),
			 		array:from_list([3, 1, 2] ++ lists:seq(4, array:size(Succ))), array:from_list([2, 3, 1] ++ lists:seq(array:size(Succ), 4, -1), 0)}}, Edges),
		case array:to_list(element(1, dom_to_d(get_pdom(Graph)))) =:= array:to_list(element(1, get_post_dom({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}}))) of true -> true; _ -> {get_edgeset(Succ), Edges, array:to_list(element(1, dom_to_d(get_pdom(Graph)))), array:to_list(element(1, get_post_dom({succ_to_pred(Succ), Succ, {}, {}, {}, {}, {}, {}})))} end
		end, lists:seq(1, 100)).
	...
	lists:map(fun (_El) -> Succ = if Loops -> random_graph(12); true -> random_acyclic_graph(20) end,
		Edges = array:foldl(fun (Idx, Val, Acc) ->
			gb_sets:fold(fun (S, A) -> Vs = gb_sets:size(Val),
				...
				%then its right number must come before any other right number which is greater
				Chk = lists:takewhile(fun ({P, E}) -> Idx+1 =/= P andalso Idx+1 =/= E end, A),
				St = lists:takewhile(fun ({P, E}) -> not (E =/= 2 andalso E =/= 3 andalso E < P) andalso (S =/= 2 andalso S =/= 3 andalso S < Idx+1 andalso (E =:= 2 orelse E =:= 3 orelse P < E) orelse Idx+1 =:= E andalso E =/= 2 andalso E =/= 3 orelse S =:= 2 orelse S =:= 3 orelse (E =/= 2 andalso E =/= 3) andalso S >= E) end, lists:nthtail(length(Chk), A)),
				Chk ++ St ++ [{Idx+1, S}|lists:nthtail(length(Chk) + length(St), A)] end end, Acc, Val) end, [], Succ),
		{Graph, _AST} = lists:foldl(fun ({P, S}, {AccGraph, AccAST}) -> case S =:= 2 andalso gb_sets:is_element(P, array:get(2-1, element(1, AccGraph))) orelse
			S =:= 3 andalso gb_sets:is_element(P, array:get(3-1, element(1, AccGraph))) of true -> {AccGraph, AccAST}; %infinite loop previously maintained a return connection
			_ -> {add_edge(P, S, AccGraph, AccAST, false), AccAST} end end,
			{{array:from_list([gb_sets:new(), gb_sets:from_list([1]), gb_sets:from_list([2])], gb_sets:new()),
			 ...
			 #{1 => {2, gb_sets:new(), gb_sets:new(), 3},
			 	...
			 	{[], array:from_list(lists:duplicate(array:size(Succ), [{}]), []),
			 		array:from_list([3, 1, 2] ++ lists:seq(4, array:size(Succ))), array:from_list([2, 3, 1] ++ lists:seq(array:size(Succ), 4, -1), 0)}}, Edges),
		case all_nca(get_pdom(Graph)) of true -> true; _ -> {get_edgeset(Succ),all_nca(get_pdom(Graph))} end
		end, lists:seq(1, 100)).
	
check_sgl_loops() ->
	PaperDJExamp = to_gb_sets({[2, 10], [3, 4], [5], [5, 6], [7, 8], [7], [9], [5, 9], [2, 4, 10], []}), %[[5,8],[5,7,9,4,6,8],[2,3,4,5,6,7,8,9]]
	DJExampDom = do_tarjan_immdom({succ_to_pred(PaperDJExamp), PaperDJExamp, {}, {}, {}, {}, {}, {}}, 1, true),
	Reducible = to_gb_sets({[2, 6], [3], [4], [2, 5], [3], [5]}), %[[2,3,4,5]] %paper error: this is not reducible unless the END arrow is reversed
	ReducDom = do_tarjan_immdom({succ_to_pred(Reducible), Reducible, {}, {}, {}, {}, {}, {}}, 1, true),
	ReducibleC = to_gb_sets({[2, 6], [3], [4], [2, 5], [3, 6], []}), %[[3,4,5],[2,3,4,5]] %fixed paper erro
	ReducCDom = do_tarjan_immdom({succ_to_pred(ReducibleC), ReducibleC, {}, {}, {}, {}, {}, {}}, 1, true),
	BasicIrred = to_gb_sets({[2, 3], [3], [2]}), %[[2, 3]]
	IrredDom = do_tarjan_immdom({succ_to_pred(BasicIrred), BasicIrred, {}, {}, {}, {}, {}, {}}, 1, true),
	PaperSucc = to_gb_sets({[2, 3], [3, 4], [2], [5, 6], [6], [3, 5]}), %[[5,6],[2,3,4,5,6]]
	PaperDom = do_tarjan_immdom({succ_to_pred(PaperSucc), PaperSucc, {}, {}, {}, {}, {}, {}}, 1, true),
	[get_sgl_loops({succ_to_pred(PaperDJExamp), PaperDJExamp, {}, {}, {}, {}, {}, {}}, d_to_dom(DJExampDom, get_j_succ(PaperDJExamp, DJExampDom), make_dom_tree(DJExampDom), dom_depth(DJExampDom))),
	get_sgl_loops({succ_to_pred(Reducible), Reducible, {}, {}, {}, {}, {}, {}}, d_to_dom(ReducDom, get_j_succ(Reducible, ReducDom), make_dom_tree(ReducDom), dom_depth(ReducDom))),
	get_sgl_loops({succ_to_pred(ReducibleC), ReducibleC, {}, {}, {}, {}, {}, {}}, d_to_dom(ReducCDom, get_j_succ(ReducibleC, ReducCDom), make_dom_tree(ReducCDom), dom_depth(ReducCDom))),
	get_sgl_loops({succ_to_pred(BasicIrred), BasicIrred, {}, {}, {}, {}, {}, {}}, d_to_dom(IrredDom, get_j_succ(BasicIrred, IrredDom), make_dom_tree(IrredDom), dom_depth(IrredDom))),
	get_sgl_loops({succ_to_pred(PaperSucc), PaperSucc, {}, {}, {}, {}, {}, {}}, d_to_dom(PaperDom, get_j_succ(PaperSucc, PaperDom), make_dom_tree(PaperDom), dom_depth(PaperDom)))]
.

partial_tarjan_scc(Graph, NodeSet) ->
	{_, _, _, SCCs} = gb_sets:fold(fun (V, {AccIdx, AccV, AccS, AccSCCs}) ->
		case element(1, array:get(V-1, AccV)) =:= -1 of true ->
			fun StrongConnect(CurV, CurIdx, CurVs, CurS, CurSCCs) ->
				VsNew = array:set(CurV-1, {CurIdx, CurIdx, true}, CurVs),
				{IdxUp, UpVs, UpS, UpSCCs} = gb_sets:fold(fun (W, {AIdx, AV, AS, ASCCs}) -> case gb_sets:is_element(W, NodeSet) of false -> {AIdx, AV, AS, ASCCs}; _ ->
					case element(1, array:get(W-1, AV)) =:= -1 of true -> Res = StrongConnect(W, AIdx, AV, AS, ASCCs), VNow = array:get(CurV-1, element(2, Res)),
						setelement(2, Res, array:set(CurV-1, setelement(2, VNow, min(element(2, VNow), element(2, array:get(W-1, element(2, Res))))), element(2, Res)));
					_ -> case element(3, array:get(W-1, AV)) of true -> VNow = array:get(CurV-1, AV), {AIdx, array:set(CurV-1, setelement(2, VNow, min(element(2, VNow), element(1, array:get(W-1, AV)))), AV), AS, ASCCs};
						_ -> {AIdx, AV, AS, ASCCs} end end end
					end, {CurIdx+1, VsNew, [CurV|CurS], CurSCCs}, get_succs(CurV, Graph)),
				VUp = array:get(CurV-1, UpVs),
				if element(1, VUp) =:= element(2, VUp) ->
				  {FinalVs, FinalS, NewSCC} = fun TakeSCC(Ve, St, CurSCC) -> W = hd(St), Vts = array:set(W-1, setelement(3, array:get(W-1, Ve), false), Ve),
				  	if W =/= CurV -> TakeSCC(Vts, tl(St), [W|CurSCC]); true -> {Vts, tl(St), [W|CurSCC]} end end(UpVs, UpS, []), {IdxUp, FinalVs, FinalS, [NewSCC|UpSCCs]};
				true -> {IdxUp, UpVs, UpS, UpSCCs} end
				end(V, AccIdx, AccV, AccS, AccSCCs);
		_ -> {AccIdx, AccV, AccS, AccSCCs} end
		end, {0, array:new(array:size(element(1, Graph)), {default, {-1, -1, false}}), [], []}, NodeSet),
	SCCs
.

get_sgl_loops(Graph, Dom) -> %must have DJ/dominator-join information obviously
	{DFS, RDFS} = do_interval_dfs(Graph, 1),
	MaxLevel = maps:fold(fun (_, {_, _, _, El}, Acc) -> max(El, Acc) end, 0, Dom),
	lists:foldl(fun (I, Acc) ->
		{Irreducible, Loops} = maps:fold(fun (N, {_, _, _, Lvl}, {AIrr, ALoops}) -> if Lvl =:= I ->
			{CJEdges, BJEdges} = gb_sets:fold(fun (M, {AcCJ, AcBJ}) ->
				#{M := {_, J, _, _}} = Dom,
				case gb_sets:is_element(N, J) of false -> {AcCJ, AcBJ}; _ ->
				case nca(Dom, M, N) of N -> {AcCJ, [M|AcBJ]}; %BJ edge, m -> n where n dom n
				_ -> {[M|AcCJ], AcBJ} end %CJ edge
				end end, {[], []}, get_preds(N, Graph)),
			%ReachUnder(n) for loop headed at n to all BJEdges effectively means scan the subtree of n since by definition this header dominates all entries in the loop, could go up the tree and then to the side and down to try to optimize the search
			RedLoop = gb_sets:to_list(element(2, pred_set_recurse(gb_sets:from_list(BJEdges), gb_sets:from_list([N]), Graph))),
				%gb_sets:union(lists:map(fun (E) -> fun DomReach(B, L) -> #{B := {Parent, _, _, _}} = Dom, if Parent =:= N -> gb_sets:add_element(B, L); true -> DomReach(Parent, gb_sets:add_element(B, L)) end end(E, gb_sets:new())
					%end, BJEdges))),
			{AIrr orelse lists:any(fun (M) ->
				array:get(M-1, DFS) > array:get(N-1, DFS) andalso array:get(M-1, RDFS) < array:get(N-1, RDFS) %DFS/sp-back edge
				end, CJEdges), if RedLoop =:= [] -> ALoops; true -> [[N|RedLoop]|ALoops] end}
				; true -> {AIrr, ALoops} end end, {false, []}, Dom),
		Acc ++ if Irreducible -> Loops ++ lists:filtermap(fun (El) -> if length(El) =/= 1 -> {true, lists:sort(fun (A, B) -> #{A := {_, _, _, ALvl}} = Dom, #{B := {_, _, _, BLvl}} = Dom, ALvl < BLvl orelse ALvl =:= BLvl andalso A < B end, El)}; true -> false end end, %need to sort it by level as the lowest level are the collection of loop headers
			partial_tarjan_scc(Graph, maps:fold(fun (N, {_, _, _, Lvl}, A) -> if Lvl >= I -> gb_sets:add_element(N, A); true -> A end end, gb_sets:new(), Dom))); true -> Loops end
		end, [], lists:seq(MaxLevel, 1, -1))
.

all_nca(Dom) ->
	[[nca(Dom, X, Y) || Y <- lists:seq(1, maps:size(Dom))] || X <- lists:seq(1, maps:size(Dom))].
...
check_sgl() ->
	...
	[array:to_list(do_tarjan_immdom({succ_to_pred(PaperSucc), PaperSucc, {}, {}, {}, {}, {}, {}}, 10, true)) =:= array:to_list(PaperDom),
	cmp_edgeset(get_j_succ(PaperSucc, PaperDom), PaperJEdges),
...
sreedhar_gao_lee_add(Graph, Dom, Xd, Yd) ->
	%BeforeNca = all_nca(Dom), %incremental NCA testing
	...
	%AfterNca = all_nca(NewDom), %incremental NCA testing
	%DomMap = gb_sets:fold(fun (Wd, AccDom) ->
	%	fun GetChildDoms(Node, A) -> #{Node := {_,_,NodeTree,_}} = Dom, gb_sets:fold(fun (El, Acc) -> #{El := ElDom} = Dom, GetChildDoms(El, Acc#{El => Wd}) end, A, NodeTree) end(Wd, AccDom) end,
	%	#{}, DomAffected),
	%BuildNca = lists:foldl(fun (El, Acc) ->
	%		[lists:foldl(fun (E, A) -> En = case DomMap of #{E := X} -> X; _ -> E end, Eln = case DomMap of #{El := Y} -> Y; _ -> El end,
	%			[case En =/= Eln andalso (gb_sets:is_element(Eln, DomAffected) andalso gb_sets:is_element(En, DomAffected) orelse
	%				gb_sets:is_element(Eln, DomAffected) andalso lists:member(lists:nth(En, lists:nth(Eln, BeforeNca)), lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(Dom, Eln)))) orelse
	%				gb_sets:is_element(En, DomAffected) andalso lists:member(lists:nth(En, lists:nth(Eln, BeforeNca)), lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(Dom, En))))) of true -> case lists:nth(En, lists:nth(Eln, BeforeNca)) of Zd -> error("Non-Minimal Update"); _ -> Zd end;
	%			_ -> lists:nth(E, lists:nth(El, BeforeNca)) end|A] end, [], lists:seq(maps:size(Dom), 1, -1))|Acc]
	%	end, [], lists:seq(maps:size(Dom), 1, -1)),
	%case AfterNca =/= BuildNca of true -> error({"Invalid Incremental NCA", Xd, Yd, DomAffected, Zd, BeforeNca, AfterNca, BuildNca}); _ -> true end,
	NewDom
	%lists:foldl(fun (Wd, Acc) -> true end, {Dom, J}, DomAffected)
.

sreedhar_gao_lee_remove_affected(Graph, PossiblyAffected, PseudoDom, DomSt, DomDy, Xd, Yd, DChange, Monitor) ->
	{NewDomDy, Changes} = gb_sets:fold(fun (Wd, {AccDomDy, AccChange}) ->
			#{Wd := WdDomDy} = AccDomDy,
			PreDomTemp = gb_sets:fold(fun (Pf, AcDomTemp) ->
				...
				D = case PseudoDom of #{Pf := P} -> P; _ -> Pf end,
				#{D := PDy} = AccDomDy,
				#{Pf := PSt} = DomSt,
				if PDy =:= [] -> AcDomTemp; true -> [{PSt, PDy}|AcDomTemp] end end,
				 [], get_preds(Wd, Graph)),
			...
			case NextDomTemp =/= [] andalso (WdDomDy =:= [] orelse
			 sets:size(NextDomTemp) =/= sets:size(WdDomDy) orelse
			 not sets:is_subset(NextDomTemp, WdDomDy)) of true -> {AccDomDy#{Wd => NextDomTemp}, case Monitor of #{Wd := WM} -> gb_sets:union(AccChange, gb_sets:filter(fun (Z) -> #{Z := ZD} = AccDomDy, ZD =:= [] orelse sets:size(sets:subtract(sets:del_element(Z, ZD), NextDomTemp)) =/= 0 end, WM)); _ -> AccChange end};
			_ -> {AccDomDy, AccChange} end end,
		{DomDy, gb_sets:new()}, DChange),
		%io:format("~p~n", [NewDomDy]),
	if map_size(Changes) =/= 0 -> sreedhar_gao_lee_remove_affected(Graph, PossiblyAffected, PseudoDom, DomSt, NewDomDy, Xd, Yd, Changes, Monitor); true -> NewDomDy end
.
...
tarjan_sgl_alt(Graph, ZdSubtreeIdx, ZdSubtree, Zd, Dom, DomAffected) ->
	...
	Doms = do_tarjan_immdom(setelement(2, setelement(1, Graph, NewPred), NewSucc), ZdIdx, true),
...
sreedhar_gao_lee_remove(Graph, Dom, Xd, Yd) ->
	...
	#{Yd := {Zd,_,_,_}} = Dom, %do_tarjan_immdom(Graph, Zd, true)...

	%Time = os:timestamp(),
	case gb_sets:size(DomAffected) >= 1 of true -> %SGL is much faster than Tarjan
	{PreDomSt, PreDomDy, PseudoDom} = fun DomRec(CurNode, CurDomPath, PseudoDom, CurDomSt, CurDomDy, CurPseudoDom) ->
	...
	{DomSt, DomDy, Monitor} = gb_sets:fold(fun(W, {DS, DD, M}) ->
		gb_sets:fold(fun (P, {IDS, IDD, IM}) -> Q = case PseudoDom of #{P := PD} -> PD; _ -> P end,
			{case IDS of #{P := PS} -> IDS; _ -> #{Zd := {Zp,_,_,_}} = Dom, IDS#{P => gb_sets:from_list(pathtodom(Dom, P, Zp))} end,
			 case IDD of #{P := PS} -> IDD; _ -> IDD#{P => []} end,
			case Q =/= W andalso gb_sets:is_element(Q, DomAffected) of true -> IM#{Q => case IM of #{Q := QM} -> gb_sets:add_element(W, QM); _ -> gb_sets:from_list([W]) end}; _ -> IM end}
		end, {DS, DD, M}, get_preds(W, Graph)) end, {PreDomSt, PreDomDy, #{}}, DomAffected),	
	NewDomDy = sreedhar_gao_lee_remove_affected(Graph, DomAffected, 
		PseudoDom, DomSt, DomDy, Xd, Yd, DomAffected, Monitor),
	...
	NewDom = gb_sets:fold(fun (El, AccDom) ->
		...
		ImmDom = sets:fold(fun (E, A) -> RE = case PseudoDom of #{E := P} -> P; _ -> E end, #{RE := ElDomDy} = NewDomDy, #{E := ESt} = DomSt, case A =:= 0 andalso sets:size(AllDoms) =:= sets:size(sets:union(ElDomDy, ESt)) of true -> E; _ -> A end end, 0, AllDoms),
	...
	ZdSubtree = fun DomRec(CurNode, Set) -> #{CurNode := {_,_,CurNodeTree,_}} = Dom,
		...
		NewDom = tarjan_sgl_alt(Graph, ZdSubtreeIdx, ZdSubtree, Zd, Dom, DomAffected)%, NewDomDy = gb_sets:new()
	end,
	...
	%AfterNca = all_nca(NewJ), %incremental NCA testing
	%DomAff = gb_sets:filter(fun (El) ->
	%	#{El := ElDy} = NewDomDy,
	%	AllDoms = sets:del_element(El, ElDy),
	...
	%	ImmDom =/= 0 end, DomAffected),
	%BuildNca = lists:foldl(fun (El, Acc) ->
	%		[lists:foldl(fun (E, A) -> En = case PseudoDom of #{E := X} -> X; _ -> E end, Eln = case PseudoDom of #{El := Y} -> Y; _ -> El end,
	%			[case En =/= Eln andalso (gb_sets:is_element(Eln, DomAff) andalso gb_sets:is_element(En, DomAff) andalso sets:size(sets:intersection(sets:from_list(lists:takewhile(fun (C) -> C =/= Zd end, pathtodom(NewJ, En))), sets:from_list(lists:takewhile(fun (C) -> C =/= Zd end, pathtodom(NewJ, Eln))))) =/= 0 orelse
	%				gb_sets:is_element(Eln, DomAff) andalso lists:nth(En, lists:nth(Eln, BeforeNca)) =:= Zd andalso Zd =/= En andalso sets:size(sets:intersection(begin #{En := SNCA1} = DomSt, SNCA1 end, sets:from_list(lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, Eln)))))) =/= 0 orelse
	%				gb_sets:is_element(En, DomAff) andalso lists:nth(En, lists:nth(Eln, BeforeNca)) =:= Zd andalso Zd =/= Eln andalso sets:size(sets:intersection(begin #{Eln := SNCA2} = DomSt, SNCA2 end, sets:from_list(lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, En)))))) =/= 0
	%				) of true -> case lists:nth(En, lists:nth(Eln, BeforeNca)) =:= nca(NewJ, En, Eln) of true -> error({"Non-Minimal Update", Xd, Yd, E, El, En, Eln, DomAff, PseudoDom, DomSt, Zd, BeforeNca, AfterNca, lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, Eln))), lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, En)))}), nca(NewJ, En, Eln); _ -> nca(NewJ, En, Eln) end;
	%			_ -> case lists:nth(En, lists:nth(Eln, BeforeNca)) =/= nca(NewJ, En, Eln) of true -> error({"Missed Update", Xd, Yd, E, El, En, Eln, DomAff, DomSt, PseudoDom, Zd, BeforeNca, AfterNca, lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, Eln))), lists:takewhile(fun (C) -> C =/= Zd end, tl(pathtodom(NewJ, En)))}); _ -> lists:nth(E, lists:nth(El, BeforeNca)) end end|A] end, [], lists:seq(maps:size(Dom), 1, -1))|Acc]
	%	end, [], lists:seq(maps:size(Dom), 1, -1)),
	%case AfterNca =/= BuildNca of true -> error({"Invalid Incremental NCA", Xd, Yd, DomAff, Zd, BeforeNca, AfterNca, BuildNca}); _ -> true end,
	NewJ
.
...
pathtodom(Tree, Node, Stop) ->
	#{Node := ElDom} = Tree,
	El = element(1, ElDom),
	if El =:= 0 orelse El =:= Stop -> [Node];
...
copy_graph_node(FromPath, ToPath,
	...
	% element(5, NewGraph), element(7, RevGraph), element(8, NewGraph)}, 3, true) =/=
	...
	%			3, true), element(6, RevGraph)}); _ -> true end,
...
%can only add node as successor, not as predecessor so can latch to return node
	...
	%io:format("~p~n", [{P, S, get_edgeset(Succ), get_edgeset(RSucc), BatchMode}]),
	%P->S->2 and 2->S->P
	...
	_ ->
		...
		case not gb_sets:is_element(P, RetSet) andalso SInRetSet of true -> %NewReach =/= []
			if P =:= 3 -> %if totally disconnected, make return node root connected to exit node, and undo all fences restoring back to exit node
				...
				UpPdom = do_tarjan_immdom({RPFinal, RSFinal, AddPred, AddSucc, PDomExit, NextDom, PDomReach, CrossEdges}, 2, true), %root node incremental change is basically a moot point when the whole tree has changed
	...
	%case do_tarjan_immdom(FinGraph, 1, true) =/= element(1, element(5, FinGraph)) of true -> error({P, S, do_tarjan_immdom(FinGraph, 1, true), element(5, FinGraph)}); _ -> true end,
	%if BatchMode =:= {true, false} andalso not (P > array:size(RSucc) orelse element(P, RSucc) =:= {0,nil}) -> case do_tarjan_immdom({element(3, FinGraph), element(4, FinGraph), element(1, FinGraph), element(2, FinGraph), element(6, FinGraph), element(5, FinGraph), element(7, FinGraph), element(8, FinGraph)}, 3, true) =/= element(1, element(6, FinGraph)) of true -> io:format("~p~n", [{P, S, do_tarjan_immdom({element(3, FinGraph), element(4, FinGraph), element(1, FinGraph), element(2, FinGraph), element(6, FinGraph), element(5, FinGraph), element(7, FinGraph), element(8, FinGraph)}, 3, true), element(6, FinGraph)}]), error(true); _ -> true end; true -> true end,
	...
	case BatchMode =:= false andalso gb_sets:size(array:get(P-1, Succ)) =:= 1 andalso gb_sets:to_list(array:get(P-1, Succ)) =:= [2] of
	%if no longer will be reverse-reachable from the exit node, then first must add artificial edge to the exit node - should be the deepest node from the nearest dominating node which does reach the exit and/or return node, like in the newest LLVM
	%how can this be determined? if the new successor was previously post-dominated by the predecessor (check by nca), then
	%this will be the edge which induces the infinite loop, however if another edge comes which makes an infinite loop reach the return and/or exit by means of non-artificial edge again, then this edge must be removed
	%could store a data structure containing all nodes of infinite loops, incrementally maintain it, can only find induced edges by comparing reverse graph and forward graph, but how to determine extent of the loop?
	true -> %case gb_sets:size(array:get(2-1, element(1, FinCrossGraph))) =:= 1 andalso gb_sets:size(array:get(3-1, element(1, FinCrossGraph))) =:= 1 of true -> FinCrossGraph; %infinite loops for theoretical testing
		case nca(element(6, FinCrossGraph), P, S) =:= P of true -> remove_edge(P, 2, add_edge(P, 3, FinCrossGraph, AST, BatchMode), AST, BatchMode);
		_ -> remove_edge(P, 2, FinCrossGraph, AST, BatchMode) end;
	_ -> case BatchMode =:= false andalso gb_sets:size(array:get(P-1, Succ)) =:= 1 andalso gb_sets:is_element(3, array:get(P-1, Succ)) of
		true -> remove_edge(P, 3, FinCrossGraph, AST, BatchMode); _ -> FinCrossGraph end end
.
...
remove_edge(P, S, {Pred, Succ, RPred, RSucc, Dom, Pdom, RetReach, CrossEdges}, AST, BatchMode) -> %sanity checks: cannot remove non-existant edges, removal cannot disconnect graph
	...
	%case do_tarjan_immdom({RemRPred, RemRSucc, RemPred, RemSucc, RemPdom, NextDom, RemRetReach, ExitCrossEdges}, case gb_sets:to_list(element(3, RemSucc)) =/= [] of true -> 2; _ -> 3 end, true) =/= element(1, RemPdom) of true -> error({P, S, do_tarjan_immdom({RemRPred, RemRSucc, RemPred, RemSucc, RemPdom, NextDom, RemRetReach, ExitCrossEdges}, case gb_sets:to_list(element(3, RemSucc)) =/= [] of true -> 2; _ -> 3 end, true), RemPdom}); _ -> true end,
	...
	%case do_tarjan_immdom({NewRPred, NewRSucc, RemPred, RemSucc, NextPdom, NextDom, RemRetReach, ExitCrossEdges}, case gb_sets:to_list(element(3, RemSucc)) =/= [] of true -> 2; _ -> 3 end, true) =/= element(1, NextPdom) of true -> error({P, S, gb_sets:to_list(element(3, RemSucc)), do_tarjan_immdom({NewRPred, NewRSucc, RemPred, RemSucc, NextPdom, NextDom, RemRetReach, ExitCrossEdges}, case gb_sets:to_list(element(3, RemSucc)) =/= [] of true -> 2; _ -> 3 end, true), NextPdom, element(1, RemPdom)}); _ -> true end,
	...
	%case get_j_succ(element(2, NextGraph), do_tarjan_immdom(NextGraph, 1, true)) =/= element(2, element(5, NextGraph)) of true -> io:format("~p~n", [{P, S, get_j_succ(element(2, NextGraph), do_tarjan_immdom(NextGraph, 1, true)), element(2, element(5, NextGraph))}]); _ -> true end,
	...
	%case do_tarjan_immdom(RetGraph, 1, true) =/= element(1, element(5, RetGraph)) of true -> error({P, S, do_tarjan_immdom(RetGraph, 1, true), element(5, RetGraph)}); _ -> true end,
	%if P =/= 2 orelse S =/= 3 ->
...
-define(TSCCPaperSucc, to_gb_sets({[2], [3, 8], [4, 7], [5], [3], [], [4, 6], [1, 7]})).

%Havlak, Sreedhar Gao Lee, Steengaard are 3 identified minimal loop nesting forests
%Havlak uses a DFS
%Sreedhar Gao Lee uses the DJ graph which we already have from dominator information
-define(SimplestIrrLoop, to_gb_sets({[2, 3], [3, 4], [2, 4], []})).
%Reduced Havlak which combines loops and thereby uses a slightly different computation algorithm from Ramalingam
%http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.35.8991&rep=rep1&type=pdf
-define(RamalingamIrrLoop, to_gb_sets({[2, 3], [4], [5], [2, 5, 6], [3, 4, 5], []})).

%https://lenx.100871.net/papers/loop-SAS.pdf
-define(NewAlgoPaperCaseA, to_gb_sets({[2], [3], [4], [5], [6], []})).
-define(NewAlgoPaperCaseB, to_gb_sets({[2], [3], [4], [5], [3]})).
-define(NewAlgoPaperCaseC, to_gb_sets({[2], [3], [4, 5], [6], [6], [7], []})).
-define(NewAlgoPaperCaseD, to_gb_sets({[2], [3], [4], [5, 6], [7], [7], [8], [3]})).
-define(NewAlgoPaperCaseE, to_gb_sets({[2], [3], [4, 5], [6], [7], [8], [8], [9], [2, 6]})).
check_newalgo() ->
	[newalgo_loops({succ_to_pred(?NewAlgoPaperCaseA), ?NewAlgoPaperCaseA, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?NewAlgoPaperCaseB), ?NewAlgoPaperCaseB, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?NewAlgoPaperCaseC), ?NewAlgoPaperCaseC, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?NewAlgoPaperCaseD), ?NewAlgoPaperCaseD, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?NewAlgoPaperCaseE), ?NewAlgoPaperCaseE, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?SimplestIrrLoop), ?SimplestIrrLoop, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?RamalingamIrrLoop), ?RamalingamIrrLoop, {}, {}, {}, {}, {}, {}}),
	newalgo_loops({succ_to_pred(?TSCCPaperSucc), ?TSCCPaperSucc, {}, {}, {}, {}, {}, {}})]
.

newalgo_loops(Graph) ->
	TagLoopHead = fun (B, H, Vs) ->
		if B =:= H orelse H =:= -1 -> Vs;
		true -> {FinalCur, FinalNextCur, VsUp} = fun TagLoop(Cur, NextCur, VsAcc) ->
			case element(2, array:get(Cur-1, VsAcc)) of -1 -> {Cur, NextCur, VsAcc};
			Ih -> if Ih =:= NextCur -> {Cur, NextCur, VsAcc};
				true -> case element(1, array:get(Ih-1, VsAcc)) < element(1, array:get(NextCur-1, VsAcc)) of true ->
						TagLoop(NextCur, Ih, array:set(Cur-1, setelement(2, array:get(Cur-1, VsAcc), NextCur), VsAcc));
					_ -> TagLoop(Ih, NextCur, VsAcc) end end end
			end(B, H, Vs),
			array:set(FinalCur-1, setelement(2, array:get(FinalCur-1, VsUp), FinalNextCur), VsUp)
		end end,
	{_, LoopInfo} = fun TraverseLoopsDFS(CurV, CurIdx, CurVs) ->
		VsNew = array:set(CurV-1, {CurIdx, -1, false}, CurVs),
		UpVs = gb_sets:fold(fun (W, AV) ->
			case element(1, array:get(W-1, AV)) of -1 -> {Nh, NxV} = TraverseLoopsDFS(W, CurIdx+1, AV), TagLoopHead(CurV, Nh, NxV); %case A
			0 -> case element(2, array:get(W-1, AV)) of -1 -> AV; %case C
				H -> case element(1, array:get(H-1, AV)) > 0 of true -> TagLoopHead(CurV, H, AV); %case D
					_ -> fun TakeHeads(CurH, Vs) -> case element(2, array:get(CurH-1, Vs)) of -1 -> Vs; NxH -> case element(1, array:get(NxH-1, Vs)) > 0 of true -> TagLoopHead(CurV, NxH, Vs); _ -> TakeHeads(NxH, Vs) end end end(H, AV) end end; %case E
			_ -> TagLoopHead(CurV, W, AV) end end, VsNew, get_succs(CurV, Graph)), %case B - loop header marked but not set as header of itself
		{element(2, array:get(CurV-1, UpVs)), array:set(CurV-1, setelement(1, array:get(CurV-1, UpVs), 0), UpVs)}
		end(1, 1, array:new(array:size(element(1, Graph)), {default, {-1, -1, false}})),
	LoopInfo
.

check_tarjan_scc() ->
	[tarjan_scc({succ_to_pred(?NewAlgoPaperCaseA), ?NewAlgoPaperCaseA, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?NewAlgoPaperCaseB), ?NewAlgoPaperCaseB, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?NewAlgoPaperCaseC), ?NewAlgoPaperCaseC, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?NewAlgoPaperCaseD), ?NewAlgoPaperCaseD, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?NewAlgoPaperCaseE), ?NewAlgoPaperCaseE, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?SimplestIrrLoop), ?SimplestIrrLoop, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?RamalingamIrrLoop), ?RamalingamIrrLoop, {}, {}, {}, {}, {}, {}}),
	tarjan_scc({succ_to_pred(?TSCCPaperSucc), ?TSCCPaperSucc, {}, {}, {}, {}, {}, {}})]
.

%note Tarjan's interval algorithm which identifies loops and if it is an irreducible graph containing at least 1 multi-entry loop - is from 1974 unlike this 1972 algorithm
%but this algorithm is largely obsolete besides being a good academic exercise as the efficient new algorithm handles everything very efficiently in a single DFS pass including irreducible/multi-entry loops
%strongly connected components - identifies loop groupings (not individual loops if they are nested) - perfect for infinite loop exit node connectedness
%basically a DFS which uses a stack of nodes, and an array with for each vertex containing its index and computed lowlink and if its on the stack
tarjan_scc(Graph) ->
	{_, _, _, SCCs} = lists:foldl(fun (V, {AccIdx, AccV, AccS, AccSCCs}) ->
		case element(1, array:get(V-1, AccV)) =:= -1 of true ->
			fun StrongConnect(CurV, CurIdx, CurVs, CurS, CurSCCs) ->
				VsNew = array:set(CurV-1, {CurIdx, CurIdx, true}, CurVs),
				{IdxUp, UpVs, UpS, UpSCCs} = gb_sets:fold(fun (W, {AIdx, AV, AS, ASCCs}) ->
					case element(1, array:get(W-1, AV)) =:= -1 of true -> Res = StrongConnect(W, AIdx, AV, AS, ASCCs), VNow = array:get(CurV-1, element(2, Res)),
						setelement(2, Res, array:set(CurV-1, setelement(2, VNow, min(element(2, VNow), element(2, array:get(W-1, element(2, Res))))), element(2, Res)));
					_ -> case element(3, array:get(W-1, AV)) of true -> VNow = array:get(CurV-1, AV), {AIdx, array:set(CurV-1, setelement(2, VNow, min(element(2, VNow), element(1, array:get(W-1, AV)))), AV), AS, ASCCs};
						_ -> {AIdx, AV, AS, ASCCs} end end
					end, {CurIdx+1, VsNew, [CurV|CurS], CurSCCs}, get_succs(CurV, Graph)),
				VUp = array:get(CurV-1, UpVs),
				if element(1, VUp) =:= element(2, VUp) ->
				  {FinalVs, FinalS, NewSCC} = fun TakeSCC(Ve, St, CurSCC) -> W = hd(St), Vts = array:set(W-1, setelement(3, array:get(W-1, Ve), false), Ve),
		...
		_ -> {AccIdx, AccV, AccS, AccSCCs} end
		end, {0, array:new(array:size(element(1, Graph)), {default, {-1, -1, false}}), [], []}, lists:seq(1, array:size(element(1, Graph)))),
	SCCs
.
...
get_revgraph(Graph) -> %on return the single node paths give true post dominators, not force return node to remain
	RetReach = fun PredRecurse(Pcsd, X) -> Comb = gb_sets:union(Pcsd, X), Y = gb_sets:from_list(gb_sets:fold(fun (El, Acc) ->
			case not gb_sets:is_element(El, Comb) of true -> [El|Acc]; _ -> Acc end end, [], gb_sets:union(gb_sets:fold(fun (El, Acc) -> [get_preds(El, Graph)|Acc] end, [], X)))),
		case gb_sets:size(Y) of 0 -> Comb; _ -> PredRecurse(Comb, Y) end end(gb_sets:new(), gb_sets:from_list([2])),
	%infinite loops: maximum depth node from part which reaches return/exit is drawn to exit
	%must deal with this before fencing algorithm
	%finding infinite loops: all nodes not reaching return/exit - in this set, determine which contain back edges and add exit edges
	ExitReach = fun PredRecurse(Pcsd, X) -> Comb = gb_sets:union(Pcsd, X), Y = gb_sets:from_list(gb_sets:fold(fun (El, Acc) ->
			case not gb_sets:is_element(El, Comb) of true -> [El|Acc]; _ -> Acc end end, [], gb_sets:union(gb_sets:fold(fun (El, Acc) -> [get_preds(El, Graph)|Acc] end, [], X)))),
		case gb_sets:size(Y) of 0 -> Comb; _ -> PredRecurse(Comb, Y) end end(gb_sets:new(), gb_sets:from_list([3])),
	%DFS = do_dfs(element(2, Graph), 1),
	{DFS, RDFS} = do_interval_dfs(Graph, 1),
	InfLoop = gb_sets:difference(gb_sets:from_list(lists:seq(1, array:size(element(2, Graph)))), ExitReach),
	DeepInfLoop = gb_sets:filter(fun (El) -> gb_sets:fold(fun (E, _Acc) -> %nca(Dom, E, El) =:= E %if have dominator info, can simple check that successor dominates predecessor for reducible infinite loops only though - not irreducibles
		array:get(El-1, DFS) > array:get(E-1, DFS) andalso array:get(El-1, RDFS) < array:get(E-1, RDFS) end, false, get_succs(El, Graph)) end, InfLoop),
	InfGraph = gb_sets:fold(fun (E, AGraph) ->
		setelement(2, setelement(1, AGraph, array:set(3-1, gb_sets:add_element(E, array:get(3-1, element(1, AGraph))), element(1, AGraph))),
			array:set(E-1, gb_sets:add_element(3, array:get(E-1, element(2, AGraph))), element(2, AGraph))) end, Graph, DeepInfLoop),
	%erlang:display({gb_sets:to_list(InfLoop), gb_sets:to_list(DeepInfLoop), tarjan_scc(Graph), get_edgeset(element(2, InfGraph))}),
	...
	%non-incremental fencing and subgraph algorithm is here:
	%Doms = do_tarjan_immdom(rev_graph(InfGraph), case gb_sets:to_list(get_succs(2, InfGraph)) =/= [3] of true -> 2; _ -> 3 end, true),
	Doms = get_post_dom_divide(Graph),
	DomInfo = d_to_dom(Doms, get_j_succ(element(2, rev_graph(InfGraph)), Doms), make_dom_tree(Doms), dom_depth(Doms)),
	%erlang:display({Doms, get_edgeset(element(2, rev_graph(InfGraph))), get_edgeset(get_j_succ(element(2, rev_graph(InfGraph)), Doms)), DomInfo}),
	PostDomGraph = case gb_sets:size(get_succs(3, InfGraph)) =:= 1 andalso gb_sets:to_list(get_succs(3, InfGraph)) =:= [2] of true -> InfGraph; _ ->
	    %exhaustive maxima search, now considering all combinations of not each fence element, but each successor per fence element, as conflicts are certainly possible
	    CorrectDoms = get_post_dom_divide(Graph),
	    AllFence = lists:append(gb_sets:fold(fun (Elem, Acc) -> [lists:map(fun (E) -> {Elem, E} end, gb_sets:to_list(element(1, pred_set_recurse(gb_sets:from_list([Elem]), RetReach, InfGraph))))|Acc] end,
	      [], gb_sets:del_element(2, get_preds(3, InfGraph)))),
		  Maxes = lists:foldl(fun (Idx, AccMaxes) ->
		    %if Idx such that not even a single fence element of a given exit predecessor was chosen then must exclude - bad graph
		    {_, CurFence, Remain, RepEls} = lists:foldl(fun (E, {I, A, AR, Els}) -> if Idx band (1 bsl I) =/= 0 -> {I + 1, [E|A], AR, gb_sets:add_element(element(1, E), Els)}; true -> {I + 1, A, [E|AR], Els} end end, {0, [], [], gb_sets:new()}, AllFence),
				NextR = lists:foldl(fun ({Elem, X}, Ac) ->
				gb_sets:fold(fun (El, AGraph) -> case Elem =/= El andalso not gb_sets:is_element(El, get_succs(Elem, AGraph)) of true ->
			setelement(2, setelement(1, AGraph, array:set(El-1, gb_sets:add_element(Elem, array:get(El-1, element(1, AGraph))), element(1, AGraph))),
				array:set(Elem-1, gb_sets:add_element(El, array:get(Elem-1, element(2, AGraph))), element(2, AGraph))); _ -> AGraph end end, Ac, get_succs(X, InfGraph)) end,
					InfGraph, CurFence), %pred_set_recurse returns a fence where elements reach each other - but this causes side effects in post-dominators of the return set
				Next = lists:foldl(fun ({_, X}, Ac) ->
				gb_sets:fold(fun (El, AGraph) -> case not gb_sets:is_element(El, RetReach) of true ->
			setelement(2, setelement(1, AGraph, array:set(El-1, gb_sets:del_element(X, array:get(El-1, element(1, AGraph))), element(1, AGraph))),
				array:set(X-1, gb_sets:del_element(El, array:get(X-1, element(2, AGraph))), element(2, AGraph))); _ -> AGraph end end, Ac, get_succs(X, InfGraph)) end,
					NextR, Remain),
			CheckGraph = gb_sets:fold(fun (Elem, AGraph) ->
			  case gb_sets:size(get_succs(Elem, AGraph)) =:= 1 andalso gb_sets:to_list(get_succs(Elem, AGraph)) =:= [3] of true -> AGraph;
  	    _ -> setelement(2, setelement(1, AGraph, array:set(3-1, gb_sets:del_element(Elem, array:get(3-1, element(1, AGraph))), element(1, AGraph))),
				  array:set(Elem-1, gb_sets:del_element(3, array:get(Elem-1, element(2, AGraph))), element(2, AGraph))) end end, Next, gb_sets:del_element(2, get_preds(3, InfGraph))),
			%erlang:display({CurFence, get_post_dom(Graph, CheckGraph, RetReach), CorrectDoms}),
			case gb_sets:size(RepEls) =:= gb_sets:size(gb_sets:del_element(2, get_preds(3, InfGraph))) andalso get_post_dom(Graph, CheckGraph, RetReach) =:= CorrectDoms andalso (AccMaxes =:= [] orelse length(CurFence) >= length(hd(AccMaxes))) of true ->
			  case AccMaxes =/= [] andalso length(CurFence) =:= length(hd(AccMaxes)) of true -> [CurFence|AccMaxes]; _ -> [CurFence] end; _ -> AccMaxes end
					end, %skip no fence case as it will cause a disconnected graph for sure
			[], lists:seq(1, 1 bsl length(AllFence) - 1)),
		%erlang:display(Maxes),
	    %the IDF if it sent back all nodes visited and not just the frontier (basically the frontier plus all dominator child subtrees) would be the frontier potential
			%if the IDF is empty then there is no effect to draw an edge, if 2 or more - can select safely any one of them
			%by construction reverse graph dominators has Zd=Return node, Xd=successor of fence node, Yd=the subgraph entry!
			%loops in subgraph wont matter unless the j-edge goes to a higher level than the header but this by definition should not happen also since reaches to entry?
			%need to use dominator tree to choose only one node in any set which have a dominator relationship - bottom most or top most for consistency
			%the key is to define the sets of nodes which contain exclusion dependencies - each of them in an exclusion dependency graph - then try to maximize properties e.g. number of nodes, number of edges, dominator levels, some formula relating all of these things, and of course finally a tie breaker based on some partial order
			PotIDFs = lists:foldl(fun ({_, X}, A) -> %IDF = compute_idf(DomInfo, gb_sets:from_list([X])),
			  %PotentialIDF = gb_sets:del_element(X, gb_sets:union(lists:map(fun (E) -> gb_sets:from_list(lists:append(fun TreeDFS(Cur, Xe) ->
		%lists:foldl(fun (El, Acc) -> #{El := {_,_,ElDTree,_}} = DomInfo,
			%TreeDFS(Acc, gb_sets:to_list(ElDTree)) end, [Xe|Cur], Xe) end([], [E]))) end, [X|IDF]))),
			  IDF = compute_idf(DomInfo, gb_sets:from_list([X])),
			  PotentialIDF = gb_sets:del_element(X, gb_sets:union(lists:map(fun (E) -> gb_sets:from_list(lists:append(fun TreeDFS(Cur, Xe) ->
		lists:foldl(fun (El, Acc) -> #{El := {_,_,ElDTree,_}} = DomInfo,
			TreeDFS(Acc, gb_sets:to_list(ElDTree)) end, [Xe|Cur], Xe) end([], [E]))) end, [X|IDF]))),
			A#{X => {IDF, PotentialIDF}} end, #{}, AllFence),
			%build a dominance preservation exclusionary dependency graph using the potential IDF vs the combination of each fence node with its successors
			%then must choose a maximal set based on some criterion from this - however only a single node will ever be possible - any of them basically
			%so when can more be included?  based on Zd being the new dominator of join nodes in the IDF - so if cumulative NCA of two fence nodes successor nodes is the same dominator for of all of these successor nodes in both of their IDFs - its inclusively re-allowed!!!  seems like a narrow case but its logically by definition the only case, and likely occurs broadly in practice
			%exhaustive maximization excluding searching dominance breaking additions
			%{FenceNoJConflict, _} = maps:fold(fun (K, V, {A, AExclude}) -> case gb_sets:is_element(K, AExclude) orelse gb_sets:size(gb_sets:intersection(V, A)) =/= 0 orelse not lists:member(K, hd(Maxes)) of true -> {A, AExclude}; _ -> {gb_sets:add_element(K, A), gb_sets:union(V, AExclude)} end end, {gb_sets:new(), gb_sets:new()}, DepExclusionGraph),
			FenceNoJConflicts = fun FindMaxFence(Nodes, A, CurIDFs) -> if Nodes =:= [] -> [A]; true -> [K|Remaining] = Nodes, {Elem, X} = K, ContinueRes = FindMaxFence(Remaining, A, CurIDFs),
			  #{X := {XDom, _, _, XDomLevel}} = DomInfo, #{X := {IDF, PotentialIDF}} = CurIDFs,
			  ReverseConflict = gb_sets:fold(fun ({_,N}, AccConf) -> #{N := {NDom, _, _, NDomLevel}} = DomInfo, #{N := {RevIDF, _PIDF}} = CurIDFs,
			  	DAffected = lists:filter(fun (Wd) -> #{Wd := {_,_,_,WdLvl}} = DomInfo, WdLvl > NDomLevel end, [N|RevIDF]),
			  	AccConf orelse lists:any(fun (D) -> #{D := {Dm, _, _, _}} = DomInfo, %erlang:display({D, Dm, NDom, N}),
				NDom =/= Dm end, DAffected) end, false, A),
		  	DomAffected = lists:filter(fun (Wd) -> #{Wd := {_,_,_,WdLvl}} = DomInfo, WdLvl > XDomLevel end, [X|IDF]), %Zd=XDom, XDomLevel = ZdLevel + 1 by definition due to construction
			  %erlang:display({A, Elem, X, XDom, gb_sets:to_list(get_succs(X, InfGraph)), IDF, gb_sets:to_list(PotentialIDF), DomAffected}),
			  case ReverseConflict orelse lists:any(fun (D) -> #{D := {Dm, _, _, _}} = DomInfo, erlang:display({D, Dm, XDom, X}), XDom =/= Dm end, DomAffected) %orelse
			    %lists:any(fun ({_, D}) -> #{D := {Dm, _, _, _}} = DomInfo, XDom =/= Dm end, gb_sets:to_list(A))
			    of true -> ContinueRes;
			  _ -> NextA = FindMaxFence(Remaining, gb_sets:add_element(K, A),
			  	maps:fold(fun (Kv, {VIDF, VPIDF}, Av) -> #{Kv := {_, _, _, KvLevel}} = DomInfo, case gb_sets:is_element(X, VPIDF) andalso XDomLevel =< KvLevel of true -> Av#{Kv := {[X|VIDF], VPIDF}}; _ -> Av end end, CurIDFs, CurIDFs)),
			  	ContinueRes ++ NextA end end %case gb_sets:size(ContinueRes) > gb_sets:size(NextA) of true -> ContinueRes; _ -> NextA end end end
			  end(AllFence, gb_sets:new(), PotIDFs),
			FenceNoJConflict = lists:foldl(fun (L, A) -> case gb_sets:size(L) >= gb_sets:size(A) of true -> L; _ -> A end end, gb_sets:new(), FenceNoJConflicts),
			%after already taking the highest dominator to try to minimize J edges, as any lower in a dominator set would automatically be J edges relative to the higher ones
			%this means the D and J edges which cannot cause affectedness are SUCC(X)->Elem (D edge if one, otherwise J-edges it multiple) and Elem->X (J edge), so if IDF(X) is not empty, can only have a single one
			%but any which dominate nothing or have no j edges are always safe to draw
			%FenceNoJConflict = gb_sets:union(FenceNoJConf, gb_sets:filter(fun (X) -> IDF = compute_idf(DomInfo, gb_sets:from_list([X])), %how about mutually exclusive IDFs?
			  %IDF =:= [] orelse %gb_sets:fold(fun (E, A) -> 
			  %gb_sets:fold(fun (E, A) -> A andalso (gb_sets:size(get_succs(X, AccGraph)) > gb_sets:size(get_succs(E, AccGraph)) orelse gb_sets:size(get_succs(X, AccGraph)) =:= gb_sets:size(get_succs(E, AccGraph)) andalso begin #{X := {_, _, _, L1}} = DomInfo, #{E := {_, _, _, L2}} = DomInfo, L1 < L2 orelse L1 =:= L2 andalso (gb_sets:is_subset(get_succs(X, AccGraph), get_succs(E, AccGraph)) orelse gb_sets:smallest(gb_sets:difference(get_succs(X, AccGraph), get_succs(E, AccGraph))) < gb_sets:smallest(gb_sets:difference(get_succs(E, AccGraph), get_succs(X, AccGraph)))) end) end, true, gb_sets:del_element(X, FenceNoJConfPos)) end, FenceNoJConfPos)),
			case Maxes =/= [] andalso lists:all(fun (E) -> lists:sort(gb_sets:to_list(FenceNoJConflict)) =/= lists:sort(E) end, Maxes) of true ->
				error({gb_sets:to_list(RetReach), gb_sets:to_list(FenceNoJConflict), AllFence -- gb_sets:to_list(FenceNoJConflict), FenceNoJConflicts, io:format("~s", [edgeset_to_dot(element(2, Graph), "v")])}); _ -> true end,
			NextR = gb_sets:fold(fun ({Elem, X}, Ac) ->
			gb_sets:fold(fun (El, AGraph) -> case Elem =/= El andalso not gb_sets:is_element(El, get_succs(Elem, AGraph)) andalso gb_sets:is_element(El, RetReach) of true ->
		setelement(2, setelement(1, AGraph, array:set(El-1, gb_sets:add_element(Elem, array:get(El-1, element(1, AGraph))), element(1, AGraph))),
			array:set(Elem-1, gb_sets:add_element(El, array:get(Elem-1, element(2, AGraph))), element(2, AGraph))); _ -> AGraph end end, Ac, get_succs(X, InfGraph)) end,
				InfGraph, FenceNoJConflict), %pred_set_recurse returns a fence where elements reach each other - but this causes side effects in post-dominators of the return set
			Next = lists:foldl(fun ({_, X}, Ac) -> % andalso gb_sets:fold(fun (E, Acc) -> Acc orelse not gb_sets:is_element(E, RetReach) end, false, get_preds(El, AccGraph))
			gb_sets:fold(fun (El, AGraph) -> case not gb_sets:is_element(El, RetReach) of true -> %if this would cause a node to no longer reach the entry node, how to approach this as it appears as a "corrupt" reverse graph
		setelement(2, setelement(1, AGraph, array:set(El-1, gb_sets:del_element(X, array:get(El-1, element(1, AGraph))), element(1, AGraph))),
			array:set(X-1, gb_sets:del_element(El, array:get(X-1, element(2, AGraph))), element(2, AGraph))); _ -> AGraph end end, Ac, get_succs(X, InfGraph)) end,
				NextR, AllFence -- gb_sets:to_list(FenceNoJConflict)),
		gb_sets:fold(fun (Elem, AGraph) ->
		  case gb_sets:size(get_succs(Elem, AGraph)) =:= 1 andalso gb_sets:to_list(get_succs(Elem, AGraph)) =:= [3] of true -> AGraph;
		  _ -> setelement(2, setelement(1, AGraph, array:set(3-1, gb_sets:del_element(Elem, array:get(3-1, element(1, AGraph))), element(1, AGraph))),
				array:set(Elem-1, gb_sets:del_element(3, array:get(Elem-1, element(2, AGraph))), element(2, AGraph))) end end, Next, gb_sets:del_element(2, get_preds(3, InfGraph)))
	  end,
	erlang:display({get_edgeset(element(1, PostDomGraph)), get_edgeset(element(2, PostDomGraph)), check_pred_succ(InfGraph), check_pred_succ(PostDomGraph)}),
	%{MinPostDomGraph, RetReach, InfGraph}
	{PostDomGraph, RetReach, InfGraph}
.

check_post_doms(Graph) ->
	element(1, get_post_dom(Graph)) =:= get_post_dom_divide(Graph).

get_post_dom_divide(Graph) ->
	%in graph with no return, this is trivial
	case gb_sets:to_list(get_succs(2, Graph)) =/= [3] of true -> do_tarjan_immdom(rev_graph(Graph), 2, true); _ ->
	...
	{RetNodeMap, NotRetNodeMap, BackRetNodeMap, BackNotRetNodeMap} = lists:foldl(fun (El, {RetAcc, NotRetAcc, BRetAcc, BNotRetAcc}) -> case gb_sets:is_element(El, RetReach) of true -> Sz = maps:size(RetAcc) + 1, {RetAcc#{El => Sz}, NotRetAcc, BRetAcc#{Sz => El}, BNotRetAcc}; _ -> Sz = maps:size(NotRetAcc) + 1, {RetAcc, NotRetAcc#{El => Sz}, BRetAcc, BNotRetAcc#{Sz => El}} end end,
		{#{}, #{}, #{}, #{}}, lists:seq(1, array:size(element(1, InfGraph)))),
	{RetGraph, NotRetGraph} = lists:foldl(fun (N, {RetAcc, NotRetAcc}) -> NP = array:get(N-1, element(1, InfGraph)), NS = array:get(N-1, element(2, InfGraph)), PIsect = gb_sets:intersection(NP, RetReach), SIsect = gb_sets:intersection(NS, RetReach),
		case gb_sets:is_element(N, RetReach) of true -> #{N := M} = RetNodeMap, {setelement(2, setelement(1, RetAcc, array:set(M-1, gb_sets:fold(fun (El, Acc) -> #{El := X} = RetNodeMap, gb_sets:add(X, Acc) end, gb_sets:new(), PIsect), element(1, RetAcc))), array:set(M-1, gb_sets:fold(fun (El, Acc) -> #{El := X} = RetNodeMap, gb_sets:add(X, Acc) end, gb_sets:new(), SIsect), element(2, RetAcc))), NotRetAcc};
		_ -> #{N := M} = NotRetNodeMap, {RetAcc, setelement(2, setelement(1, NotRetAcc, array:set(M-1, gb_sets:fold(fun (El, Acc) -> #{El := X} = NotRetNodeMap, gb_sets:add(X, Acc) end, gb_sets:new(), gb_sets:difference(NP, PIsect)), element(1, NotRetAcc))), array:set(M-1, gb_sets:fold(fun (El, Acc) -> #{El := X} = NotRetNodeMap, gb_sets:add(X, Acc) end, gb_sets:new(), gb_sets:difference(NS, SIsect)), element(2, NotRetAcc)))} end end,
		{setelement(2, setelement(1, InfGraph, array:new(gb_sets:size(RetReach))), array:new(gb_sets:size(RetReach))), setelement(2, setelement(1, InfGraph, array:new(array:size(element(1, InfGraph)) - gb_sets:size(RetReach))), array:new(array:size(element(1, Graph)) - gb_sets:size(RetReach)))}, lists:seq(1, array:size(element(1, Graph)))),
	PostRet = do_tarjan_immdom(rev_graph(RetGraph), 2, true),
	PostNotRet = do_tarjan_immdom(rev_graph(NotRetGraph), 1, true),
	array:set(2-1, 3, array:foldl(fun (Idx, Val, Acc) -> I=Idx+1, #{I := RetIdx} = BackNotRetNodeMap, if Val =:= 0 -> Num = 0; true -> #{Val := Num} = BackNotRetNodeMap end, array:set(RetIdx-1, Num, Acc) end, 
		array:foldl(fun (Idx, Val, Acc) -> I=Idx+1, #{I := RetIdx} = BackRetNodeMap, if Val =:= 0 -> Num = 0; true -> #{Val := Num} = BackRetNodeMap end, array:set(RetIdx-1, Num, Acc) end, array:new(array:size(element(1, Graph)), {default,0}), PostRet), PostNotRet)) end.

get_post_dom(Graph, PostDomGraph, RetReach) ->
  LenPred = next_node(Graph) - 1,
	fun FixPdom(Elem, Acc) -> if Elem =:= LenPred + 1 -> Acc; true -> FixPdom(Elem+1,
		case gb_sets:to_list(get_succs(3, Graph)) =/= [2] andalso
			gb_sets:is_element(Elem, gb_sets:del_element(2, get_preds(3, Graph))) orelse
			gb_sets:is_element(array:get(Elem-1, Acc), RetReach) andalso not gb_sets:is_element(Elem, RetReach) of true ->
			array:set(Elem-1, 3, Acc); _ -> Acc end) end end(1,
			do_tarjan_immdom(rev_graph(PostDomGraph), case gb_sets:to_list(get_succs(2, PostDomGraph)) of [3] -> 3; _ -> 2 end, true)).
			
get_post_dom(Graph) -> %on return the single node paths give true post dominators, not force return node to remain
	{PostDomGraph, RetReach, _RevGraph} = get_revgraph(Graph),
	{get_post_dom(Graph, PostDomGraph, RetReach), RetReach, PostDomGraph}
.
		...
		lists:filter(fun (A) -> A =/= false end,
			...
			of false -> case gb_sets:to_list(get_preds(P, Graph)) of [Par] -> case getnodeblockstruct(AST, Par, 0) =:= 'try' andalso isblockchild(AST, try_end, true, Par, P) of true -> {try_end, Par}; _ -> false end; _ -> false end;
				...
				Kind -> if Node =:= 2 -> false; true -> #{Node := PDomNode} = Pdom, case {
					getblockstructchild(AST, get_succs(P, Graph), Kind, true, P, lists:member(otp20, Opts)),
					getblockstructchild(AST, get_succs(P, Graph), Kind, false, P, lists:member(otp20, Opts))} of
				...
				{if IsTry andalso not TryHasOf ->
					setgraphpathchild(AccAST, {'try', true}, true, HeadNode,
						setelement(4, getgraphpathchild(AccAST, {'try', true},
							true, HeadNode, lists:member(otp20, Opts)), []), lists:member(otp20, Opts)); true -> AccAST end,
			AccGraph, 0, 0};
		...
		%if they are merge nodes which post dominate the head nodes of the blocks
			...
			FixAST = if FixNodes =:= [] -> AccAST; true ->
				...
				{graphdata, 0, array:new(1024, {default,[]}), array:new({default,[]}),
					array:new(16, {default,[]})}, LenAccPred, lists:member(otp20, Opts)) end,
			NextAST = if FixAfterNodes =:= [] -> FixAST;
			true ->
				FixASTNext = insertgraphnodechild(FixAST, HeadNode, {Kind, true}, false,
					{graphdata, 0, array:new(1024, {default,[]}), array:new({default,[]}),
						array:new(16, {default,[]})},
					LenAccPred + if FixNodes =:= [] -> 0; true -> 1 end, lists:member(otp20, Opts)),
				if FixNodes =:= [] -> if IsTry andalso not TryHasOf ->
					setgraphpathchild(FixASTNext, {'try', true}, true, HeadNode,
						setelement(4, getgraphpathchild(FixASTNext, {'try', true},
							true, HeadNode, lists:member(otp20, Opts)), []), lists:member(otp20, Opts)); true -> FixASTNext end;
...
handle_merge_node(AST, Graph, CurNode, Node, AssignedVars, VarPrefix,
	...
	_ ->
	  %first must redirect all return labels not yet reached to the exit latch
		...
		%if Node =:= 2 -> Idom = do_tarjan_immdom(rev_graph(Graph), 3, true); true ->
		%{Idom, _, _} = get_post_dom(Graph),
		...
		IsNotPdomOf = Node =/= 2 andalso gb_sets:size(NodeDTree) =:= 0 andalso if IsCatch -> gb_sets:size(NodePdomDTree) =:= 0; true -> true end, %still need variable assignment here, or must go through its predecessors not just dominators at the post dominating node which assigns
		  ...
		  NearestCrossPdom = array:map(fun (Idx, X) -> IdxP1 = Idx + 1, #{IdxP1 := {IdxPdom,_,_,_}} = Pdom, IdxPdom, XPath = pathtodom(Pdom, IdxP1),
		  	...
		  	lists:map(fun (Y) -> #{Y := {YPdom,_,_,_}} = Pdom,
			  	...
			  	NCA = fun WalkPdom(El) -> case getnodeblockstruct(AST, El, 0) of
			  		...
			  		'try' -> #{El := {ElPdom,_,_,_}} = Pdom, case issinglepath(AST, 'try', El, ElPdom, lists:member(otp20, Opts)) of true -> 2
			  		; _ -> if ElPdom =:= 2 orelse ElPdom =:= 3 -> ElPdom; true -> WalkPdom(ElPdom) end end;
			  		'receive' -> #{El := {ElPdom,_,_,_}} = Pdom, case issinglepath(AST, 'receive', El, ElPdom, lists:member(otp20, Opts)) of true -> 2
			  		; _ -> if ElPdom =:= 2 orelse ElPdom =:= 3 -> ElPdom; true -> WalkPdom(ElPdom) end end;
		  ...
		  %		lists:nth(N, Pred)) || N <- lists:seq(1, LenPred - 1)],
			...
			true ->
				%ChkDom = do_tarjan_immdom(rev_graph(Graph), 1, true),
			...
			Dom = element(5, RetGraph), %do_tarjan_immdom(RetGraph, 1, true),
			...
			%if Node =:= 2 ->
			%	PostIdom = do_tarjan_immdom(rev_graph(RetGraph), 3, true); true -> 
	...
	_ -> case InstList of [] ->
		...
		case Val of
			...
			remove_message -> %in this case predecessor search looks to be a valid option and largely the only option due to no side effects or more specifically receives between one receive polling and its processing
				...
				NewAST = insertgraphnode(AST, CurNode, {call,0,getgraphpathchild(AST, remove_message, false, RecvNode, lists:member(otp20, element(5, Glob))),[getgraphpathchild(AST, remove_message, true, RecvNode, lists:member(otp20, element(5, Glob)))]}, Cur, CurNode),
			...
			timeout ->
			  case isnodeloopback(AST, CurNode) of false -> %wait_timeout already processed
				...
				true -> RecvNode = gb_sets:smallest(get_preds(CurNode, Graph)),
					...
					NewAST = insertgraphnodechild(setgraphpathchild(AST, timeout, true, RecvNode, {call,0,{'fun',0,{clauses,get_clean_ast_all(element(4, Glob), 'receive', 3, lists:member(stubfuncs, element(5, Glob)))}},[getgraphpathchild(AST, timeout, true, RecvNode, lists:member(otp20, element(5, Glob))), {integer,0,0}, {'fun',0,[{clauses,[{clause,0,[],[],[]}]}]}]}, lists:member(otp20, element(5, Glob))), RecvNode, 'receive', false, getgraphpath(AST, RecvNode), NewNode, lists:member(otp20, element(5, Glob))),
			...
			build_stacktrace -> decompile_step({tl(InstList), %technically this is a raw stacktrace not the erlang:get_stacktrace one - only usable by raw_raise, but from a practical standpoint this raw type does not matter at all though ideally it would be indicated
				setgraphpath(AST, CurNode, setmemd(Cur, {x, 0},
					{call,0,{remote,0,{atom,0,erlang},{atom,0,get_stacktrace}},[]})), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
			raw_raise -> {NewAST, NewGraph, NewAsgn, Crs, CNodes, CEdges, CREdges} = %TODO Fix with case x0 =:= error orelse x0 =:= exit orelse x0 =:= throw of true -> erlang:raise; _ -> true end
			    %If x(0) is not a valid class, the instruction will not throw an exception, but store the atom 'badarg' in x(0) and execute the next instruction.
			    % how is it possible that x(0) is not valid though, unless the after clause can affect/corrupt it???
					handle_exit_merge(insertgraphnode(AST, CurNode,
				{call,0,{remote,0,{atom,0,erlang},{atom,0,raise}},[getmemd(Cur, {x, 0}), getmemd(Cur, {x, 1}), getmemd(Cur, {x, 2})]}, 0),
				Graph, NodeState, CurNode, AssignedVars, VarPrefix, {element(1, Glob) + 1, LabelToNode, element(2, element(3, Glob)), hd(element(2, Glob))}, element(5, Glob)),
				#{0 := {C, CN, CE, CRE}} = Stats,
		...
		%case gb_sets:size(Y) of 0 -> Comb; _ -> PredRecurse(Comb, Y) end end(gb_sets:new(), gb_sets:from_list([E]))) end, gb_sets:to_list(get_preds(NewNode, AccGraph))) of [A|_] ->
				...
				'catch' -> NewNode = next_node(Graph), LblIdx = element(2, element(3, Val)) - element(1, Glob) + 1, NotExists = case LabelToNode of #{LblIdx := _} -> false; _ -> true end,
					...
					ModAST = insertgraphnodechild(InitAST,
						CurNode, 'catch', true, {graphdata, 0, element(3, Cur), element(4, Cur), element(5, Cur)}, NewNode + if NotExists -> 1; true -> 0 end, lists:member(otp20, element(5, Glob))),
				...
				'try' -> NewNode = next_node(Graph), %this state preserved until try_end or try_case
					...
					InitAST = insertgraphnode(AST, %[4, 1, 4, 1, 5, 1] added but not referenced and either cleaned up or referenced later
						CurNode, {match,0,{var,0,list_to_atom(VarPrefix ++ "Var" ++ integer_to_list(AssignedVars))},
							[{'try', 0, [], [{clause,0,[{var,0,list_to_atom(VarPrefix ++ "TryVar" ++ integer_to_list(AssignedVars))}],[],
								...
								[{clause,0,[{tuple,0,[{var,0,list_to_atom(VarPrefix ++ "Class" ++ integer_to_list(AssignedVars))},{var,0,list_to_atom(VarPrefix ++ "Reason" ++ integer_to_list(AssignedVars))},
										{var,0,case lists:member(otp20, element(5, Glob)) of true -> '_'; _ -> list_to_atom(VarPrefix ++ "Stacktrace" ++ integer_to_list(AssignedVars)) end}]}],[],
									case lists:member(otp20, element(5, Glob)) of true -> [{match,0,{var,0,list_to_atom(VarPrefix ++ "Stacktrace" ++ integer_to_list(AssignedVars))},{call,0,{remote,0,{atom,0,erlang},{atom,0,get_stacktrace}},[]}}]; _ -> [] end}], []}]},
					...
					ModAST = insertgraphnodechild(insertgraphnodechild(InitAST,
						CurNode, 'try', true, setmemd({graphdata, 0, element(3, Cur), element(4, Cur), element(5, Cur)}, element(2, Val), [CurNode]), NewNode + 1, lists:member(otp20, element(5, Glob))),
						CurNode, 'try', false, if NotExists -> setmemd(setmemd(setmemd({graphdata, 0, element(3, Cur), element(4, Cur), element(5, Cur)}, {x,0}, {var,0,list_to_atom(VarPrefix ++ "Class" ++ integer_to_list(AssignedVars))}), {x,1}, {var,0,list_to_atom(VarPrefix ++ "Reason" ++ integer_to_list(AssignedVars))}), {x,2}, {var,0,list_to_atom(VarPrefix ++ "Stacktrace" ++ integer_to_list(AssignedVars))});
							true -> getgraphpath(AST, begin #{LblIdx := LblNode} = LabelToNode, LblNode end) end, NewNode + 2, lists:member(otp20, element(5, Glob))),
					%io:format("~p~n", [{[[getnextsibling(CurNodePath) ++ [3, 1]], [getnextsibling(CurNodePath) ++ [5, 1, 5, if IsOTP20 -> 2; true -> 1 end]]]}]),
				...
				try_end -> %all changed values must be assigned variables, and moved to the merge node outside of try block, it must be assumed to be assigned even if an unbound error could occur
					...
					%OTP21: if this is a guard try then an empty try with a try_end at the catch location occurs - must emit an emit catch _:_ in that case
					FilterNodes = lists:dropwhile(fun (El) -> gb_sets:is_element(El, array:get(CurNode-1, element(8, Graph))) end, gb_sets:to_list(HeadNodes)),
					IsTryCatch = case isnodeloopback(AST, CurNode) of true -> []; _ -> lists:filter(fun (El) -> getnodeblockstruct(AST, El, 0) =:= 'try' end, gb_sets:to_list(get_preds(CurNode, Graph))) end,
					%erlang:display({HeadNodes, CurNode, FilterNodes, IsTryCatch}),
					case lists:any(fun (HeadNode) -> isblockchild(AST, 'try', false, HeadNode, CurNode) end, IsTryCatch) of true ->
					  decompile_step({tl(InstList), setgraphpath(AST, CurNode, setmemd(Cur, element(2, Val), [])), Graph, NodeState, LabelToNode,
						  CurNode, VarPrefix, AssignedVars, false, Stats}, Glob); _ ->
					case FilterNodes of [] -> decompile_step({tl(InstList), setgraphpath(AST, CurNode, setmemd(Cur, element(2, Val), [])), Graph, NodeState, LabelToNode,
						CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
					[MainNode|_] ->
					...
					{FixAST, FixGraph, NewAsgn, CntNode, _} = lists:foldl(fun (PredNode, {AccAST, AccGraph, AccAsgn, AccNode, PriorHeadNodes}) ->
						...
						_ ->
							...
							NewAST = insertgraphnodechild(setgraphpathchild(AccAST, 'try', true, HeadNode,
								setmemd(setelement(2, NewCur, element(2, getgraphpathchild(AccAST, 'try', true, HeadNode, lists:member(otp20, element(5, Glob))))),
											{x,0}, getmemd(getgraphpathchild(AccAST, 'try', true, HeadNode, lists:member(otp20, element(5, Glob))), {x,0})), lists:member(otp20, element(5, Glob))), HeadNode, try_end, true, {}, NewNode, lists:member(otp20, element(5, Glob))),
					...
					decompile_step({tl(InstList), FxAST, FxGraph, NodeState,
						...
						false, Stats#{0 := {C + Crs, CN + CNodes, CE + CEdges, CRE + CREdges}}}, Glob) end end;
				...
				loop_rec -> NewNode = next_node(Graph), %no graphdata containing the loopback label element(2, element(2, Val))
					ModAST = insertgraphnode(insertgraphnodechild(insertgraphnode(AST,
						...
						CurNode, 'receive', true, {graphdata, 0, element(3, setmemd(Cur, element(3, Val), {var,0,list_to_atom(VarPrefix ++ "Message" ++ integer_to_list(AssignedVars))})), element(4, Cur), element(5, Cur)}, NewNode + 1, lists:member(otp20, element(5, Glob))),
						{}, {}, NewNode + 2),
					{FxAST, FxGraph, FxAsgn, _, Crs, CNodes, CEdges, CREdges} = handle_merge_node(ModAST,
						add_edge(NewNode, 3,
							add_edge(CurNode, NewNode + 2,
				...
				wait_timeout -> %if a receive node exists, extend it from a 3 tuple to a 5 tuple and record lambda from here to node which post-dominates this and the loop_rec/loop_rec_end clause
					...
					ModAST = insertgraphnodechild(insertgraphnode(AST, CurNode, {'receive',0,[],getmemd(Cur, element(3, Val)),[]}, 0),
						CurNode, wait_timeout, true, {graphdata, 0, element(3, Cur), element(4, Cur), element(5, Cur)}, NewNode, lists:member(otp20, element(5, Glob))),
					{FxAST, FxGraph, FxAsgn, _, Crs, CNodes, CEdges, CREdges} = handle_merge_node(ModAST,
				...
				true ->
					...
					%io:format("~p~n", [{CurNode, RecvNode, RecvNodePath, Graph}]),
					NewAST = insertgraphnodechild(setgraphpathchild(AST, timeout, true, RecvNode, {call,0,{'fun',0,{clauses,get_clean_ast_all(element(4, Glob), 'receive', 3, lists:member(stubfuncs, element(5, Glob)))}},[getgraphpathchild(AST, timeout, true, RecvNode, lists:member(otp20, element(5, Glob))), getmemd(getgraphpath(AST, RecvNode), element(3, Val)), {'fun',0,[{clauses,[{clause,0,[],[],[]}]}]}]}, lists:member(otp20, element(5, Glob))),
						RecvNode, 'receive', false, getgraphpath(AST, RecvNode), NewNode, lists:member(otp20, element(5, Glob))),
					decompile_step({tl(InstList), NewAST,
						add_edge(CurNode, 3,
							add_edge(RecvNode, NewNode, Graph, NewAST, false), NewAST, false),
						NodeState, LabelToNode, NewNode, VarPrefix, AssignedVars,
				...
				test -> LblIdx = element(2, element(3, Val)) - element(1, Glob) + 1, NotExists = element(2, element(3, Val)) < element(1, Glob) orelse case LabelToNode of #{LblIdx := _} -> false; _ -> true end,
					...
					{'case',0,case element(2, Val) of
						...
						bs_start_match2 -> case getmemd(Cur, hd(element(4, Val))) of {_, _} -> {atom,0,true}; _ -> {call,0,{remote,0,{atom,0,erlang},{atom,0,'is_bitstring'}},[getmemd(Cur, hd(element(4, Val)))]} end;
						...
						bs_match_string -> {op,0,'andalso',{op,0,'>=',{call,0,{remote,0,{atom,0,erlang},{atom,0,'bit_size'}},[hd(element(3, getmemd(Cur, hd(element(4, Val)))))]},{integer,0,lists:nth(2, element(4, Val))}}, {op,0,'=:=',{call,0,{'fun',0,{clauses,[get_clean_ast(element(4, Glob), get_bits, 2, lists:member(stubfuncs, element(5, Glob)))]}},[hd(element(3, getmemd(Cur, hd(element(4, Val))))), {integer,0,lists:nth(2, element(4, Val))}]},getliteral(lists:nth(3, element(4, Val)))}}
							...
							CurNode, test, true, {graphdata, 0, element(3, NewCur), element(4, NewCur), element(5, NewCur)}, NextNode, lists:member(otp20, element(5, Glob))),
							CurNode, test, false, {graphdata, 0, element(3, Cur), element(4, Cur), element(5, Cur)}, NextNode + 1, lists:member(otp20, element(5, Glob))), %should pass all function arguments as a list to second parameter of error function
				...
				put_tuple2 -> decompile_step({
					tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(2, Val),
						{tuple, 0, [getmemd(Cur, X) || X <- element(2, element(3, Val))]})),
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				bs_get_tail -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(3, Val),
						hd(element(3, getmemd(Cur, element(2, Val)))))), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				bs_start_match3 -> decompile_step({[{test,bs_start_match2,element(2, Val),[element(3, Val), element(4, Val), {integer, 0}, element(5, Val)]}|tl(InstList)],
					AST, Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				bs_get_position -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(3, Val),
						getmemd(Cur, element(2, Val)))), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				bs_set_position -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(3, Val),
						getmemd(Cur, element(2, Val)))), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				swap -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(setmemd(Cur, element(2, Val),
						getmemd(Cur, element(3, Val))), element(3, Val),
						getmemd(Cur, element(2, Val)))), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				bs_start_match4 ->
					if element(2, element(2, Val)) =:= no_fail -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(5, Val), {tuple, 0, [getmemd(Cur, element(4, Val)), getmemd(Cur, element(4, Val))]})), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
					%or element(2, element(2, Val)) =:= resume -> ;
					true -> decompile_step({[{test,bs_start_match2,element(2, Val),[element(4, Val), element(3, Val), {integer, 0}, element(5, Val)]}|tl(InstList)],
					AST, Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob) end;
				...
				fconv -> decompile_step({tl(InstList),
					setgraphpath(AST, CurNode, setmemd(Cur, element(3, Val),
						getmemd(Cur, element(2, Val)))), Graph, NodeState, LabelToNode,
					CurNode, VarPrefix, AssignedVars, false, Stats}, Glob);
				get_tuple_element -> decompile_step({tl(InstList), %compiler should guarantee an is_tuple and tuple_size check so this equivalent is valid, since tuple size is not specified in the opcode
					...
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				set_tuple_element -> decompile_step({tl(InstList), %compiler should guarantee an is_tuple and tuple_size check so this equivalent is valid, since tuple size is not specified in the opcode
					setgraphpath(AST, CurNode, setmemd(Cur, element(3, Val),
				...
				put_tuple -> decompile_step({
					...
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				get_list -> decompile_step({tl(InstList), setgraphpath(AST, CurNode, %this should use pattern match format as badarg could be thrown instead of badmatch
				...
				put_list -> decompile_step({tl(InstList), setgraphpath(AST, CurNode,
					...
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				get_hd -> decompile_step({tl(InstList), setgraphpath(AST, CurNode, %this should use pattern match format as badarg could be thrown instead of badmatch
					setmemd(Cur, element(3, Val),
						{call,0,{remote,0,{atom,0,erlang},{atom,0,hd}},
							[getmemd(Cur, element(2, Val))]})),
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				get_tl -> decompile_step({tl(InstList), setgraphpath(AST, CurNode, %this should use pattern match format as badarg could be thrown instead of badmatch
					setmemd(Cur, element(3, Val),
						{call,0,{remote,0,{atom,0,erlang},{atom,0,tl}},
							[getmemd(Cur, element(2, Val))]})),
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
				...
				bs_context_to_binary -> decompile_step({tl(InstList), case getmemd(Cur, element(2, Val)) of {tuple,_,[_,_]} -> setgraphpath(AST, CurNode,
					...
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars,
					false, Stats}, Glob);
				get_map_elements -> Binds = lists:foldl(fun(El, {Acc, Next, AccAST, AccVar}) -> if Next =:= [] -> {Acc, [El], AccAST, AccVar}; true -> Nx = getmemd(Cur, hd(Next)),
				  case hd(Next) of {var, _, _} -> {[{map_field_exact,0,Nx,{var,0,'_'}}|Acc], [], AccAST, AccVar};
				  {X, _} when X =:= atom; X =:= integer; X =:= float; X =:= literal -> {[{map_field_exact,0,Nx,{var,0,'_'}}|Acc], [], AccAST, AccVar};
				  nil -> {[{map_field_exact,0,Nx,{var,0,'_'}}|Acc], [], AccAST, AccVar};
				  _ -> erlang:display(hd(Next)), NewVar = {var, 0, list_to_atom(VarPrefix ++ "Var" ++ integer_to_list(AssignedVars + AccVar))},
				    {[{map_field_exact,0,NewVar,{var,0,'_'}}|Acc], [], insertgraphnode(AccAST, CurNode,
							{match,0, NewVar, Nx}, setmemd(Cur, hd(Next), NewVar), CurNode), AccVar + 1} end end end,
						{[], [], AST, 0}, getmemd(Cur, element(4, Val))),
				  decompile_step({tl(InstList), insertgraphnode(element(3, Binds), CurNode,
					{match,0,{map,0,element(1, Binds)}, getmemd(Cur, element(3, Val))},
					%this semantic equivalence is wrong without checking first - badkey not badmatch is the error of maps:get - really a set andalso of maps:is_key operations followed by maps:gets otherwise error with a badmatch is necessary - but the badmatch context is not even here so best use map matching immediately
					element(1, lists:foldl(fun(El, {Acc, Next}) -> if Next =:= [] -> {Acc, [getmemd(Cur, El)]}; true -> {setmemd(Acc, El, {call,0,{remote,0,{atom,0,maps},{atom,0,get}},[hd(Next),getmemd(Cur, element(3, Val))]}), []} end end, {Cur, []}, getmemd(Cur, element(4, Val)))), CurNode),
					Graph, NodeState, LabelToNode, CurNode, VarPrefix, AssignedVars + element(4, Binds),
					false, Stats}, Glob);
				...
				bif -> decompile_step({tl(InstList),
					...
					'is_map' -> setmemd(Cur, element(5, Val),
						{call,0,{remote,0,{atom,0,erlang},{atom,0,'is_map'}},
							[getmemd(Cur, hd(element(4, Val)))]});
					'is_map_key' -> setmemd(Cur, element(5, Val), 
					  {call,0,{remote,0,{atom,0,maps},{atom,0,'is_key'}},
					  [getmemd(Cur, hd(element(4, Val))),
								getmemd(Cur, lists:nth(2, element(4, Val)))]});
					'map_get' -> setmemd(Cur, element(5, Val), 
				...
				call -> MakeNamedFun = not lists:member({element(2, element(3, Val)),
					...
					{{named_fun,0,list_to_atom(case hd(lists:nth(2, FunNum)) of Nme when Nme >= $A, Nme =< $Z; Nme =:= $_ -> hd(string:split(lists:nth(2, FunNum), "\/")); _ -> "F" ++
				...
				call_only -> MakeNamedFun = not lists:member({element(2, element(3, Val)), element(3, element(3, Val))}, element(2, Glob)) andalso
					...
					{FunAST, CStats} = if MakeNamedFun -> {DecAST, InStats, _} = dodecompileast(element(3, Glob), element(2, element(3, Val)), element(3, element(3, Val)), case hd(lists:nth(2, FunNum)) of Nm when Nm >= $A, Nm =< $Z; Nm =:= $_ -> hd(string:split(lists:nth(2, FunNum), "\/")); _ -> "F" ++ lists:nth(1, FunNum) ++ "_" ++ lists:nth(3, FunNum) ++ "_" ++ VarPrefix end, [], element(4, Glob), element(2, Glob), element(5, Glob)),
						{{named_fun,0,list_to_atom(case hd(lists:nth(2, FunNum)) of Nme when Nme >= $A, Nme =< $Z; Nme =:= $_ -> hd(string:split(lists:nth(2, FunNum), "\/")); _ -> "F" ++ lists:nth(1, FunNum) ++ "_" ++ lists:nth(3, FunNum) end),element(5, DecAST)}, InStats};
						true -> {if FunNum =/= [] -> {var,0,list_to_atom(case hd(lists:nth(2, FunNum)) of Nm when Nm >= $A, Nm =< $Z; Nm =:= $_ -> hd(string:split(lists:nth(2, FunNum), "\/")); _ -> "F" ++ lists:nth(1, FunNum) ++ "_" ++ lists:nth(3, FunNum) end)}; true -> {atom,0,element(2, element(3, Val))} end, #{}} end,
					GenAST = {call,0,FunAST,[getmemd(Cur, {x, X}) || X <- lists:seq(0, element(3, element(3, Val)) - 1)]},
				...
				make_fun2 -> FunNum = string:split(lists:nth(2, string:split(atom_to_list(element(2, element(2, Val))), "/")), "-", all), %-FUNCNAME/ARITY-fun-Num- %-fun.FUNCNAME/ARITY-
					...
					{DecAST, InStats, _} = dodecompileast(if element(2, element(3, Glob)) =:= element(1, element(2, Val)) -> element(3, Glob); true -> atom_to_list(element(1, element(2, Val))) end, element(2, element(2, Val)), element(3, element(2, Val)), "F" ++ lists:nth(1, FunNum) ++ "_" ++ if length(FunNum) == 2 -> ""; true -> lists:nth(3, FunNum) ++ "_" end ++ VarPrefix, lists:sublist(array:to_list(element(3, hd(NewCur))), 1, element(5, Val)), element(4, Glob), element(2, Glob), element(5, Glob)),
...
asthassideeffect(AST) ->
	element(2, traverse_ast(fun (Elem, Acc) ->
	{Elem, case Elem of
	{call,_,{remote,_,{atom,_,A},{atom,_,B}},C} -> Acc orelse hassideeffect(A, B, length(C));
	_ -> Acc end} end, false, AST)).
...
min_cleanup_ast(AST, VarPrefix, AssignedVars) ->
	EmptyClauseAST = [element(1, traverse_ast(fun (Elem, Acc) ->
		{case Elem of 
		{clause,L,A,B,[]} -> {clause,L,A,B,[{atom,0,true}]};
		{'receive',L,A,B,[]} -> {'receive',L,A,B,[{atom,0,true}]};
		_ -> Elem end, Acc} end, [], hd(AST)))],
	%this is too simple to handle mutually exclusive situations, it only gets rid of the most basic ones and does not eliminate side effect free expressions as well
	%VarMap = element(2, traverse_ast(fun (Elem, Acc) ->
	%	{Elem, case Elem of
	%	{var,L,X} when X =/= '_' -> case Acc of #{X := V} -> Acc#{X := V+1}; _ -> Acc#{X => 1} end;
	%	_ -> Acc end} end, #{}, hd(EmptyClauseAST))),
	%this does not work if the dead variables are in different scopes!
	%DeadVarElim = [element(1, traverse_ast(fun (Elem, Acc) -> %compiler warning about unused variable
	%	case Elem of
	%	{clause,L,Args,Guards,Code} -> {{clause,L,lists:map(fun (El) -> case El of {var,L,Name} -> case VarMap of #{Name := 1} -> {var,L,list_to_atom([$_|atom_to_list(Name)])}; _ -> El end; _ -> El end end, Args),Guards,Code}, Acc};
	%	{match,L,{var,L,X},D} -> {case VarMap of #{X := 1} -> D; %dead variable
	%		_ -> Elem end, Acc};
	%	_ -> {Elem, Acc} end end, [], hd(EmptyClauseAST)))],
	%unsafe variable elimination - when variables are used outside of a case/if block which has exit paths due to structural accuracy of decompiler
	%undefined variable elimination - when variables are used outside of a try/receive block due to state leak in decompiler
	%collapse the AST while tracking initial variable assignments with their level continuing to increase, a minimum scope level, and usages whose level is always considered to be the current one
	%since only if an assignment and usage occuring in the same collapsed level or clause block would be legal
	%and sibling blocks or higher blocks would then always show at a different level
	%{varname, isassign, level, minlevel, isused}
	%selections/mutually exclusive structures include: case, if, receive, tryof.../catch... (also not yet important but later identified multiple clause functions, list and binary comprehensions)
	%fortunately case/if have multiple clauses, while functions and begin blocks cannot yet by decompiler structuring
	%simple combine mutually exclusive elements, otherwise use scopelists to appropriately analyze them while merging
	%scope structures include: catch block, 3 blocks of try/catch, receive blocks, case/if with structural exits, begin?
	%the order of evaluation semantics e.g. left-to-right or otherwise needs to be very certainly defined!
	ScopeMap = element(1, traverse_ast(fun (Elem, Acc) ->
		{case Elem of 
		{var,_L,A} when A =/= '_' -> #{A => {false, 0, 0, false}};
		{cons,_L,A,B} -> scopelists([A, B], true);
		{lc,_L,A,B} -> scopelists([scopelists(B, false), A], true);
		{bc,_L,A,B} -> scopelists([scopelists(B, false), A], true);
		{tuple,_L,A} -> scopelists(A, true);
		{map,_L,A} -> scopelists(A, true);
		{map,_L,A,B} -> scopelists([A, scopelists(B, false)], true);
			{map_field_assoc,_L,A,B} -> scopelists([maps:map(fun (_, El) -> setelement(4, El, true) end, A), B], true);
			{map_field_exact,_L,A,B} -> scopelists([maps:map(fun (_, El) -> setelement(4, El, true) end, A), B], true);
		{call,_L,A,B} -> case case A of {remote,{atom,_,Mod},{atom,_,Func}} -> erl_bifs:is_exit_bif(Mod,Func,length(B)); _ -> false end of true -> #{}; _ -> scopelists([A, scopelists(B, false)], true) end; %exit bifs clear scope...
		%everything in A can be a potential binding, except keys in map expressions!!!
		{match,_L,A,B} -> scopelists([B, maps:map(fun (_, El) -> if element(4, El) =:= false -> setelement(1, El, true); true -> El end end, A)], false); %Parsing rules only (not execution): = Right associative
		{op,_L,_A,B} -> B; % ++ -- (Right associative)
		{op,_L,_A,B,C} -> scopelists([B, C], true); %andalso and orelse have special execution semantics!!!
		%Parsing rules only (not execution): / * div rem band and (Left associative) + - bor bxor bsl bsr or xor (Left associative) (! Right associative)
		{bin,_L,A} -> scopelists(A, true);
			{bin_element,_L,A,B,_C} ->
				if B =:= default -> A; true -> scopelists([A,B], true) end;
		{remote,_L,A,B} -> scopelists([A, B], true);
		%A of clause is an implicit match where function arguments or other variables can be bound
		{clause,_L,A,B,C} -> scopelists([maps:map(fun (_, El) -> setelement(1, El, true) end, scopelists(A, false)), scopelists(lists:map(fun (El) -> scopelists(El, false) end, B), false), scopelists(C, false)], false); %in case/if/try of/try-catch/receive/fun args/function args mutually exclusive, for begin...end block, is not but moot point with only single clause
		{function,_L,_A,_B,C} -> scopedup(C);
		
		{block,_L,A} -> scopelists(A, true);
		{'if',_L,A} -> scopedup(A);
		{'case',_L,A,B} -> scopelists([A,scopedup(B)], true);
		{'try',_L,A,B,C,D} -> scopelists([scopelists(A, false), scopedup([scopedup(B), scopedup(C)]), scopelists(D, false)], true);
		{'receive',_L,A} -> scopedup(A);
		{'receive',_L,A,B,C} -> scopelists([B, scopedup([scopedup(A), scopelists(C, false)])], false);
		{'fun',_L,{clauses,A}} -> scopedup(A);
		{'fun',_L,{function,A,B}} -> scopelists([A, B], false);
		{named_fun,_L,A,B} -> if A =/= '_' -> scopelists([#{A => {true, 0, 0, false}}, scopedup(B)], false); true -> scopedup(B) end;
		{'catch',_L,A} -> A;
		_ -> #{} end, Acc} end, #{}, hd(EmptyClauseAST))),
	{_NewAssignedVars, ScopeRemap} = maps:fold(fun (Nm, {IA, L, ML, IU}, {AV, SM}) ->
	  case ML of 0 -> {AV, SM}; _ ->
	    {AV+L, lists:foldl(fun (Lvls, AccSM) -> AccSM#{if Lvls == ML -> Nm; true -> list_to_atom(VarPrefix ++ "Var" ++ integer_to_list(AV+ML-1-Lvls)) end => {IA, L, Lvls, IU,
	      {var,0,list_to_atom(VarPrefix ++ "Var" ++ integer_to_list(AV+ML-Lvls))}, [Nm|[list_to_atom(VarPrefix ++ "Var" ++ integer_to_list(AV+ML-1-X)) || X <- lists:seq(ML-1, Lvls, -1)]]}} end, SM, lists:seq(ML, 1, -1))} end end, {AssignedVars, ScopeMap}, ScopeMap),
	erlang:display({_NewAssignedVars, ScopeRemap}),
	RescopeAST = [element(1, traverse_ast(fun (Elem, Acc) ->
	  case Elem of
	  {var,L,X} -> %{{var,L,X}, Acc};
	  erlang:display({X, Acc}),
	    { case Acc of #{X := Y} -> {var,L,Y}; _ -> {var,L,X} end, Acc };
	  {match,L,{var,L,X},B} -> case ScopeRemap of #{X := {_, _, _, _, _, _}} ->
	  	%AllPrev = fun RecPrev(Els, E) -> case ScopeRemap of #{E := {_, _, _, _, {var,_,El}, _}} -> RecPrev([El|Els], El); _ -> Els end end([], X),
	  	erlang:display({X, B, Acc}),
	    case maps:fold(fun (K, V, Found) -> if V =:= X -> K; true -> Found end end, false, Acc) of false -> {{true, [{var,L,X}], {match,L,{var,L,X},B}}, Acc}; Kv -> {{true, [{var,L,Kv}], {match,L,{var,L,Kv},B}},
	      maps:remove(Kv, Acc)} end;
	    _ -> {{match,L,{var,L,X},B}, Acc} end; %assuming for now based on decompiler logic that matches only occur in clause, but in map keys this is not true!
	  {clause,L,A,B,C} -> {Rescp, Fin} = lists:foldl(fun (El, {Aa, Ac}) -> case element(1, El) of true -> {element(2, El) ++ Aa, [element(3, El)|Ac]}; _ -> {Aa, [El|Ac]} end end, {[], []}, C),
	  %erlang:display({Rescp, Fin}),
	    case Rescp of [] -> {{clause,L,A,B,C}, Acc}; _ -> {{true, Rescp, {clause,L,A,B,lists:reverse(Fin) ++ [case length(Rescp) of 1 -> hd(Rescp); _ -> {tuple,L,Rescp} end]}}, Acc} end;
	  {function,L,A,B,C} -> {_Rescp, _Fin} = lists:foldl(fun (El, {Aa, Ac}) -> case element(1, El) of true -> {element(2, El) ++ Aa, [element(3, El)|Ac]}; _ -> {Aa, [El|Ac]} end end, {[], []}, C),
	    {{function,L,A,B,C}, Acc};
	  {block,L,A} -> {{block,L,lists:map(fun (El) -> case element(1, El) of true -> element(3, El); _ -> El end end, A)}, Acc}; %catch though singular in its expression defaults safely to always wrapping the content in a begin end block
	  {'if',L,A} -> {{'if',L,A}, Acc};
	  {'case',L,A,B} -> {RescpInit, Fin} = lists:foldl(fun (El, {Aa, Ac}) -> case element(1, El) of true -> {element(2, El) ++ Aa, [element(3, El)|Ac]}; _ -> {Aa, [El|Ac]} end end, {[], []}, B),
	  %erlang:display({RescpInit, Fin}),
	  	%must eliminate any lower levels which are consolidated by higher levels, and obviously
	  	Rescp = lists:filtermap(fun ({var,_,El}) -> AllPrev = fun RecPrev(Els, E) -> case ScopeRemap of #{E := {_, _, _, _, {var,_,X}, _}} -> RecPrev([X|Els], X); _ -> Els end end([], El),
	  		erlang:display({El, AllPrev}), case lists:any(fun (E) -> lists:keyfind(E, 3, RescpInit) =/= false end, AllPrev) of true -> false; _ -> true end end, RescpInit),
	  	case Rescp of [] -> {{'case',L,A,B}, Acc}; _ -> NxtRescp = lists:filtermap(fun ({var,_,El}) -> Nme = element(3, element(5, begin #{El := Ele} = ScopeRemap, Ele end)), case ScopeRemap of #{Nme := {_, _, _, _, _, _}} -> {true, {var,L,Nme}}; _ -> false end end, Rescp),
	  	erlang:display({NxtRescp, Rescp, Acc}),
	  	Mtch = {match,L,case length(Rescp) of 1 -> {var,_,El} = hd(Rescp), element(5, begin #{El := Elm} = ScopeRemap, Elm end); _ -> {tuple,L,lists:map(fun ({var,_,El}) -> element(5, begin #{El := Ele} = ScopeRemap, Ele end) end, Rescp)} end,{'case',L,A,lists:reverse(Fin)}},
	  	{ case NxtRescp of [] -> Mtch; _ -> {true, NxtRescp, Mtch} end,
	  	  lists:foldl(fun ({var,_,El}, AccMap) -> N = element(3, element(5, begin #{El := Ele} = ScopeRemap, Ele end)),
	  	  	AllPrev = fun RecPrev(Els, E) -> case ScopeRemap of #{E := {_, _, _, _, {var,_,X}, _}} -> RecPrev([X|Els], X); _ -> Els end end([], El),
	  	  	_Repl = lists:dropwhile(fun (E) -> lists:all(fun ({var,_,Ee}) -> Ee =/= E end, Rescp) end, AllPrev),
	  	  	%erlang:display({El, N, AllPrev, Repl, begin #{El := ElSel} = ScopeRemap, ElSel end}),
	  	  	case element(6, begin #{El := ElSelf} = ScopeRemap, ElSelf end) of [El] -> case AllPrev of [_] -> AccMap#{El => N}; _ -> AccMap end;
	  	  	_ -> case AllPrev of [_] -> AccMap#{El => N, hd(element(6, begin #{El := ElSelf} = ScopeRemap, ElSelf end)) => N}; _ -> AccMap#{El => N} end end
	  	  	%case Repl =/= [] andalso lists:member({var,L,hd(Repl)}, RescpInit) of true -> AccMap; _ ->
	  	  	%  lists:foldl(fun (E, A) -> case A of #{E := _} -> A#{E := N};
	  	  	%    _ -> case maps:fold(fun (K, V, Found) -> if V =:= E -> K; true -> Found end end, false, A) of false -> A#{E => N}; Kv -> A#{Kv := case Repl of [] -> N; [Hd|_] -> Hd end} end end end, AccMap, [El|element(6, begin #{El := Ele} = ScopeRemap, Ele end)]) end
	  	  	end, Acc, RescpInit)} end;
	  {'try',L,A,B,C,D} -> {{'try',L,lists:map(fun (El) -> case element(1, El) of true -> element(3, El); _ -> El end end, A),B,C,D}, Acc};
	  {'receive',L,A} -> {{'receive',L,A}, Acc};
	  {'receive',L,A,B,C} -> {{'receive',L,A,B,C}, Acc};
	  {'fun',L,{clauses,A}} -> {{'fun',L,{clauses,A}}, Acc};
	  {named_fun,L,A,B} -> {{named_fun,L,A,B}, Acc};
	  _ -> {Elem, Acc} end end, #{}, hd(EmptyClauseAST)))],
	erlang:display({RescopeAST, ScopeMap}), %the minimums used to be NestDepthRaise = (MinLvl / 2) - 1, because case/clause combination, and match and outer clause are counted, but now depth is properly dealt with on a AST construct basis
	DeadVarElim = [element(1, traverse_ast(fun (Elem, Acc) -> %compiler warning about unused variable
		case Elem of
		{clause,L,Args,Guards,Code} -> {{clause,L,lists:map(fun (El) -> case El of {var,L,Name} -> case ScopeMap of #{Name := {_, _, _, false}} -> {var,L,list_to_atom([$_|atom_to_list(Name)])}; _ -> El end; _ -> El end end, Args),Guards,Code}, Acc};
		{match,L,{var,L,X},D} -> {case ScopeMap of #{X := {_, _, _, false}} -> D; %dead variable
			_ -> Elem end, Acc};
		{cons,L,A,B} -> {{cons,L,case A of {var,L,X} -> case ScopeMap of #{X := {_, _, _, false}} -> {var,L,'_'}; _ -> A end; _ -> A end,
			case B of {var,L,X} -> case ScopeMap of #{X := {_, _, _, false}} -> {var,L,'_'}; _ -> B end; _ -> B end}, Acc}; %list pattern match with dead variable
		{tuple,L,A} -> {{tuple,L,lists:map(fun (El) -> case El of {var,L,X} -> case ScopeMap of #{X := {_, _, _, false}} -> {var,L,'_'}; _ -> El end; _ -> El end end, A)}, Acc}; %tuple pattern match with dead variable
		{named_fun,L,A,B} -> {case ScopeMap of #{A := {_, _, _, false}} -> {'fun',L,{clauses,B}}; _ -> {named_fun,L,A,B} end, Acc}; %named_fun without its name referenced
		_ -> {Elem, Acc} end end, [], hd(RescopeAST)))],
  DeadVarElim,RescopeAST,EmptyClauseAST
.

scopedup([]) -> #{};
scopedup([Hd]) -> Hd;
scopedup(Lst) -> Out = lists:foldl(fun (El, Acc) -> %find in Acc but update A, since if in prior group, its out of scope, in same group is in scope
  maps:fold(fun (Name, E, A) -> {IsAssign, Lvl, _, _} = E,
    Tail = case Acc of #{Name := {_, L, _ ,_}} when IsAssign, Lvl >= L -> maps:remove(Name, A); _ -> A end,
    case case Acc of #{Name := {_, Lv, _, _}} -> Lvl < Lv orelse not IsAssign; _ -> false end of true -> Tail; _ -> Tail#{Name => E} end end, Acc, El)
  end, hd(Lst), tl(Lst)), erlang:display({Lst, Out}), Out.
scopelists([], _	) -> #{};
scopelists([Hd], _	) -> Hd;
scopelists(Lst, AddDepth) -> Out = lists:foldl(fun (El, Acc) -> %find in Acc but update A, since if in prior group, its out of scope, in same group is in scope
  maps:fold(fun (Name, E, A) -> {IsAssign, Lvl, MinLvl, IsUsed} = E,
    Tail = case Acc of #{Name := {true, L, _M, _}} when L =/= 0 -> %erlang:display({E, {true, L, M}}),
        A#{Name := {true, if IsAssign -> min(Lvl, L); true -> L end, if IsAssign -> min(Lvl, L); true -> L end, not IsAssign}};
    	#{Name := {true, Lv, Min, _}} when Lv =:= 0 -> %erlang:display({E, {true, Lv, Min}}),
    	  A#{Name := {true, Lv, Min, not IsAssign}};
    	{false, _, _, _} when IsAssign -> maps:remove(Name, A); _ -> A end,
    case Acc of #{Name := _} -> Tail; _ -> Tail#{Name => case IsAssign of true -> {true, Lvl+ if AddDepth -> 1; true -> 0 end, MinLvl, IsUsed orelse case Acc of #{Name := {Asgn, _, _, _}} -> Asgn =:= false; _ -> false end}; _ -> E end} end end, Acc, El)
  end, hd(Lst), tl(Lst)), Out.
...
checkbinmatch(BAST, CAST, CleanAST, SubOut) ->
  case BAST of [] -> {[], [], [], [], {[], []}};
	_ -> Last = case lists:last(BAST) of 
	{'case',L,Ai,[{clause,L,[{atom,L,true}],[],Bi},{clause,L,[{atom,L,false}],[],Ci}]} ->
 		isbinary(Ai, Bi, Ci, CleanAST, SubOut);
 	{'if',L,[{clause,L,[],[[Ai]],Bi},{clause,L,[],[[{atom,L,true}]],Ci}]} -> 
 		isbinary(Ai, Bi, Ci, CleanAST, SubOut);
 	{'case',L,Ai,[{clause,L,[Ae],[],Bi},{clause,L,[{var,L,'_'}],[],Ci}]} ->
 		isbinary({op,L,'=:=',Ai,Ae}, Bi, Ci, CleanAST, SubOut);
 	Bl -> case isbinarysequence(Bl, [], [], CleanAST, SubOut) of false -> false; X -> X end end,
 	%guard expression not accepted here or in following code - only in boolean condition is it accepted!
 	case Last of false -> Ret = lists:foldl(fun (El, Acc) -> case element(5, Acc) =:= {[],[]} andalso case isbinarysequence(El, [], [], CleanAST, SubOut) of false -> false; {[], _, _, _, _} -> false; _ -> true end of false -> setelement(5, Acc, {element(1, element(5, Acc)) ++ [El], CAST}); Xe -> setelement(1, Acc, element(1, Acc) ++ element(1, Xe)) end end, {[], [], [], [], {[], []}}, lists:droplast(BAST)), setelement(5, Ret, {element(1, element(5, Ret)) ++ [lists:last(BAST)], CAST});
 		{[], _, _, _, _} -> Ret = lists:foldl(fun (El, Acc) -> case element(5, Acc) =:= {[],[]} andalso case isbinarysequence(El, [], [], CleanAST, SubOut) of false -> false; {[], _, _, _, _} -> false; _ -> true end of false -> setelement(5, Acc, {element(1, element(5, Acc)) ++ [El], CAST}); Xe -> setelement(1, Acc, element(1, Acc) ++ element(1, Xe)) end end, {[], [], [], [], {[], []}}, lists:droplast(BAST)), setelement(5, Ret, {element(1, element(5, Ret)) ++ [lists:last(BAST)], CAST});
 	_ -> lists:foldl(fun (El, Acc) -> setelement(1, Acc, element(1, Acc) ++ element(1, isbinarysequence(El, [], [], CleanAST, SubOut))) end, Last, lists:droplast(BAST)) end end.

isbinary(AST, BAST, CAST, CleanAST, SubOut) ->
	case isbinarypattern(AST, BAST, CAST, CleanAST, SubOut) of BinPat when BinPat =/= false, element(1, BinPat) =/= [] -> erlang:display(BinPat),
		BinPat;
	_ ->
	%case case BAST of [] -> []; [_] -> []; _ -> lists:last(BAST) end of
	%{'if',L,[{clause,L,[],[[Cond]],Next},{clause,L,[],[[{atom,L,true}]],NoMatchCode}]} ->
  %	case isbinarysequence(AST, lists:last(BAST), CleanAST, SubOut) of false -> false; Elems -> Elems end;
  %{'case',L,Cond,[{clause,L,[{atom,L,true}],[],Next},{clause,L,[{atom,L,false}],[],NoMatchCode}]} ->
  %	case isbinarysequence(AST, lists:last(BAST), CleanAST, SubOut) of false -> false; Elems -> Elems end;
  %{'case',L,A,[{clause,L,[B],[],Next},{clause,L,[{var,L,'_'}],[],NoMatchCode}]} ->
    case isbinarysequence(AST, BAST, CAST, CleanAST, SubOut) of false -> {[], [], [], [], {BAST, CAST}}; {[], _, _, _, _} -> {[], [], [], [], {BAST, CAST}}; Elems -> Elems end %setelement(5, Elems, {BAST, CAST}) end
  end
.

isbinarysequence(AST, BAST, CAST, CleanAST, SubOut) ->
	GetBits = get_clean_ast(CleanAST, get_bits, 2, SubOut),
	SkipBits = get_clean_ast(CleanAST, skip_bits, 2, SubOut),
	HasFloat = get_clean_ast(CleanAST, has_float, 3, SubOut),
	HasUtf8 = get_clean_ast(CleanAST, has_utf8, 1, SubOut),
	GetUtf8Size = get_clean_ast(CleanAST, get_utf8_size, 1, SubOut),
	HasUtf16 = get_clean_ast(CleanAST, has_utf16, 2, SubOut),
	GetUtf16Size = get_clean_ast(CleanAST, get_utf16_size, 2, SubOut),
	HasUtf32 = get_clean_ast(CleanAST, has_utf32, 2, SubOut),
  case AST of
	{op,L,'=:=',{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,Bs]},B} ->
		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> 
			setelement(1, Elems, [{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,Bs]}, B}]) end;

	{op,L,'=:=',{op,L,'rem',{call,L,{remote,L,{atom,L,erlang},{atom,L,bit_size}},
		[B]},{integer,L,C}},{integer,L,0}} -> %bs_get_binary2 all
		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems ->
 			setelement(1, Elems, [{B,{bin,L,[{bin_element,L,{var,L,'_'},default,[bitstring,{unit,C}]}]}}]) end;
 	{op,L,'=:=',{call,L,{remote,L,{atom,L,erlang},{atom,L,'bit_size'}},[B]},
 		{integer,L,C}} -> %bs_test_tail2
 	io:format("bs_test_tail2~n"),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> %terminating sequence - no need to correlate further but can have a case true false clause
 			setelement(1, Elems, [{B, {bin,L,[{bin_element,L,{var,L,'_'},{integer,L,C},[bitstring]}]}}|element(1, Elems)]) end;
 	{op,L,'andalso',{atom,L,true},A} ->
 		isbinarysequence(A, BAST, CAST, CleanAST, SubOut); %bs_start_match2 only for comprehensions!
 	{op,L,'>=',{call,L,{remote,L,{atom,L,erlang},{atom,L,'bit_size'}},[B]},
 		{op,L,'*',{integer,L,C},{integer,L,D}}} -> %bs_get_integer2
	io:format("bs_get_integer2~n"),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> %erlang:display(Elems),
 			case element(1, Elems) of [] -> false; _ -> case hd(element(1, Elems)) of
 				{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{op,L,'*',{integer,L,_C},{integer,L,_D}}]}, {bin,L,F}} when B =:= Bin -> setelement(1, Elems, [{Bin, {bin,L,[{bin_element,L,{var,L,'_'},{integer,L,C*D},[integer]}|F]}}|tl(element(1, Elems))]);
 				_ -> io:format("~p~n", [{length(element(1, Elems)),hd(element(1, Elems))}]), false end end end;
 	{'call',L,{'fun',L,{clauses,[HasFloat]}},[B, {op,L,'*',{integer,L,C},{integer,L,D}},_Endian]} -> %bs_get_float2
	io:format("bs_get_float2~n"), erlang:display(BAST),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
 			case element(1, Elems) of [] -> false; _ -> case hd(element(1, Elems)) of
 				{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{op,L,'*',{integer,L,_C},{integer,L,_D}}]}, {bin,L,F}} when B =:= Bin -> setelement(1, Elems, [{Bin, {bin,L,[{bin_element,L,{var,L,'_'},{integer,L,C*D},[float]}|F]}}|tl(element(1, Elems))]);
 				_ -> io:format("~p~n", [{length(element(1, Elems)),hd(element(1, Elems))}]), false end end end;
 	{'call',L,{'fun',L,{clauses,[HasUtf8]}},[B]} -> %bs_get_utf8, bs_skip_utf8
	io:format("bs_get_utf8~n"), erlang:display(BAST),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
 			case element(1, Elems) of [] -> false; _ -> case hd(element(1, Elems)) of
 				{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{op,L,'*',{integer,L,8},{'call',L,{'fun',L,{clauses,[GetUtf8Size]}},[B]}}]}, {bin,L,F}} when B =:= Bin -> setelement(1, Elems, [{Bin, {bin,L,[{bin_element,L,{var,L,'_'},default,[utf8]}|F]}}|tl(element(1, Elems))]);
 				_ -> io:format("~p~n", [{length(element(1, Elems)),hd(element(1, Elems))}]), false end end end;
 	{'call',L,{'fun',L,{clauses,[HasUtf16]}},[B,Endian]} -> %bs_get_utf16, bs_skip_utf16
	io:format("bs_get_utf16~n"), erlang:display(BAST),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
 			case element(1, Elems) of [] -> false; _ -> case hd(element(1, Elems)) of
 				{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{op,L,'*',{integer,L,8},{'call',L,{'fun',L,{clauses,[GetUtf16Size]}},[B, Endian]}}]}, {bin,L,F}} when B =:= Bin -> setelement(1, Elems, [{Bin, {bin,L,[{bin_element,L,{var,L,'_'},default,[utf16]}|F]}}|tl(element(1, Elems))]);
 				_ -> io:format("~p~n", [{length(element(1, Elems)),hd(element(1, Elems))}]), false end end end;
 	{'call',L,{'fun',L,{clauses,[HasUtf32]}},[B, _Endian]} -> %bs_get_utf32, bs_skip_utf32
	io:format("bs_get_utf32~n"), erlang:display(BAST),
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
 			case element(1, Elems) of [] -> false; _ -> case hd(element(1, Elems)) of
 				{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{integer,0,4*8}]}, {bin,L,F}} when B =:= Bin -> setelement(1, Elems, [{Bin, {bin,L,[{bin_element,L,{var,L,'_'},default,[utf32]}|F]}}|tl(element(1, Elems))]);
 				_ -> io:format("~p~n", [{length(element(1, Elems)),hd(element(1, Elems))}]), false end end end;
 	...
 	io:format("bs_match_string~n"), %bs_match_string
		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems ->
 		 	setelement(1, Elems, [{B, D}|element(1, Elems)]) end;
	{op,L,'=:=',{op,L,'rem',{call,L,{remote,L,{atom,L,erlang},
		{atom,L,'bit_size'}},[B]},{integer,L,C}},{integer,L,0}} -> %bs_skip_bits2, bs_get_binary
 	...
 	io:format("bs_skip_bits2 all~n"), %_ is length of C * D
		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
	 		setelement(1, Elems, [{B, {bin,L,[{bin_element,L,{var,L,'_'},{integer,L,C},[bitstring,{unit,D}]}]}}|element(1, Elems)]) end;
 	{op,L,'andalso',{op,L,'>=',{integer,L,C},{integer,L,0}},
 		{op,L,'>=',{call,L,{remote,L,{atom,L,erlang},{atom,L,bit_size}},[B]},
 			{op,L,'*',{integer,L,D},{integer,L,C}}}} -> %bs_skip_bits2 all
 	io:format("bs_skip_bits2 all~n"), %_ is length of C * D
		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display(Elems),
	 		setelement(1, Elems, [{B, {bin,L,[{bin_element,L,{var,L,'_'},{integer,L,D},[bitstring,{unit,C}]}]}}|element(1, Elems)]) end;
 	{op,_,'andalso',A,B} -> io:format("binary andalso~n"), %could be a guard for here or equality also, must save them somehow
 		case isbinarysequence(A, [], [], CleanAST, SubOut) of false -> false; Elems ->
 			case isbinarysequence(B, BAST, CAST, CleanAST, SubOut) of false -> false; Els ->
 			  {element(1, Elems) ++ element(1, Els), element(2, Elems) ++ element(2, Els), element(3, Elems) ++ element(3, Els), element(4, Elems) ++ element(4, Els), element(5, if element(5, Elems) == {[], []} -> Els; true -> Elems end)}
 					end end;
	%any already pattern matching that is correlated at the current level
 	{match,_L,B,A} ->
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> {[{A, B}], [], [], [], {[], []}}; Elems -> setelement(1, Elems, [{A, B}|element(1, Elems)]) end;
	%any strict equality must be after more specific ones
 	{op,_,'=:=',A,B} ->
 		case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> {[{A, B}], [], [], [], {[], []}}; Elems -> setelement(1, Elems, [{A, B}|element(1, Elems)]) end;
	_ -> case isguardexpr(AST) of true -> erlang:display(AST), {[], [], [], case AST of {atom,_,true} -> []; _ -> [AST] end, {[], []}};
 	_ -> io:format("~p~n", [AST]), false end
 	end
.

isbinarypattern(AST, BAST, CAST, CleanAST, SubOut) ->
%io:format("~p~n", [AST]),
	case AST of
	{call,L,{remote,L,{atom,L,erlang},{atom,L,is_bitstring}},[_A]} ->
	  case checkbinmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems ->
	    case lists:foldl(fun (El, {Acc, Asn}) ->
	  	  case Acc of %{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{integer,L,C}]}, F} -> [{{bin,L,case Bin of {bin,L,BE} -> BE ++ [F]; _ -> [Bin,F] end}, element(2, hd(Els))}];
	 				%{{call,L,{'fun',L,{clauses,[SkipBits]}},[Bin,{op,L,'*',{integer,L,_C},{integer,L,_D}}]}, {bin,L,F}} -> case El of {_H, {bin, L, G}} -> {Bin, {bin,L,F ++ G}}; _ -> io:format("~p~n", [{Acc, El}]), false end;
	 				{{var,L,E},{bin,L,F}} -> case El of {_H, {bin, L, G}} -> {{{var,L,E}, {bin,L, F ++ G}}, Asn}; Asgn -> {Acc, [Asgn|Asn]} end;
	 				%{{block,L,[{match,L,{bin,L,_},{bin,L,E}},{var,L,_}]}, {bin,L,F}} -> case El of {_H, {bin, L, G}} -> {{bin,L,E}, {bin,L, F ++ G}}; _ -> io:format("~p~n", [{Acc, El}]), false end;
	 				%{{bin,L,E}, {bin,L,F}} -> case El of {_H, {bin, L, G}} -> {{bin,L,E}, {bin,L, F ++ G}}; _ -> io:format("~p~n", [{Acc, El}]), false end;
	 				_ -> io:format("~p~n", [{Acc, El}]), false
	 			end end, {hd(element(1, Elems)), []}, tl(element(1, Elems))) of false -> false; Bin -> erlang:display(Bin), {[element(1, Bin)], element(2, Elems),
	 			  element(2, Bin) ++ element(3, Elems), element(4, Elems), element(5, Elems)} end end;
	{call,L,{remote,L,{atom,L,erlang},{atom,L,is_binary}},[_A]} ->
	  checkbinmatch(BAST, CAST, CleanAST, SubOut);
	_ -> false
	end.
	
checkmatch(BAST, CAST, CleanAST, SubOut) ->
	case BAST of [] -> {[], [], [], [], {[], []}};
	_ -> Last = case lists:last(BAST) of 
	{'case',L,Ai,[{clause,L,[{atom,L,true}],[],Bi},{clause,L,[{atom,L,false}],[],Ci}]} ->
 		ismatch(Ai, Bi, Ci, CleanAST, SubOut);
 	{'if',L,[{clause,L,[],[[Ai]],Bi},{clause,L,[],[[{atom,L,true}]],Ci}]} -> 
 		ismatch(Ai, Bi, Ci, CleanAST, SubOut);
 	{'case',L,Ai,[{clause,L,[Ae],[],Bi},{clause,L,[{var,L,'_'}],[],Ci}]} ->
 		ismatch({op,L,'=:=',Ai,Ae}, Bi, Ci, CleanAST, SubOut);
 	Bl -> case ispatternmatch(Bl, [], [], CleanAST, SubOut) of false -> false; X -> X end end,
 	case Last of false -> Ret = lists:foldl(fun (El, Acc) -> case element(5, Acc) =:= {[],[]} andalso case ispatternmatch(El, [], [], CleanAST, SubOut) of false -> false; {[], _, _, _, _} -> false; _ -> true end of false -> setelement(5, Acc, {element(1, element(5, Acc)) ++ [El], CAST}); Xe -> setelement(1, Acc, element(1, Acc) ++ element(1, Xe)) end end, {[], [], [], [], {[], []}}, lists:droplast(BAST)), setelement(5, Ret, {element(1, element(5, Ret)) ++ [lists:last(BAST)], CAST});
 		{[], _, _, _, _} -> Ret = lists:foldl(fun (El, Acc) -> case element(5, Acc) =:= {[],[]} andalso case ispatternmatch(El, [], [], CleanAST, SubOut) of false -> false; {[], _, _, _, _} -> false; _ -> true end of false -> setelement(5, Acc, {element(1, element(5, Acc)) ++ [El], CAST}); Xe -> setelement(1, Acc, element(1, Acc) ++ element(1, Xe)) end end, {[], [], [], [], {[], []}}, lists:droplast(BAST)), setelement(5, Ret, {element(1, element(5, Ret)) ++ [lists:last(BAST)], CAST});
 	_ -> lists:foldl(fun (El, Acc) -> setelement(1, Acc, element(1, Acc) ++ element(1, ispatternmatch(El, [], [], CleanAST, SubOut))) end, Last, lists:droplast(BAST)) end end.

ismatch(AST, BAST, CAST, CleanAST, SubOut) ->
erlang:display(BAST),
	case isbinarypattern(AST, BAST, CAST, CleanAST, SubOut) of BinPat when BinPat =/= false, element(1, BinPat) =/= [] -> erlang:display(BinPat),
		BinPat;
	_ -> %case case BAST of [] -> []; _ -> lists:last(BAST) end of
	%{'if',L,[{clause,L,[],[[Cond]],Next},{clause,L,[],[[{atom,L,true}]],NoMatchCode}]} ->
  %	case ispatternmatch(AST, lists:last(BAST), CleanAST, SubOut) of false -> false; Elems -> Elems end;
  %{'case',L,Cond,[{clause,L,[{atom,L,true}],[],Next},{clause,L,[{atom,L,false}],[],NoMatchCode}]} ->
  %	case ispatternmatch(AST, lists:last(BAST), CleanAST, SubOut) of false -> false; Elems -> Elems end;
  %{'case',L,A,[{clause,L,[B],[],Next},{clause,L,[{var,L,'_'}],[],NoMatchCode}]} ->
  %	case ispatternmatch(AST, lists:last(BAST), CleanAST, SubOut) of false -> false; Elems -> Elems end;
  %_ ->
  case ispatternmatch(AST, BAST, CAST, CleanAST, SubOut) of false -> {[], [], [], [], {BAST, CAST}}; {[], _, _, _, _} -> {[], [], [], [], {BAST, CAST}}; Elems -> Elems end %setelement(5, Elems, {BAST, CAST}) end
  end %end
.

ispatternmatch(AST, BAST, CAST, CleanAST, SubOut) -> %{[{Original, Pattern}|...}], NewVar, VarUse, Guards, {CaseMatch, CaseNoMatch}}
erlang:display(AST),
	%base cases: lists, tuples, maps, binaries, recursive constructions
	...
	%any already pattern matching that is correlated at the current level
 	{match,_L,B,A} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> {[{A, B}], [], [], [], {[], []}}; Elems -> setelement(1, Elems, [{A, B}|element(1, Elems)]) end;
	%any strict equality
 	{op,_,'=:=',A,B} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> {[{A, B}], [], [], [], {[], []}}; Elems -> setelement(1, Elems, [{A, B}|element(1, Elems)]) end;
 	%tuples
 	{call,L,{remote,L,{atom,L,erlang},{atom,L,is_tuple}},[_A]} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> Elems end;
 	{op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_tuple}},[A]},{op,L,'=:=',{call,L,{remote,L,{atom,L,erlang},{atom,L,tuple_size}},[A]},{integer,L,C}}} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> erlang:display({"here", Elems}), {Rest, Asgn} = lists:partition(fun({Elem,_}) -> case Elem of {call,L,{remote,L,{atom,L,erlang},{atom,L,element}},[{integer,L,_},A]} -> false; _ -> true end end, element(1, Elems)),
 			{[{A, {tuple,L,[case lists:keyfind({call,L,{remote,L,{atom,L,erlang},{atom,L,element}},[{integer,L,X},A]},1,element(1, Elems)) of {_,Pat} -> Pat; _ -> {var,L,'_'} end || X <- lists:seq(1, C)]}}|Rest],
 				element(2, Elems), Asgn ++ element(3, Elems), element(4, Elems), element(5, Elems)} end;
 	%lists
 	{op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_list}},[A]},{op,L,'=/=',A,{nil,L}}} -> erlang:display(BAST),
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> false;
 			Elems -> erlang:display(A), {Rest, Asgn} = lists:partition(fun({Elem,_}) -> case Elem of {call,L,{remote,L,{atom,L,erlang},{atom,L,hd}},[A]} -> false; {call,L,{remote,L,{atom,L,erlang},{atom,L,tl}},[A]} -> false; _ -> true end end, element(1, Elems)),
 			{[{A, {cons,L,case lists:keyfind({call,L,{remote,L,{atom,L,erlang},{atom,L,hd}},[A]},1,element(1, Elems)) of {_,Pat} -> Pat; _ -> {var,L,'_'} end,
 				case lists:keyfind({call,L,{remote,L,{atom,L,erlang},{atom,L,tl}},[A]},1,element(1, Elems)) of {_,Pat} -> Pat; _ -> {var,L,'_'} end}}|Rest],
 					element(2, Elems), Asgn ++ element(3, Elems), element(4, Elems), element(5, Elems)} end;
 	%maps 	
 	{op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},{call,L,{remote,L,{atom,L,maps},{atom,L,is_key}},[_,A]}} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> {[{AST, {atom,L,true}}], [], [], [], {[], []}}; Elems -> setelement(1, Elems, [{AST, {atom,L,true}}|element(1, Elems)]) end;
 	{op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},B} ->
 		case ispatternmatch(B, BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> case Elems of {[{A, {map,L,_}}], _, _, _, _} -> Elems; _ -> {Rest, Asgn} = lists:partition(fun({Elem,_}) -> case Elem of {op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},{call,L,{remote,L,{atom,L,maps},{atom,L,Kf}},[_,A]}} when Kf =:= is_key; Kf =:= get -> false; _ -> true end end, element(1, Elems)), erlang:display({Rest, Asgn}),
 			{[{A, {map,L,[begin {op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},{call,L,{remote,L,{atom,L,maps},{atom,L,_}},[K,A]}} = X, {map_field_exact,L,K,{var,L,'_'}} end || {X, _} <- Asgn]}}|Rest],
 				element(2, Elems), Asgn ++ element(3, Elems), element(4, Elems), element(5, Elems)} end end;
 	{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]} ->
 		case checkmatch(BAST, CAST, CleanAST, SubOut) of false -> false; Elems -> case Elems of {[{A, {map,L,_}}], _, _, _, _} -> erlang:display({"here", Elems}), Elems; _ -> {Rest, Asgn} = lists:partition(fun({Elem,_}) -> case Elem of {op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},{call,L,{remote,L,{atom,L,maps},{atom,L,Kf}},[_,A]}} when Kf =:= is_key; Kf =:= get -> false; _ -> true end end, element(1, Elems)), erlang:display({Rest, Asgn}),
 			{[{A, {map,L,[begin {op,L,'andalso',{call,L,{remote,L,{atom,L,erlang},{atom,L,is_map}},[A]},{call,L,{remote,L,{atom,L,maps},{atom,L,_}},[K,A]}} = X, {map_field_exact,L,K,{var,L,'_'}} end || {X, _} <- Asgn]}}|Rest],
 				element(2, Elems), Asgn ++ element(3, Elems), element(4, Elems), element(5, Elems)} end end;
 	%multiple tie-ins
 	{op,_,'andalso',A,B} -> case ispatternmatch(A, [], [], CleanAST, SubOut) of false -> false; Elems1 -> case ispatternmatch(B, BAST, CAST, CleanAST, SubOut) of false -> false; Elems2 -> {element(1, Elems1) ++ element(1, Elems2), element(2, Elems1) ++ element(2, Elems2), element(3, Elems1) ++ element(3, Elems2), element(4, Elems1) ++ element(4, Elems2), element(5, if element(5, Elems1) == {[], []} -> Elems2; true -> Elems1 end)} end
 	end;
	_ -> case isguardexpr(AST) of true -> erlang:display(AST), {[], [], [], case AST of {atom,_,true} -> []; _ -> [AST] end, {[], []}}; _ -> io:format("~p~n", [AST]), false end end
.

get_dependent_vars(Deps) ->
	lists:reverse(element(2, traverse_ast(fun (Elem, Acc) -> %get in order based on first dependency only - later ones would prevent single usage anyway
		{Elem, case Elem of {var,_L,X} when X =/= '_' -> [X|Acc];
		_ -> Acc end} end, [], Deps)))
.
...
cleanup_ast(AST, CleanAST, _VarPrefix, _AssignedVars, Beam, SubOut) ->
  ...
  %SkipBits = get_clean_ast(CleanAST, skip_bits, 2, SubOut),
	...
	IfCase = traverse_ast(fun (Elem, Acc) ->
		case Elem of
		{'case',L,A,[{clause,L,[{atom,L,true}],[],B},{clause,L,[{atom,L,false}],[],[{call,L,{remote,L,{atom,L,erlang},{atom,L,error}},[{atom,L,if_clause}]}]}]} ->
			case isguardexpr(A) of true -> %{'case',L,{atom,L,true},[{clause,L,[{atom,L,true}],[guardexprs(X) || X <- guardseqs(A)],B},{clause,L,[{atom,L,false}],[],C}]};
				{{'if',L,[{clause,L,[],[[A]],B}]}, Acc}; _ -> {Elem, Acc} end;
		{'case',L,A,[{clause,L,[{atom,L,true}],[],B},{clause,L,[{atom,L,false}],[],[{call,L,{remote,L,{atom,L,erlang},{atom,L,error}},[{atom,L,case_clause}]}]}]} ->
			{{'case',L,A,[{clause,L,[{atom,L,true}],[],B}]}, Acc};
		%{'case',L,A,[{clause,L,[B],[],MatchCase},{clause,L,[{var,L,'_'}],[],[{call,L,{remote,L,{atom,L,erlang},{atom,L,error}},[{tuple,L,[{atom,L,badmatch},C]}]}]}]} ->
			%if C =:= A -> {{block,L,[{match,L, B, A}|MatchCase]}, Acc}; true -> {Elem, Acc} end;
		{'case',L,A,[{clause,L,[{atom,L,true}],[],B},{clause,L,[{atom,L,false}],[],[{call,L,{remote,L,{atom,L,erlang},{atom,L,error}},[{tuple,L,[{atom,L,badmatch},C]}]}]}]} ->
			case ismatch(A, B, [], CleanAST, SubOut) of {[{E, D}], _, Vars, _, {MatchCase, _}} -> io:format("~p~n", [{"Match", C, D, E}]),if C =:= E -> if MatchCase =:= [] -> {{match,L, D, E}, Acc ++ Vars}; true -> {{block,L,[{match,L, D, E}|MatchCase]}, Acc ++ Vars} end; true -> {Elem, Acc} end; _ -> {Elem, Acc} end;
		{'case',L,A,[{clause,L,[{atom,L,true}],[],B},{clause,L,[{atom,L,false}],[],C}]} -> %erlang:display(Elem),
			case ismatch(A, B, C, CleanAST, SubOut) of {[{E, D}], _, Vars, Guards, {MatchCase, NoMatchCase}} -> erlang:display({"Here", MatchCase, NoMatchCase}),
				case NoMatchCase of [{call,L,{remote,L,{atom,L,erlang},{atom,L,error}},[{tuple,L,[{atom,L,badmatch},E]}]}] -> if MatchCase =:= [] -> {{match,L, D, E}, Acc ++ Vars}; true -> {{block,L,[{match,L, D, E}|MatchCase]}, Acc ++ Vars} end; _ ->
				{{'case',L,E,[{clause,L,[D],if Guards =:= [] -> []; true -> [Guards] end,MatchCase},{clause,L,[{var,L,'_'}],[],NoMatchCase}]}, Acc ++ Vars} end;
			_ -> case isguardexpr(A) of true -> %{{'case',L,{atom,L,true},[{clause,L,[{atom,L,true}],[guardexprs(X) || X <- guardseqs(A)],B},{clause,L,[{atom,L,false}],[],C}]}, Acc};
				if A =:= {atom,L,true} -> case B of [Bhd] -> {Bhd, Acc}; _ -> {{block,L,B}, Acc} end; true ->
				{{'if',L,[{clause,L,[],[[A]],B},{clause,L,[],[[{atom,L,true}]],C}]}, Acc} end; _ -> {Elem, Acc} end end;
		_ -> {Elem, Acc} end end, [], hd(CaseRetAST)),
	IfCaseAST = [element(1, IfCase)],
	%create equivalence classes first, then make sure each has a variable bound, or introduce new variables
	%efficiently do this by using a two way map - an exclusive and inclusive one - from an item to an equivalence class number, and from an equivalence class number to all of its items
	%variables chould go first at the head of the list always - but this is not sufficient and weakens restrictions, the first comparison or match goes first always...
	{EqClassFind, EqClass} = lists:foldl(fun ({El1, El2}, {AccExc, AccInc}) -> case AccExc of #{El1 := Num} ->
		case AccExc of #{El2 := Num2} -> if Num == Num2 -> {AccExc, AccInc}; true ->
				#{Num2 := L} = AccInc, #{Num := L2} = AccInc, {lists:foldl(fun (El, Acc) -> Acc#{El := Num} end, AccExc, L), maps:map(fun (El) -> if El >= Num2 -> El - 1; true -> El end end, maps:remove(Num2, AccInc#{Num := if Num2 < Num -> L2 ++ L; true -> L ++ L2 end}))} end;
			_ -> #{Num := L} = AccInc, {AccExc#{El2 => Num}, AccInc#{Num => [hd(L),El2|tl(L)]}} end;
			#{El2 := Num} -> #{Num := L} = AccInc, {AccExc#{El1 => Num}, AccInc#{Num => [hd(L),El1|tl(L)]}};
			_ -> Sz = maps:size(AccInc) + 1, {AccExc#{El1 => Sz, El2 => Sz}, AccInc#{Sz => [El2, El1]}} end end, {#{}, #{}}, element(2, IfCase)),
	%now these variable equivalence classes may contain each other - unfortunately there is complexity to replace all of them but it must be done - O(n^2)
	EqClassFindReps = maps:fold(fun (K, V, A) -> NewEl = element(1, traverse_ast(fun (Elem, Acc) -> {case A of #{Elem := X} -> #{X := L} = EqClass, hd(L); _ -> Elem end, Acc} end, [], K)), A#{NewEl => V} end, EqClassFind, EqClassFind),
	erlang:display({element(2, IfCase)}),
	erlang:display({EqClassFind}),
	erlang:display({EqClassFindReps}),
	PatMatVarAST = [element(1, traverse_ast(fun (Elem, Acc) ->
		case EqClassFindReps of #{Elem := X} -> #{X := L} = EqClass, {hd(L), Acc}; _ -> {Elem, Acc} end end, [], hd(IfCaseAST)))],
	%side effects numbered in order seen - partial ordering is enough
	%ordered directed rooted tree with edges with exactly one path from root to any node (arborescence) - not only the AST, but a variable side effect dependency tree as well
	%map stores tuple with: {reference count, has a side effect in its assignment, side effect occurs after assignment, usage occurs after side effect occurs after assignment, assignment value, index of order seen, dependent variables, for processing only: has yet excluded variable reference of match}
%	VarMap = element(2, traverse_ast(fun (Elem, Acc) ->
%		{Elem, case Elem of
%		{var,_L,X} when X =/= '_' -> case Acc of #{X := {V, Sd, Closed, _Necessary, D, Idx, Deps, _}} -> Acc#{X := {V+1, Sd, Closed, Closed, D, Idx, Deps, false}}; _ -> Acc#{X => {1, false, false, false, [], map_size(Acc), [], false}} end;
%		{match,L,{var,L,X},D} -> begin #{X := {V, Sd, Closed, Necessary, DOrig, Idx, _, _}} = Acc, Acc#{X => {V, Sd orelse V =:= 1 andalso asthassideeffect(D), Closed, Necessary, if V =:= 1 -> D; true -> DOrig end, Idx, get_dependent_vars(D), false}} end;
%		{call,_,{remote,_,{atom,_,A},{atom,_,B}},C} -> case hassideeffect(A, B, length(C)) of false -> Acc; _ -> maps:map(fun (_K, {V, Sd, _Closed, Necessary, D, Idx, Deps, _}) -> {V, Sd, true, Necessary, D, Idx, Deps, false} end, Acc) end;
		%do not forget if a single use variable moves forward, an implicit dependency exists so that the side effect may effect other variables
%		_ -> Acc end} end, #{}, hd(PatMatVarAST))),
	%detect function arguments by D being empty list, could merely not include them also as they are non-matched
	%have all variable dependencies, but what are needed for marking are dependents
%	Dependents = maps:fold(fun (K, {_, _, _, _, _, _, Deps, _}, AccIn) -> lists:foldl(fun (Dep, Acc) -> case Acc of #{Dep := _} -> Acc; _ -> Acc#{Dep => K} end end, AccIn, Deps) end, #{}, VarMap),
		%{function,L,FuncName,L,[{clause,L,Vars,_,_}]}
		%{'fun',L,{clauses,[{clause,L,Vars,_,_}]}} could detect function args where Vars=[{var,L,ArgName}|...]
		%local not global minimum by using aggressive/greedy strategy which always optimizes first found and records the side-effect into others
		%need to take each variable without any dependents and generate a dependent ordered list
		%then use a pruning algorithm to recursively determine based on the current level, and the level one more deep the number of variable assignments
		%this recurses to the bottom, then back up each time using the deeper level to calculate the two results for the current level, until back to the top again, complexity is minimized, should generalize to multi-use variables
%	_VarElim = [element(1, traverse_ast(fun (Elem, Acc) ->
%		case Elem of
%		{clause,L,Args,Guards,Code} -> {{clause,L,lists:map(fun (El) -> case El of {var,L,Name} -> case VarMap of #{Name := {1, _, _, _, _, _, _, _}} -> {var,L,list_to_atom([$_|atom_to_list(Name)])}; _ -> El end; _ -> El end end, Args),Guards,Code}, Acc};
%		{match,L,{var,L,X},D} -> {case Acc of #{X := {1, true, _, _, _, _, _, _}} -> D; #{X := {1, false, _, _, _, _, _, _}} -> {atom,L,true}; %if its the return value of a block would also be needed, but decompiler never generates assignment as return value of block - right now unnecessary
%			#{X := {2, Sd, _, Necessary, _, _, _, _}} when not Sd; not Necessary -> {atom,L,true}; %unnecessary or no side effect can be eliminated
%			_ -> Elem end, begin #{X := OrigX} = Acc, Acc#{X := setelement(8, OrigX, true)} end};
%		{var, _L, X} -> case Acc of #{X := {2, Sd, _, Necessary, D, _, _, true}} when not Sd, D =/= []; not Necessary, D =/= [] -> {D, case Dependents of #{X := XDepndt} -> #{XDepndt := V} = Acc, Acc#{X := setelement(2, V, true)}; _ -> Acc end}; _ -> {Elem, Acc} end;
%		_ -> {Elem, Acc} end end, VarMap, hd(PatMatVarAST)))],
	%optimization problem to minimize the number of variables in dependency graph!!!
	%naive strategy: any variable dependent on a side effect variable is considered to have a side effect
	%This was an older strategy to only eliminate and simplify specific single use variables for automated verification - but the above general algorithm should take its place
    ...
    _ -> E end end,
		...
		X -> X end, Acc} end, [], hd(PatMatVarAST)))], %hd(VarElim)))],
	InternalClean = [element(1, traverse_ast(fun (Elem, Acc) ->
		{case Elem of {call,L,{remote,0,{atom,_,erlang},{atom,L,Name}},A} -> case erl_internal:bif(Name, length(A)) andalso not lists:any(fun (Func) -> element(2, Func) =:= Name end, element(6, Beam)) of true -> {call,L,{atom,L,Name},A};
...
dodecompileast(Func, VarPrefix, Capts, Beam, CleanAST, Ctxt, Opts) ->
	...
	case lists:member(nosanitycheck, Opts) of false ->
		...
		case gb_sets:to_list(array:get(3-1, element(2, element(3, DecState)))) =/=
			...
			case array:to_list(PostDoms) =/= array:to_list(element(1, dom_to_d(get_pdom(element(3, DecState))))) of true ->
				error({"Bad post dominators", PostDoms, get_post_dom_divide(element(3, DecState)), check_post_doms(element(3, DecState)),
				element(1, dom_to_d(get_pdom(element(3, DecState))))}); _ -> true end;
		_ -> true end,
	...
	{ResAST, DepthList} = try {case lists:member(optimize, Opts) of true ->
		cleanup_ast(ResData, CleanAST, VarPrefix, element(8, DecState), Beam, lists:member(stubfuncs, Opts));
	_ -> min_cleanup_ast(ResData, VarPrefix, element(8, DecState)) end, get_ast_depth_agg(ResData)} catch Cause:Reason:StackTrace -> io:format("~p~n", [{Cause, Reason, StackTrace, ResData}]), {ResData, []} end,
...
decompileast(Fname, Funcname, Arity, Opts) ->
	Filename = case is_atom(Fname) of true -> code:which(Fname); _ -> Fname end,
	...
	{Funcs, Times, ErrMsgs} =
		lists:foldl(fun(Func, {F, T, E}) ->
			...
			atom_to_list(element(2, Func))) of true -> {F, T, E}; _ ->
				...
				catch Y:Z:ST -> ErrMsg = io_lib:format("~p~n",
					[{Filename, element(2, Func), element(3, Func),
						Y, Z, ST}]), FinErr = lists:flatten(ErrMsg), %ST=erlang:get_stacktrace()
					io:format("~s", [FinErr]),
					case Z of {"Bad reverse graph", _, _, _} -> true; _ -> erlang:raise(error, {Y, Z}, ST) end,
