%c("src/decomptest.erl", [debug_info,{outdir, "ebin"}]).
-module(decomptest).
-import(decomp, [traverse_ast/3, decompileast/4]).

-export([order/0, expressive/0, incstruct/5, incstructlateerr/5, papertryreceive/2, paperrevgraph/5,
	unsafevar/1,
	matchhelper/0,
	matchlistskip/0, matchlistpreexistvar/0, matchlistresult/0, matchlistrecursive/0,
	matchtupleskip/0, matchtuplepreexistvar/0, matchtupleresult/0, matchrepeat/0,
	matchmapskip/0,
...
order() -> 
  [[begin erlang:display("Head") end|[begin erlang:display("Tail"), [] end]],
  [erlang:display("Value") || _ <- begin erlang:display("GeneratorsConditions1"), [1,1] end, X <- begin erlang:display("GeneratorsConditions2"), [2,3] end, begin erlang:display("Condition1"), true end, begin erlang:display("Condition2"), X =/= 2 end, begin erlang:display("Condition3"), true end],
  [erlang:display("Value") || <<_>> <= begin erlang:display("GeneratorsConditions1"), <<1, 1>> end, <<X>> <= begin erlang:display("GeneratorsConditions2"), <<2, 3>> end, begin erlang:display("Condition1"), true end, begin erlang:display("Condition2"), X =/= 2 end, begin erlang:display("Condition3"), true end],
  {begin erlang:display("Element1"), 1 end, begin erlang:display("Element2"), 2 end},
  #{begin erlang:display("Key1"), 1 end=>erlang:display("Value1"), begin erlang:display("Key2"), 2 end=>erlang:display("Value2")},
  begin erlang:display("Map"), #{1 => true, 2 => true} end#{begin erlang:display("Key1"), 1 end:=erlang:display("Value1"), begin erlang:display("Key2"), 2 end:=erlang:display("Value2")},
  begin erlang:display("Map"), #{1 => true, 2 => true} end#{begin erlang:display("Key1"), 3 end=>erlang:display("Value1"), begin erlang:display("Key2"), 4 end=>erlang:display("Value2")},
  begin erlang:display("Function"), fun (A, B, C) -> A + B + C end end(begin erlang:display("Argument1"), 1 end, begin erlang:display("Argument2"), 2 end, begin erlang:display("Argument3"), 3 end),
  _A = begin erlang:display("Expression"), 1 end,
  -begin erlang:display("Expression"), 1 end,
  begin erlang:display("Expression1"), self() end ! begin erlang:display("Expression2"), "msg" end,
  <<begin erlang:display("Element1"), 3 end, begin erlang:display("Element2"), 3 end>>,
  begin erlang:display("Module"), erlang end:begin erlang:display("Function"), display end(erlang:display("Argument")),
  begin erlang:display("Statement1"), erlang:display("Statement2") end,
	receive _ -> erlang:display("ReceiveBlock") after begin erlang:display("Timeout"), 0 end -> erlang:display("After") end,
	catch begin erlang:display("Statement1"), erlang:display("Statement2") end].

%left sided matches are really an AST issue, probably the compiler just rearranges always right-to-left, and assignment in context of bad match is either all or nothing
expressive() -> A = case true of C = true -> C; _ -> false end,
  [A, (B = B) = A, B = (B = A)].
  
%convenient short-hand terminology: function/fun/named fun body, matched if, case/matched case-of, try/matched try-of/matched try-catch/try-after, matched receive/receive-after clauses
%guarded matches: function/fun/named fun, if, case-of, try-of, try-catch, receive
%scopes:
%match - nearest try/try-of/try-catch/try-after/receive/receive-after/catch level otherwise function/fun/named fun level - bound all or nothing on match - but not in eval module which has top-level scope
%function arguments/case-of/try-of/try-catch/receive - scoped under bound structure - bound on match
...
paperrevgraph(A, B, C, D, E) ->
	if A and B -> D;
	A =:= true, B =:= false -> error(E);
	A =:= false, C =:= true -> paperrevgraph(A, B, C, D, E), error(E);
	true -> F = paperrevgraph(A, B, C, D, E), error(F) end.
...
unsafevar(A) -> %only when matching to a side effect value
  {B, C} = A(),
  {D, E} = A(),
  B + D.
...
matchtuplepreexistvar() ->
	[fun(A, B) -> A = {B}, true end,
	fun(A, B) -> {B} = A, true end,
	fun(A, B) -> case A of {B} -> true; _ -> false end end,
	fun(A, B) -> case {B} of A -> true; _ -> false end end,
	fun(A, B, C, D) -> A = {B, C, D}, true end,
	fun(A, B, C, D) -> {B, C, D} = A, true end,
	fun(A, B, C, D) -> case A of {B, C, D} -> true; _ -> false end end,
	fun(A, B, C, D) -> case {B, C, D} of A -> true; _ -> false end end].
	
matchtupleskip() ->
	%[fun(A) -> {_} = A, true end,
	fun(A) -> case A of {_} -> true; _ -> false end end.
	%fun(A, B) -> {_, B, _} = A, true end,
	%fun(A, B) -> case A of {_, B, _} -> true; _ -> false end end].
	
matchtupleresult() ->
	[fun(A) -> {B} = A, B end,
	fun(A) -> case A of {B} -> B; _ -> false end end,
	fun(A) -> {_, B, _} = A, B end,
	fun(A) -> case A of {_, B, _} -> B; _ -> false end end].


matchlistpreexistvar() ->
	[fun(A, B) -> A = [B], true end,
	fun(A, B) -> [B] = A, true end,
	fun(A, B) -> case A of [B] -> true; _ -> false end end,
	fun(A, B) -> case [B] of A -> true; _ -> false end end,
...
matchlistskip() ->
	...
	fun(A, B) -> [_, B, _] = A, true end,
	fun(A, B) -> case A of [_, B, _] -> true; _ -> false end end,
	fun(A, B) -> [B, _|_] = A, true end,
	fun(A, B) -> case A of [B, _|_] -> true; _ -> false end end].
...
matchlistresult() ->
	...
	fun(A) -> case A of [_, B, _] -> B; _ -> false end end,
	fun(A) -> [B, _|_] = A, B end,
	fun(A) -> case A of [B, _|_] -> B; _ -> false end end].
	
matchlistrecursive() ->
[fun(A, B) -> [B, [_]] = A, true end,
	fun(A, B) -> case A of [B, [_]] -> true; _ -> false end end,
	fun(A, B, C, D) -> A = [B, [C, D]], true end,
	fun(A, B, C, D) -> [B, [C, D]] = A, true end,
	fun(A, B, C, D) -> case A of [B, [C, D]] -> true; _ -> false end end,
	fun(A, B, C, D) -> case [B, [C, D]] of A -> true; _ -> false end end,
...
matchmapskip() ->
	...
	fun (A, B) -> case A of #{B := _} -> true; _ -> false end end,
	fun (A, B) -> #{B := C} = A, C end,
	fun (A, B) -> case A of #{B := C} -> C; _ -> false end end]
.
...
testfunref() -> [fun erlang:list_to_binary/1, fun testfun/0].
